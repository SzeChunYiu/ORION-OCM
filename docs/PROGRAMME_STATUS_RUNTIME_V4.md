# Runtime V4 programme status — historical terminals and current scope

This table preserves the historical roadmap v1 terminals from `docs/provenance/M*_RECEIPT_V1.json`.
Those immutable receipts describe the code and evidence at their recorded revisions.

**Current revision, 2026-09-05:** M11 is `M11_ENGINEERING_REVALIDATION__HISTORICAL_ADOPTION_CELLS_REOPENED`;
M12 is `M12_ENGINEERING_REVALIDATION__PROTECTED_REEVALUATION_REQUIRED`. Historical M11 adoption cells and
M12 phase G used invalid predecessor bindings. Fixed runtime checks and same-scenario replays do
not revalidate their scientific conclusions. See [the correction report](RUNTIME_LIFECYCLE_REVALIDATION_V2.md).

The chain `python tools/mN_receipt.py --verify` (N=1…12) now checks the declared
[successor receipts](provenance/runtime_revision_20260905_v4/), their current source bindings,
dependency aliases and unchanged historical custody. Missing successors fail closed. Original
receipt generation is disabled; a new revision needs explicit evidence and receipt identities.
...
* **Historical M12 `FULL_OCM_RESIDUAL_SUPPORTED`** was scoped relative to the matched whole-system parent
  ...
  arm were CANNOT_CHECK in that evaluation environment. The current corrected engine requires
  protected reevaluation; the historical terminal is not inherited by a reference replay.

...
**Documented tightening (M2, MEG-06 / ORION-V2 batch-1 T2).** The canonical `navigate` iterates from `a_0 = α·s` so that the iterates are the partial Neumann sums, monotone from below and ≤ a*: a FOUND at any budget is sound for the fixed point, and `BUDGET_EXHAUSTED` carries the bracket `a*(t) ∈ [a_k(t), a_k(t) + (1−α)^{k+1}‖s‖₁]` (reason `BUDGET_BRACKET_EXCLUDES_THRESHOLD` when the bracket decides negatively). The frozen reference started at `s`, whose iterates overshoot; old-vs-new equivalence therefore compares outcomes, not `steps_used`.

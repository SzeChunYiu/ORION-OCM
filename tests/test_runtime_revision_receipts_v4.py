"""Fourth successors verify frozen parent code without reusing old runtime authority."""
...
SPEC = importlib.util.spec_from_file_location("receipt_revision_v4_tests", ROOT / "tools/runtime_revision_receipts_v4.py")
...
def refresh_replay(root):
    ...
    save(root, config["engineering_replay_path"], {
        "schema": "ocm.engineering-replay.v4", "revision": R.REVISION,
...
def baseline(tmp_path_factory):
    root = tmp_path_factory.mktemp("receipt-v4-parent")
    config = R.read_json(ROOT, R.CONFIG_PATH)
    ...
    upstream_manifest = R.read_json(ROOT, config["upstream_manifest"]["path"])
    for rel in (config["upstream_manifest"]["path"], upstream_manifest["archive"]["path"]):
        path = root / rel
        path.parent.mkdir(parents=True, exist_ok=True)
        shutil.copyfile(ROOT / rel, path)
    ...
    with zipfile.ZipFile(ROOT / upstream_manifest["archive"]["path"]) as archive:
        for rel in upstream_manifest["entries"]:
...
def test_editable_install_metadata_is_generated_but_extra_code_is_still_bound(evidence):
    before = R.source_inventory(evidence)
    path = evidence / "src/orion_ocm.egg-info"
    path.mkdir()
    (path / "PKG-INFO").write_text("Name: orion-ocm\nVersion: 0.1.0\n")
    (path / "SOURCES.txt").write_text("src/ocm/__init__.py\n")
    assert R.source_inventory(evidence) == before
    R.CurrentReceipts(evidence).verify(12)
    (path / "unexpected.py").write_text("value = 1\n")
    with pytest.raises(R.ReceiptError, match="DRIFT"):
        R.CurrentReceipts(evidence).verify(12)
...
def test_upstream_archive_and_merged_history_are_bound(evidence):
    config = R.read_json(evidence, R.CONFIG_PATH)
    manifest = R.read_json(evidence, config["upstream_manifest"]["path"])
    path = evidence / manifest["archive"]["path"]
    path.write_bytes(path.read_bytes() + b"changed")
    with pytest.raises(R.ReceiptError):
        R.CurrentReceipts(evidence).verify(12)


def test_both_parent_histories_remain_readable(evidence):
    config = R.read_json(evidence, R.CONFIG_PATH)
    snapshot = R.ParentSnapshot(evidence, config)
    assert snapshot.contents["docs/provenance/M12_RECEIPT_V1.json"] != snapshot.upstream_contents["docs/provenance/M12_RECEIPT_V1.json"]
    assert (evidence / "docs/provenance/M12_RECEIPT_V1.json").read_bytes() == snapshot.upstream_contents["docs/provenance/M12_RECEIPT_V1.json"]
    for receipt in ("M12_REFERENCE_RECEIPT_V1.json", "M12_PAIRED_RECEIPT_V1.json", "M12_PAIRED_RECEIPT_V4.json"):
        assert R.verify_upstream_receipt(evidence, "docs/provenance/" + receipt)["legacy_recipe_execution"] == "NOT_EXECUTED"

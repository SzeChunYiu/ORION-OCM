"""Third successors verify frozen parent code without reusing old runtime authority."""
...
SPEC = importlib.util.spec_from_file_location("receipt_revision_v3_tests", ROOT / "tools/runtime_revision_receipts_v3.py")
...
def refresh_replay(root):
    ...
    save(root, config["engineering_replay_path"], {
        "schema": "ocm.engineering-replay.v3", "revision": R.REVISION,
...
def baseline(tmp_path_factory):
    root = tmp_path_factory.mktemp("receipt-v3-parent")
    config = R.read_json(ROOT, R.CONFIG_PATH)

"""H1 amortized-lifetime protocol tests. Production src is imported, not copied."""
...
V1_RESULT = Path(__file__).resolve().parents[1] / "h1-amortized-acquisition-v1" / "RESULT.json"
V2_RESULT = Path(__file__).resolve().parents[1] / "h1-amortized-rewrite-v2" / "RESULT.json"


class TestH1AmortizedLifetime(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_salts_are_new_not_v1_v2_or_g2(self):
        salts = {E.TRAIN_SALT, E.VAL_SALT, E.LATER_SALT}
        self.assertTrue(all(s.startswith("orion-ocm-h1-lifetime-") for s in salts))
        self.assertTrue(all(s.endswith("-v3") for s in salts))
        ...
        self.assertNotIn("orion-ocm-h1-rewrite-train-v2", salts)
    ...
    def test_acquisition_is_scan_plus_single_confirmation_not_tournament(self):
        library = self.result["library"]
        self.assertFalse(library["tournament_run"])
        self.assertIsNone(library["tournament_compute"])
        self.assertEqual(library["scan_enumeration_attempts"], 0)
        self.assertGreaterEqual(library["scan_token_operations"], 0)
        self.assertEqual(self.result["acquisition"], "search-aware-scan-plus-single-confirmation")
        self.assertLessEqual(library["val_indexes_built"], 2)
        self.assertEqual(
            library["cost"]["compute"],
            library["training_compute"] + library["confirmation_compute"],
        )
        if library["search_aware_selected"]:
            self.assertEqual(library["val_indexes_built"], 2)
            confirmation = library["confirmation"]
            self.assertEqual(
                library["confirmation_compute"],
                confirmation["primitive_val_compute"] + confirmation["selected_val_compute"],
            )
        self.assertIn("DROPPED_ACQUISITION_COST_FROM_LEDGER", self.result["not_issued"])

    def test_scan_token_ops_are_not_added_to_compute(self):
        library = self.result["library"]
        compute = library["cost"]["compute"]
        charged_search = library["training_compute"] + library["confirmation_compute"]
        self.assertEqual(compute, charged_search)
        if library["scan_token_operations"]:
            self.assertNotEqual(compute, charged_search + library["scan_token_operations"])

    def test_library_overrun_is_first_class_negative(self):
        terminal = self.result["terminal"]
        self.assertIn(
            terminal,
            self.result["negative_terminals_frozen"] + ["AMORTIZED_ACQUISITION_SUPPORTED_AT_POLYNOMIAL_SCOPE"],
        )
        ...
        if terminal == "AMORTIZED_ACQUISITION_SUPPORTED_AT_POLYNOMIAL_SCOPE":
            ...
            self.assertIsNotNone(lifetime["break_even_n"])
            self.assertLessEqual(lifetime["break_even_n"], E.LATER_N)
        else:
        ...
        if later_cheaper(self.result) and lifetime["kt"]["compute"] >= lifetime["k0"]["compute"]:
            ...
            self.assertIsNotNone(lifetime["break_even_n"])
            self.assertGreater(lifetime["break_even_n"], E.LATER_N)
    ...
    def test_search_aware_is_a_scan_with_zero_enumeration(self):
        programs = [("inc", "square", "double", "inc"), ("square", "square", "dec", "double")]
        candidates = (("inc", "square"), ("square", "square"), ("inc", "inc"))
        scores, token_ops = E.score_search_aware(candidates, programs)
        self.assertEqual(token_ops, sum(len(program) for program in programs) * len(candidates))
        self.assertEqual(set(scores), set(candidates))
        winner, _scores2, ops2 = E.select_search_aware(candidates, programs)
        self.assertEqual(ops2, token_ops)
        self.assertIn(winner, candidates)
        square_gain = scores[("square", "square")]
        miss_gain = scores[("inc", "inc")]
        self.assertGreater(square_gain, miss_gain)
        self.assertEqual(miss_gain, 0.0)
        self.assertGreater(E.cumulative_words(4), E.cumulative_words(3))

    def test_search_aware_zeroes_g2_widening_term(self):
        """Rewrite serving does not add a token; a length-2 hit reduces depth, not width."""
        program = ("square", "square", "inc", "dec")
        fragment = ("square", "square")
        _word, used, op_cost = E.greedy_rewrite(program, fragment)
        self.assertTrue(used)
        self.assertEqual(op_cost, 3)
        gain = E.cumulative_words(4) - E.cumulative_words(3)
        scores, _ops = E.score_search_aware((fragment,), (program,))
        self.assertEqual(scores[fragment], float(gain))
    ...
    def test_capsule_does_not_patch_production_src_or_add_macro_token(self):
        ...
        self.assertIn("score_search_aware", text)

    def test_does_not_touch_v1_or_v2_trees(self):
        v1 = Path(__file__).resolve().parents[1] / "h1-amortized-acquisition-v1"
        v2 = Path(__file__).resolve().parents[1] / "h1-amortized-rewrite-v2"
        capsule = Path(__file__).resolve().parent
        self.assertEqual(capsule.name, "h1-amortized-lifetime-v3")
        self.assertNotEqual(v1.resolve(), capsule.resolve())
        self.assertNotEqual(v2.resolve(), capsule.resolve())

    def test_predecessor_result_files_frozen(self):
        ...
        self.assertEqual(v1["terminal"], "NO_AMORTIZED_ACQUISITION_LATER_NOT_CHEAPER")
        self.assertEqual(v1["later"]["k0"]["compute"], 50176)
        self.assertEqual(v1["later"]["kt"]["compute"], 62043)
        self.assertEqual(v2["terminal"], "LIBRARY_ACQUISITION_EXCEEDS_LATER_SAVINGS")
        self.assertEqual(v2["later"]["k0"]["compute"], 59929)
        self.assertEqual(v2["later"]["kt"]["compute"], 36289)
        self.assertFalse(v2["lifetime"]["library_pays_back_compute"])
        self.assertEqual(self.result["predecessor_v1_terminal"], v1["terminal"])
        self.assertEqual(self.result["predecessor_v2_terminal"], v2["terminal"])
    ...
    def test_break_even_horizon_is_reported_honestly(self):
        lifetime = self.result["lifetime"]
        self.assertIn("break_even_n", lifetime)
        self.assertEqual(lifetime["observed_later_n"], 48)
        if later_cheaper(self.result):
            self.assertGreater(lifetime["saving_per_later_identity"], 0)
            self.assertIsInstance(lifetime["break_even_n"], int)
            self.assertGreater(lifetime["break_even_n"], 0)
...
class TestH1LifetimeResultFreeze(unittest.TestCase):
    def test_written_result_matches_schema(self):
        ...
        self.assertEqual(data["acquisition"], "search-aware-scan-plus-single-confirmation")
        self.assertEqual(data["predecessor_v1_terminal"], "NO_AMORTIZED_ACQUISITION_LATER_NOT_CHEAPER")
        self.assertEqual(data["predecessor_v2_terminal"], "LIBRARY_ACQUISITION_EXCEEDS_LATER_SAVINGS")
        self.assertFalse(data["library"]["tournament_run"])
        self.assertEqual(data["partition"]["later_n"], 48)
        self.assertIn("not retuned", data["mechanism_comment"])
        self.assertEqual(data["terminal"], "AMORTIZED_ACQUISITION_SUPPORTED_AT_POLYNOMIAL_SCOPE")
        self.assertEqual(data["lifetime_terminal"], "LIFETIME_LIBRARY_PAYBACK_ON_COMPUTE")
        self.assertEqual(
            data["earned"],
            ["H1/001-new_information", "H1/004-compute", "H1/005-verifier_calls"],
        )
        self.assertEqual(data["library"]["admitted"], ["double", "square"])
        self.assertLess(data["later"]["kt"]["compute"], data["later"]["k0"]["compute"])
        self.assertTrue(data["lifetime"]["library_pays_back_compute"])
        self.assertEqual(data["later"]["k0"]["compute"], 183463)
        self.assertEqual(data["later"]["kt"]["compute"], 144623)
        self.assertEqual(data["library"]["cost"]["compute"], 27919)
        self.assertEqual(data["lifetime"]["kt"]["compute"], 172542)
        self.assertEqual(data["lifetime"]["k0"]["compute"], 183463)
        self.assertEqual(data["lifetime"]["break_even_n"], 35)
        self.assertTrue(data["parent_sufficient"])
        self.assertFalse(data["h1_vector_strictly_less"])

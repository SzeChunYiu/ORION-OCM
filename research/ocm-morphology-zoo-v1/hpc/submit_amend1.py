"""AMEND-1 campaign driver: freeze the amend-1 manifest, sync to LUNARC,
smoke, submit the production array, and chain the aggregate after it.

Sequence (run from the Mac worktree):
  1. write manifests/CAMPAIGN_AMEND1_MANIFEST.json (frozen task list;
     sha256 of FREEZE_V1_AMEND_1.json + capsule code + P00B truth head;
     never rewritten in place)
  2. rsync capsule to <lunarc>:$HOME/zoo221/capsule (binary-safe)
  3. sbatch smoke_amend1 (--qos=test) and require "SMOKE OK"
  4. sbatch qd_amend1 array (production; %K concurrency cap)
  5. sbatch aggregate_amend1 with --dependency=afterok:<array>

Usage: python3 submit_amend1.py <capsule_root> <lunarc_host>
"""
...
def main():
    ...
    budget = a1["production_MZ_D3_amend1"]["budget"]
    ...
    sh("cd %s && find . \\( -name '*.py' -o -name '*.json' \\) | LC_ALL=C sort | "
       "xargs sha256sum > /tmp/zoo_a1_local.list" % ROOT)
    sh("ssh %s \"cd zoo221/capsule && find . \\\\( -name '*.py' -o -name '*.json' \\\\) | "
       "LC_ALL=C sort | xargs sha256sum > /tmp/zoo_a1_remote.list\"" % HOST)
    sh("scp -q %s:/tmp/zoo_a1_remote.list /tmp/zoo_a1_remote.list.local" % HOST)
    diff = sh("diff /tmp/zoo_a1_local.list /tmp/zoo_a1_remote.list.local", check=False)
    if diff.returncode != 0:
        print(diff.stdout[-3000:])
        raise SystemExit("capsule checksum mismatch after rsync (listing above)")
    chk = sh("sha256sum /tmp/zoo_a1_local.list | cut -d' ' -f1").stdout.strip()
    print("capsule synced byte-identical, listing digest %s" % chk[:16])

    # smoke first (--qos=test only for short smoke jobs)
    r = sh("ssh %s 'cd zoo221/capsule && sbatch --parsable hpc/smoke_amend1.sbatch'" % HOST)
    ...
    ok = sh("ssh %s 'grep -c \"SMOKE OK\" zoo221/capsule/logs/smoke_a1_%s.out'" % (HOST, jid), check=False)
    if "1" not in ok.stdout:
        raise SystemExit("amend-1 smoke did not pass (state=%s) — refusing production submit" % state)
    print("smoke OK")

    # production array + chained aggregate
    r = sh("ssh %s 'cd zoo221/capsule && sbatch --parsable hpc/qd_amend1.sbatch'" % HOST)
    ...
    r = sh("ssh %s 'cd zoo221/capsule && sbatch --parsable --dependency=afterok:%s "
           "hpc/aggregate_amend1.sbatch'" % (HOST, aid))

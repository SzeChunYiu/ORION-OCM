"""Row-L outcome producer on the posterior-dated sources (FREEZE_V1.md section 4).

The parent's ``train_continual_v3.py`` instrument, byte-for-byte in every
registered constant, seed rule, net, criterion and split.  The ONLY differences:
the source table is read from ``POSTERIOR_SOURCES_V1.json`` (the admitted
posterior sources), ``B_prop`` per source is set by the frozen pilot rule PR-1
(smallest grid budget with p0 in [9/24, 21/24], baseline law only), and the
sequence ids are T21..T26 (POS) / T31..T36 (NEG).

Runs on laptop-billy after commit 2b.  Writes REAL_RUNS_L4/pilot_<sid>.json and
REAL_RUNS_L4/cl4_<sid>.json.  All verdicts are computed by the route A scorer.

Usage: python3 -B train_continual_v4.py <POSTERIOR_SOURCES_V1.json> <out dir>
"""
...
PILOT_LO, PILOT_HI = Fraction(9, 24), Fraction(21, 24)
...
def baseline_hits(n_sid, Xtr, Ytr, Xev, Yev, budget):
    ...
    for i in range(N_PROP):
        ...
        if correct(c, Xev, Yev) >= crit_correct:
            hits += 1
    return hits


def pilot(sid, src, outdir):
    """PR-1: baseline law only; smallest grid budget with p0 in [9/24, 21/24]."""
    n_sid = int(sid[1:])
    need = (K_SEQ + 1) * (T_TRAIN + T_EVAL) + NFEAT + 64
    bits = bits_from_file(src, need)
    Xtr, Ytr, Xev, Yev = u_split(bits)
    grid = {}
    chosen = None
    for b in PILOT_GRID:
        h = baseline_hits(n_sid, Xtr, Ytr, Xev, Yev, b)
        grid[str(b)] = h
        if chosen is None and PILOT_LO <= Fraction(h, N_PROP) <= PILOT_HI:
            chosen = b
    rec = {"sequence_id": sid, "source": src, "source_sha256": sha256_file(src),
           "pilot_grid": grid, "rule": "smallest budget with p0 in [9/24, 21/24]",
           "B_prop": chosen, "status": "OK" if chosen else "AT_FLOOR_OR_CEILING"}
    os.makedirs(outdir, exist_ok=True)
    with open(os.path.join(outdir, "pilot_%s.json" % sid), "w") as f:
        json.dump(rec, f, indent=1, sort_keys=True)
        f.write("\n")
    print("pilot", sid, grid, "->", chosen, flush=True)
    return chosen


def run(spec, outdir):
    sid, fam, src, seq, uoffs, b_prop, source_id = spec
    B_PROP = b_prop
    ...
    os.makedirs(outdir, exist_ok=True)
    p = os.path.join(outdir, "cl4_%s.json" % sid)
    with open(p, "w") as f:
        json.dump(rec, f, indent=1, sort_keys=True)
        f.write("\n")
...
def main():
    ...
    for n, src in enumerate(record["admitted"]):
        ...
        b_prop = pilot(pos_id, path, outdir)
        if b_prop is None:
            print(src["source_id"], "AT_FLOOR_OR_CEILING -> not scored", flush=True)
            continue
        run((pos_id, "POS", path, POS_SEQ, UOFF, b_prop, src["source_id"]), outdir)
        run((neg_id, "NEG", path, NEG_SEQ, UOFF, b_prop, src["source_id"]), outdir)

"""Re-execute only the new finite/static checker with fixed full authority."""
import json
import subprocess
import sys
...
def replay(root=None,worker=None):
    ...
    if worker is None:
        ...
        run=subprocess.run([sys.executable,*flags,str(root/"check_memory_teaching_v1.py")],
                           cwd=root,capture_output=True,timeout=60,check=False)
        if run.returncode or run.stderr:
            raise ValueError("finite/static worker failed")
        actual=run.stdout
    else:
        actual=worker(root)

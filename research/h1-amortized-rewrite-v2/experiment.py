"""Issue #165 H1 amortized-acquisition successor: rewrite in the 4-primitive grammar.
...
This capsule serves the earned fragment as a rewrite/operator inside the same
four primitives, replacing matching sequences rather than adding a token.

Production methods.py is imported, not copied. No src edits. New salts.
Ordinary-parent parity and restart/revocation. Library capital that exceeds
later-task savings is a first-class negative.
"""
...
SCHEMA = "ocm.h1.amortized-rewrite.v2"
METHOD_BLOB = "50323a33418b8ef8bb6500ddeba4b9d1f795e9e3"
TRAIN_SALT = "orion-ocm-h1-rewrite-train-v2"
VAL_SALT = "orion-ocm-h1-rewrite-val-v2"
LATER_SALT = "orion-ocm-h1-rewrite-later-v2"
TRAIN_N = 16
VAL_N = 16
LATER_N = 16
TRAIN_LEN = 4
...
SCOPE = Scope.of("polynomial-h1-amortized-rewrite.v2")
...
def admit_fragment(root: Path, fragment, training_receipt, utility_receipt, revoke=False):
    ...
    _tr, training_evidence = runtime.admit_evidence(
        training_receipt, Channel.PROOF, "h1-rewrite-training.v2", scope=SCOPE
    )
    _ur, utility_evidence = runtime.admit_evidence(
        utility_receipt, Channel.OBSERVATION, "h1-rewrite-utility.v2", scope=SCOPE
    )
    warrant = WarrantProfile.of({training_evidence, utility_evidence})
    source_payload = {
        "kind": "h1.rewrite.support.v2",
        "training": content_hash(training_receipt),
        "utility": content_hash(utility_receipt),
    }
    source_id = "h1-rewrite-support:" + content_hash(source_payload)
...
def mechanism_comment(terminal: str, admitted, later_kt, later_k0, library, lifetime_kt, lifetime_k0) -> str:
    if terminal == "UTILITY_GATE_SELECTS_NO_METHOD":
        return (
            "No rewrite earned: every training-derived fragment lost aggregate validation "
            "search to the primitive 4-ary index. H1 later-task comparison was not run under an earned library."
        )
    ...
    if terminal == "NO_AMORTIZED_ACQUISITION_LATER_NOT_CHEAPER":
        ...
        info_delta = later_k0["new_information"] - later_kt["new_information"]
        return (
            f"Earned rewrite {list(admitted) if admitted else []} was admitted by a length-5 validation "
            ...
            "Mechanism: the fragment is a greedy-leftmost rewrite inside the same 4-primitive grammar "
            "(not a 5th BFS token). v1's 5-ary MACRO product is closed; this v2 serving still failed "
            f"to make C_acquire later cheaper than reset. Unique-program delta vs K_0 is {info_delta} "
            ...
            "library ties OCM (PARENT_SUFFICIENT vs program-library rewrite). Salts and counts "
            "were not retuned."
        )
    if terminal == "LIBRARY_ACQUISITION_EXCEEDS_LATER_SAVINGS":
        saved = later_k0["compute"] - later_kt["compute"]
        return (
            "Utility-gated 4-primitive rewrite reduces later related-task search against reset, but "
            f"library capital does not pay back on this frozen 16/16/16 length-4/5/6 horizon: "
            f"training+tournament compute {library['compute']} plus later K_t {later_kt['compute']} "
            f"= {lifetime_kt['compute']} >= K_0 later {lifetime_k0['compute']} "
            f"(later-task compute saving {saved}). The dominant mechanism is the validation tournament: "
            "every candidate is fully searched on the disjoint validation stratum before admission, "
            "and that selection work exceeds later savings. Training demonstrations and persist bytes "
            "are additional unrecouped capital. Ordinary persistent library ties OCM "
            "(PARENT_SUFFICIENT vs program-library rewrite). Salts and counts were not retuned."
        )
    if terminal == "AMORTIZED_ACQUISITION_SUPPORTED_AT_POLYNOMIAL_SCOPE":
        return (
            "Later related-task C_acquire is strictly cheaper given the earned 4-primitive rewrite, "
            "and library capital is recouped on the frozen compute coordinate. Ordinary parent ties "
            "OCM. Not a programme-wide H1 close and not an OCM residual over library rewrite. "
            "v1's 5-ary MACRO token is not used."
        )
    return terminal
...
def run_study() -> dict[str, Any]:
    ...
    for salt in (TRAIN_SALT, VAL_SALT, LATER_SALT):
        if salt in FOREIGN_SALTS:
            raise RuntimeError(f"H1 rewrite salt collides with a frozen predecessor salt: {salt}")
    ...
    for candidate in candidates:
        ...
        if index["grammar_width"] != 4 or index["grammar_tokens"] != list(M.PRIMITIVES):
            raise RuntimeError("rewrite index widened the grammar")
        if index["total_enumeration_attempts"] != index["total_unique_candidates_checked"]:
    ...
    if admitted is None:
        ...
        with tempfile.TemporaryDirectory(prefix="ocm-h1-rewrite-") as temp_dir:
            root = Path(temp_dir)
    ...
    return {
        ...
        "parent": "ordinary persistent rewrite/library operator inside the 4-primitive grammar (DreamCoder/Stitch/#192 replacement, no 5th token)",
        ...
        "claim_ceiling": (
            ...
            "4-primitive rewrite. Ordinary parent parity and restart/revocation are required. Library "
            "capital is first-class. Not programme-wide H1, not P6 residual, not language meta-learning, "
            "not an OCM architecture uniqueness claim, not a salt retune of H1 v1."
        ),

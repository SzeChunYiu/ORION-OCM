"""Section L V2 outcome producer (FREEZE_V2_REVIVAL.md).

Created AFTER revival freeze commit bdf2cdab719396f67b8feef9834a25d45ef40103.
Runs on the execution host (laptop-billy, torch CPU). Writes only
REAL_RUNS/cl2_<id>.json; all verdicts are computed by the Route A scorer.

Usage: python3 -B train_continual_v2.py <out REAL_RUNS dir>
"""
...
NFEAT = 16
K_SEQ = 4
SEQ_STEPS = 480          # h_high = K_SEQ * SEQ_STEPS = 1920
B_PROP = 120             # = c
B1, B2 = 40, 120         # DP-1 budget ladder
N_PROP = 24
BLOCKS = 4
...
SEQUENCES = [
    ("S1", "POS", SRC_ROOT + "/g1342.txt",
     ((1, 3), (2, 4), (1, 4), (2, 3)), (1, 2)),
    ("S2", "POS", "/usr/share/sounds/alsa/Noise.wav",
     ((1, 2), (3, 4), (1, 4), (2, 4)), (2, 3)),
    ("S3", "POS", "/usr/lib/python3.8/json/__init__.py",
     ((1, 2), (2, 3), (3, 4), (2, 4)), (1, 3)),
    ("S4", "POS", "/usr/share/dict/american-english",
     ((1, 2), (1, 3), (2, 3), (3, 4)), (2, 4)),
    ("S5", "POS", SRC_ROOT + "/g84.txt",
     ((1, 2), (2, 3), (3, 4), (1, 3)), (1, 4)),
    ("S6", "NEG", "/usr/bin/python3.8",
     ((12, 13), (13, 14), (14, 15), (12, 15)), (1, 2)),
    ("S7", "NEG", "/usr/share/sounds/alsa/Rear_Left.wav",
     ((12, 14), (13, 15), (12, 15), (13, 14)), (2, 3)),
    ("S8", "NEG", "/usr/share/dict/swedish",
     ((0, 0), (0, 0), (0, 0), (0, 0)), (1, 3)),
]
...
def run(spec, outdir):
    ...
    p = os.path.join(outdir, "cl2_%s.json" % sid)
    with open(p, "w") as f:

...
| Ecology extension 15 coordinates + remint guard (`ECOLOGY_EXTENSION_CONTRACT_V1.json`, this lane, sibling `gmi-ecology-extension-v1` — closes A2 item 2) | 15 explicit coordinates (other_agents `NONE..MIXED_POPULATION` plus observation/action/verifier, drift, recurrence, noise, compositionality, social topology, embodiment, budgets/prices, latency, acquisition, protected splits) plus bijective remint / anti-leakage booleans `true`; extends MIM `ECOLOGY_CONTRACT+AXES_V2` without editing them | TOM-1 opponent forcing + teaching break-even own `other_agents`; per-coordinate parents cited in-contract (POMDP/Pearl/Angluin/Kearns etc.) — no new graphical-model law claimed | any coordinate thin (<20/10/20), any `other_agents.values` missing `NONE`/`ADAPTIVE_OPPONENTS`, any equivalence boolean `false`, or any non-bijective "remint" accepted on the 3-world toy (re-run `test_ecology_extension_v1.py`) |
...
| 27-row capability contract (`gmi-capability-contract-v1/CONTRACT_V1.json`, this lane, sibling) | 27 architecture-independent rows with `inputs→allowed_info→behaviour→success→resource→twin→parent→atlas→falsifier` at declared GMI interface; admissibility only | `PHENOMENOLOGY_REDUCTION_ATLAS` (GG53-59) plus per-capability mature parent with first refusal (cited per row) | a row whose field count or falsifier mismatches `CONTRACT_V1.json` on re-run (re-run `test_capability_contract_v1.py`) |
| Planning/stopping (I4, `gmi-planning-stopping-v1`, this lane, sibling — composes META-3 `EVC`) | `n=6` finite worlds/actions/tests with exact costs `1/2`, admissibility only; drift/replanning outside registered `tau` excluded | `GMI_GOAL_FORMATION`/`SUBGOAL`/`PLANNING_STOPPING` (MIM) + `META-3` `13/35` myopic-optimality parent owns `EVC`; no new graphical-model law claimed | any `S` with `C(S)` nonempty and `EVC>0` for its common action, or miscount of the `13/35` greedy condition (re-run `test_planning_stopping_v1.py`) |
| Derived-component lesions (L5, `gmi-derived-lesion-v1`, this lane, sibling — doubles as K/V6 probe) | finite task families with 0/1 scoring; each lesion removes one derived mechanism not the search basis (`delta`/`F` intact); no neuropsychological claim | prior basis-ablation lesion literature (control `L_basis`) with first refusal — derived lesions are mechanism-specific, not basis deletions | any lesion row whose 0/1 entry mismatches the `8×6` table on re-run, or claimed dissociation pair failing on the registered families (re-run `test_derived_lesion_v1.py`) |
| Reachability refinement (E, `gmi-reachability-refinement-v1`, this lane, sibling — 602 E) | 4-node graph with 5 laws sharing `R`, burden as BFS distance, greedy monotone and encoding symdiff ≤ 1 bound, admissibility only (DU-1) | `DEVELOPMENTAL_UNDERDETERMINATION` (DU-1) + `DEVELOPMENTAL_REACHABILITY_SELECTION` parents own the split/selection; no new search law beyond exhaustive enumeration | any enumerated `Reach` or `b` mismatch, any claimed sufficient/greedy success where premise fails, or any encoding permutation violating `≤1` (re-run `test_reachability_refinement_v1.py`) |

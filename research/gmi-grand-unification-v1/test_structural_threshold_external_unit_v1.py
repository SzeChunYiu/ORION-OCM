"""Complete-unit controls use the actual frozen STR packet, never toy-only maps."""
...
MANIFEST_SHA = "e55db0c72cb0d57c918727a4f252f39ade18e8716c4db8e31861f25371a1e8ad"


class StructuralThresholdExternalUnitTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_actual_frozen_complete_packet_has_no_alarm(self):
        self.assertEqual(self.verify(), {"manifest_sha256": MANIFEST_SHA,
                                       "payload_files": 41, "payload_bytes": 137309})

    def test_changed_transitive_raw_parent_is_rejected(self):
        p = self.unit / "raw/STRUCTURAL_NEURAL_BOUND_THEOREM_V1.md"
    ...
    def test_boolean_size_and_wrong_payload_totals_are_rejected(self):
        for change in (lambda d: d["files"]["CORE.md"].update(bytes=True),
                       lambda d: d.update(payload_files=42),

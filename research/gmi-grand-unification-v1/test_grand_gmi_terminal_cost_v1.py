"""Complete TCR packet/wrapper custody and later-checker mutation controls."""
...
WRAPPER = "grand_gmi_terminal_cost_checks_v1.py"
UNIT = "research/gmi-terminal-cost-reconstruction-v1"
SHA = "a4089a30750d72695d004fcc58b004be40dc11f6de0e4ee155fb94419be0c8b3"


class TerminalCostCapsuleTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_actual_wrapper_preserves_the_complete_original_payload(self):
        ...
        expected = (HERE / "GRAND_GMI_TERMINAL_COST_RECONSTRUCTION_RECEIPT_V1.json").read_bytes()
        self.assertEqual(result.stdout, expected)
        original = json.loads((HERE.parent.parent / UNIT / "TERMINAL_COST_RECEIPT_V1.json").read_text())
    ...
    def test_original_pass_status_cannot_hide_an_altered_cost_profile(self):
        wrapper = runpy.run_path(str(HERE / WRAPPER))
        original = json.loads((HERE.parent.parent / UNIT / "TERMINAL_COST_RECEIPT_V1.json").read_text())
        self.assertEqual(original["status"], "PASS")
        original["controls"]["boolean_parent_s"]["frontier"] = [["0", "0"]]
    ...
    def test_original_field_projection_is_rejected(self):
        wrapper = runpy.run_path(str(HERE / WRAPPER))
        original = json.loads((HERE.parent.parent / UNIT / "TERMINAL_COST_RECEIPT_V1.json").read_text())
        del original["scalar_census"]
        fake = SimpleNamespace(returncode=0, stderr="", stdout=json.dumps(original))
        with patch("subprocess.run", return_value=fake):
            with self.assertRaisesRegex(ValueError, "complete original.*differs"):
                wrapper["run"]()

    def test_complete_census_allowance_does_not_change_ordinary_leaf_allowance(self):
        source = self.root / "grand_gmi_ordinary_checks_v1.py"
        source.write_text("print(" + repr(json.dumps(self.payload)) + ")")
        receipt = self.root / "ORDINARY_RECEIPT_V1.json"
        receipt.write_text(json.dumps(self.payload))
        ...
        self.save()
        observed = {}
        def execute(root, row, timeout=60):
            observed[row["checker"]] = timeout
            return self.payload
        with patch.dict(GATE["replay"].__globals__, {"execute_checker": execute}):
            self.assertTrue(GATE["replay"](self.root)["all_green"])
        self.assertEqual(observed, {WRAPPER: 180, source.name: 60})
    ...
    def test_changed_terminal_model_is_rejected(self):
        target = self.repository / UNIT / "terminal_kernel_v1.py"
        target.write_bytes(target.read_bytes() + b"unreviewed terminal cost")
        with self.assertRaisesRegex(ValueError, "content changed"):
            GATE["load_inventory"](self.root)

    def test_missing_transitive_query_parent_is_rejected(self):
        (self.repository / UNIT / "raw/RELATIONAL_QUERY_RECONSTRUCTION_THEOREM_V1.md").unlink()
    ...
    def test_later_checker_cannot_mutate_an_already_checked_external_parent(self):
        ...
        target = self.repository / UNIT / "syntax_parent_v1.py"
    ...
    def test_workflow_covers_unit_and_isolated_original_tests(self):
        ...
        self.assertEqual(text.count("'research/gmi-terminal-cost-reconstruction-v1/**'"), 2)
        self.assertEqual(text.count("research/gmi-terminal-cost-reconstruction-v1/test_terminal_cost_v1.py"), 2)
        self.assertIn("python -I -B research/gmi-terminal-cost-reconstruction-v1/test_terminal_cost_v1.py", text)
        self.assertIn("python -I -O -B research/gmi-terminal-cost-reconstruction-v1/test_terminal_cost_v1.py", text)

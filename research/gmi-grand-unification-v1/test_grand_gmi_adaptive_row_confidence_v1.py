"""Complete ARC confidence packet/wrapper custody and later-checker mutation controls."""
...
WRAPPER = "grand_gmi_adaptive_row_confidence_checks_v1.py"
UNIT = "research/gmi-adaptive-row-confidence-v1"
SHA = "e1ea64e390d2690f36058895d1da03f2597696b7da055bad4e65ed8fc63f25d3"


class AdaptiveRowConfidenceCapsuleTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_actual_wrapper_preserves_the_complete_original_payload(self):
        ...
        expected = (HERE / "GRAND_GMI_ADAPTIVE_ROW_CONFIDENCE_RECEIPT_V1.json").read_bytes()
        self.assertEqual(result.stdout, expected)
        original = json.loads((HERE.parent.parent / UNIT / "ADAPTIVE_ROW_CONFIDENCE_RECEIPT_V1.json").read_text())
    ...
    def test_pass_status_cannot_hide_an_altered_optional_monitoring_probability(self):
        wrapper = runpy.run_path(str(HERE / WRAPPER))
        original = json.loads((HERE.parent.parent / UNIT / "ADAPTIVE_ROW_CONFIDENCE_RECEIPT_V1.json").read_text())
        self.assertEqual(original["status"], "PASS")
        original["optional_peek"]["optional_failure"] = "1/5"
    ...
    def test_original_field_projection_is_rejected(self):
        wrapper = runpy.run_path(str(HERE / WRAPPER))
        original = json.loads((HERE.parent.parent / UNIT / "ADAPTIVE_ROW_CONFIDENCE_RECEIPT_V1.json").read_text())
        del original["adaptive_allocation_controls"]
        fake = SimpleNamespace(returncode=0, stderr="", stdout=json.dumps(original))
        with patch("subprocess.run", return_value=fake):
            with self.assertRaisesRegex(ValueError, "complete original.*differs"):
                wrapper["run"]()
    ...
    def test_changed_confidence_calculation_is_rejected(self):
        target = self.repository / UNIT / "row_confidence_v1.py"
        target.write_bytes(target.read_bytes() + b"unreviewed radius")
        with self.assertRaisesRegex(ValueError, "content changed"):
            GATE["load_inventory"](self.root)

    def test_missing_transitive_confidence_parent_is_rejected(self):
        (self.repository / UNIT / "raw/OCM_FOUNDATIONS_CLOSURE_V1.md").unlink()
    ...
    def test_later_checker_cannot_mutate_an_already_checked_external_parent(self):
        ...
        target = self.repository / UNIT / "visit_oracle_v1.py"
    ...
    def test_workflow_covers_frozen_tests_in_isolated_processes(self):
        ...
        self.assertEqual(text.count("'research/gmi-adaptive-row-confidence-v1/**'"), 2)
        for flags in ("-I -B", "-I -O -B"):
            self.assertIn("python " + flags + " " + UNIT + "/test_adaptive_rows_v1.py -v", text)

"""FQC wrapper integration, including mutation after the first entry succeeds."""
...
WRAPPER = "grand_gmi_finite_quantum_cover_checks_v1.py"
UNIT = "research/gmi-finite-quantum-cover-v1"
SHA = "6f9d3ee193e7da7918bee85bde75e1579b60fa0faeb0a0ab7a8b40364c604a1c"


class FiniteQuantumWrapperTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_actual_wrapper_retains_every_original_field_and_exact_receipt(self):
        result = subprocess.run([sys.executable, "-I", "-B", str(HERE / WRAPPER)],
                                capture_output=True, check=True)
        self.assertEqual(result.stderr, b"")
        frozen = (HERE / "GRAND_GMI_FINITE_QUANTUM_COVER_RECEIPT_V1.json").read_bytes()
        self.assertEqual(result.stdout, frozen)
        original = json.loads((HERE.parent.parent / UNIT / "FINITE_QUANTUM_COVER_RECEIPT_V1.json").read_text())
        self.assertEqual(GATE["canonical"](json.loads(result.stdout)["original_payload"]),
                         GATE["canonical"](original))
        self.assertFalse(original["general_quantifier_eliminator_executed"])
        self.assertFalse(original["hardware_or_ecology_measurements"])

    def test_original_field_projection_is_rejected_by_wrapper(self):
        wrapper = runpy.run_path(str(HERE / WRAPPER))
        original = json.loads((HERE.parent.parent / UNIT / "FINITE_QUANTUM_COVER_RECEIPT_V1.json").read_text())
        del original["mixed_support_controls"]
    ...
    def test_later_checker_cannot_mutate_already_verified_external_input(self):
        ...
        target = self.repository / UNIT / "raw/QUANTUM_PROCESS_INSTANTIATION_THEOREM_V1.md"
    ...
    def test_workflow_triggers_cover_frozen_unit_and_its_tests(self):
        ...
        self.assertEqual(text.count("'research/gmi-finite-quantum-cover-v1/**'"), 2)
        self.assertIn("python -I -B research/gmi-finite-quantum-cover-v1/test_finite_quantum_v1.py", text)
        self.assertIn("python -I -O -B research/gmi-finite-quantum-cover-v1/test_finite_quantum_v1.py", text)

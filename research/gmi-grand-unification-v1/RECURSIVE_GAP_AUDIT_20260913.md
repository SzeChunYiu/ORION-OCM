...
| 49 | Ledger item 8, upgraded from recomputation to confirmation. Three predictions were registered and deliberately left unevaluated, and that state merged to main: no price table for the deferred ecologies existed in any module, so no checker could compute an answer and no edit could backfill one without changing the frozen text. The executor parses the merged document for both the price tables and the prediction rows rather than hardcoding them. All three held, terminal ALL_DEFERRED_PREDICTIONS_HELD, against frozen digest 38493781de864856409a607ff75afea80a3f51ad89d5c8b947a66d77a6f84787, with each prediction carrying three registered ways to fail and none occurring. A parser defect found during execution is recorded rather than hidden: an unscoped pattern read a prediction row's law cell as a price clause, the same over-matching error as a substring classifier. Holding confirms that the registered charge table's consequences were correctly derived; it does not show the table describes any physical cost, and it does not claim the hosted parity workflow's status. | `../gmi-learning-law-selection-v1/DEFERRED_EXECUTION_V1.md` | Current iteration; commit and merge only after required local gates pass |
| 50 | Ledger item 10, run rather than argued, with the standing verdict untouched. In an exact enumeration microworld charging one unit per candidate examined, K2 holds on 148 of 148 held-out reuse targets for one library, so K2 is supported at a registered scope. Semantic reuse, meaning the library shortens the minimal description, is necessary: across four libraries and two disjoint target populations there are 1,707 no-reuse library-target pairs and zero K2 successes. It is not sufficient, since rates run from 45 to 100 percent, so an iff would be false. Where reuse is absent retention is actively harmful rather than neutral, strictly worse on 195/195, 300/300, 161/161 and 448/448 held-out cases, because macros inflate the search alphabet with nothing to shorten; ties are kept as a third category and the exploratory population contains three. An earlier syntactic classifier was corrected to a semantic one after it mislabelled programs that are semantically equal but share no substring. Library learning is parent-owned and no novelty is claimed for it. K2 at the #323 grammar remains NOT_ESTABLISHED: this is a microworld result and a falsifiable prediction about that negative, not a measurement of it. | `../gmi-k2-acquisition-experiment-v1/K2_ACQUISITION_EXPERIMENT_V1.md` | Current iteration; commit and merge only after required local gates pass |
| 51 | Five defects in my own items 7 to 10 units, raised by the ledger repair unit and each verified independently before being accepted. The worst was a rigged control: a test asserted that an atomic target can never satisfy K2 while drawing libraries from a universe that excluded the target, so disjointness was imposed by construction and the test could not fail. The claim is false, since with P equal to L equal to {a} the history arm charges 0 against RESET's 1. The governing condition is library intersects target in both failure modes; decomposition size alone decides nothing. The comparison helper certified empty and negative intervals because it omitted the validation its sibling performs, and now mirrors it. A digest test asserted only that a hash had 64 alphanumeric characters, which passes on any altered table, and now binds the frozen value. The interval iff is qualified to full Cartesian product semantics, since under a supplied correlated joint set the marginal boxes can touch while every world is strictly ordered, and the claim that the rule was reused verbatim from DCR is withdrawn. Two charges were not accepted after checking: the malformed-interval charge does not apply to the sibling function, which rejects every malformed form, and the two-thirds gate is unsourced, since a control-verified search finds no numeric bar registered anywhere and every line states only that the bar held on 2 of 9 fresh seeds. The retry registration therefore removes its own unsourced majority wording and requires the bar and its source to be stated before execution, rather than importing a threshold from a pull request description. | `../gmi-developmental-capital-v1/K2_REACHABILITY_CONDITION_V1.md`, `../gmi-transport-ledger-v1/TRANSPORT_LEDGER_V1.md` TL-8 | Current iteration; commit and merge only after required local gates pass |
| 52 | Three scope and arithmetic defects in my own item 10 unit, raised by the capital-acquisition repair unit and each recomputed before acceptance. The reported held-out success range of 45 to 100 percent named a figure that appears nowhere in the table: the three rates are 129/253 at 50.99 percent, 148/148 at 100 percent and 113/287 at 39.3728 percent, so the minimum was stated without being computed. The necessity claim is true at its registered scope but was written as a general law. KAE-10 records the counterexample: with the library renamed so a macro sorts before the primitives, the target 2x+1 has no shortening available since both minimal lengths are 2, yet the history arm first hits at candidate 8 against RESET's 11 and K2 holds; renaming the same macro late moves that hit to candidate 15 and K2 fails, with RESET unchanged. First-hit rank is therefore a property of the library together with its naming under the authored enumeration order, not of the library alone, and an independent sweep of 654 targets across the four registered libraries returns zero such cases, which is why the original search could not have found it. The read-first note separately miscounted 1,707 library-target pairs as distinct targets. | `../gmi-k2-acquisition-experiment-v1/K2_ACQUISITION_EXPERIMENT_V1.md` KAE-10 | Current iteration; commit and merge only after required local gates pass |
| 53 | The one gap the parity-n registration named as unfinished, closed with two results that point in opposite directions. PN-5 ended by saying a neural win "requires a task whose structure favours a threshold unit, which is separate work"; that task is majority-n, measured through DCR's own typed machine, which reproduces the registered parity-3 facts of 11, 39 with four native obligations, and 6 with one in the same run. **Negative half, quantified over every task rather than the one measured:** the cheapest constant table costs exactly 3n+4 whatever it computes, nothing in the typed register costs below 3n+2, nothing needing a constant costs below 3n+4, and there is no constant-free budget below 3n+4 reaches majority at n=3 or n=4, so the threshold form can tie the table and can never be certified below it. The one numerically cheaper realization, sum(x) >= k at 6 opcodes, has an unpriced native obligation and under PNIR-1 its interval is [6, infinity), so no ordering is certifiable; pricing it at zero would certify it below everything, and that outcome is recorded as the artifact PN-4 hit rather than used. **Positive half:** in the two-component coordinate (opcodes, constant cells) the threshold rendering at (3n+4, 1) strictly dominates the table at (3n+4, 2**n) for every n in 3..8, which is the first structurally neural realization to strictly beat a registered non-neural one anywhere in this evidence line, and it follows from charging a cell at any positive rate rather than from a chosen price. Three things are corrected or refuted rather than asserted. PN-3's lookup figure of 5n+2 is wrong: the flat-index rendering measures 5n+4 and the nested rendering 3n+4; PN-3's conclusion that table size is unpriced survives and is strengthened, and PN-1, PN-2 and PN-4R are untouched. The claim that minimal cost forces the threshold structure is **refuted** by exhaustive enumeration, not assumed: majority-3's 3n+4 optimum has 8 comparison-form realizations of which only 2 are affine, the other 6 being ((v0 + v1) << v2) > 1 and its variants, and majority-4 has 10 of which 2 are affine. And the win is not unique: the constant-free monotone gate chain is also undominated, at 570 opcodes against 28 at n=8, so the content is that the threshold family is on the frontier and the 2**n table is off it. The enumeration is over reachable value vectors rather than source texts, saturated against its magnitude cap at 32/64/256 and its constant range at [-6,6]/[-12,12]. One defect in this iteration's own first commit was found in review by Cursor Bugbot and is recorded in the theorem's section 10 rather than quietly fixed: TT-2 asserted that no written realization costs 3n+3, which is false because each admitted unary operation costs one opcode and -(a ^ b ^ c) costs exactly 3n+3, and the enumeration behind TT-3 searched binary operations only, so its exhaustiveness claim did not cover 3n+3 at all. The search now spends a unary budget anywhere in the tree and reports the 3n+2, 3n+3 and two-unary 3n+4 layers separately; majority remains unreachable in all of them at n=3 and n=4 and at caps 32 and 64, so the minimum is still 3n+4 and no conclusion moved. The report's own example, +(a ^ b ^ c), is in fact refused by the register because CPython 3.12 compiles unary + to CALL_INTRINSIC_1; the substance held for -, ~ and not. Both verdicts remain point verdicts over registered candidates under CU-1 and CU-3b, the Boolean-versus-int return type changes which side of the tie the table falls on and both readings are reported, constant cells are a count of integer cells and not bytes or resident memory, and no timing, physical memory or learning claim is made anywhere. | `../gmi-threshold-task-frontier-v1/THRESHOLD_TASK_FRONTIER_THEOREM_V1.md` TT-1 to TT-7 | Current iteration; commit and merge only after required local gates pass |
| 54 | Two defects in my own item 10 unit and one gap in how all four of my sectors are gated, each measured before it was written down. **The held-out figures were read back, not reproduced.** The counts behind KAE-4 and KAE-5 came from the frozen receipt and nothing recomputed them, so the falsifier the document already listed, that the held-out counts disagree with the frozen receipt, was never evaluated and a drift in the model would have left every number standing. `heldout_census` now regenerates the census on each run. Its reach is established by perturbing the model rather than by asserting coverage: a semantic drift, `double` multiplying by 3 instead of 2, reds 9 of 24 tests, but an order-preserving cost drift, charging 2 per step instead of 1, **passed 24 of 24**, because every recorded figure is a count of comparisons and `k2_trial` asks only whether `cost_H < cost_RESET`, so scaling both sides moved nothing. That blind spot was real, not hypothetical. Folding the raw cost totals into the same pass closes it: the same drift now reds 2 of 27 reporting `792160 != 396080`, and `reset_cost_total` is 396080 for all four libraries precisely because RESET never consults the library. **A test class was silently dropped on direct invocation.** `KAE10_AccelerationWithoutShortening` and its three methods were defined after the `if __name__ == "__main__"` block, so `python test_k2_acquisition_v1.py` ran 24 tests and printed OK while discovery ran 27; the three that vanished are exactly the KAE-10 result recorded in row 52, and the tripwire count never caught it because that count was taken from discovery. The block moved to the end of the file and both paths now run 27. **The four sectors behind items 7 to 10 had no workflow.** Repo-wide, 104 sectors carrying `test_*.py` are covered by a workflow and 38 are not, so this is an exception rather than the repo's norm. Rows 51 to 53 claimed only that required local gates pass, and they did pass, so no earlier positive is withdrawn: what was missing is that nothing re-ran these sectors after merge, leaving them unprotected against later drift by another lane. A workflow now runs all four in both modes at the repo's pinned 3.11.14, asserts the per-sector counts 43, 26, 29 and 27, and fails if direct invocation collects fewer tests than discovery. Two claims of my own are withdrawn rather than quietly dropped. I proposed trimming the `dec_inc` sweep to buy runtime headroom; measurement refuted both halves, since `dec_inc` is 15 percent of the regeneration while the dominant sweep at 49 percent is also the most discriminating, and the 60-second limit I was optimising against binds capsule checkers through `execute_checker`, not this suite, which appears nowhere in the inventory. No trim was made. The suites pass on 3.9.5 and 3.12.14 in both modes; 3.11.14 is not installable on the measurement host, so the CI interpreter is bracketed between two green points rather than directly measured. This is a reproduction guard over existing claims: it establishes no new K2 result, and the rates in KAE-4 and KAE-5 are unchanged by it. | `../gmi-k2-acquisition-experiment-v1/K2_ACQUISITION_EXPERIMENT_V1.md` KAE-11 | Current iteration; commit and merge only after required local gates pass |
| 55 | TT-6's pricing premise, discharged by measurement rather than left standing, with one earlier ranking withdrawn and one conclusion strengthened. TT-6 said that charging a constant cell at any positive rate makes the threshold rendering strictly dominate the 2**n table, and its second component was a declared count of integer cells that the theorem itself flagged as not bytes and not memory. Two exact sizes now replace it, on an object-size contract the checker validates and refuses to guess about (empty tuple 40 bytes, slot 8, small integer 28, 63-bit maxsize; any other build raises rather than restating different numbers): the serialized length marshal.dumps writes, and the in-memory size of the payload's tuple objects. The integers are deliberately not charged, and that is measured rather than assumed: every value in these payloads is 0 or 1, and the checker verifies by identity that each leaf is the same object as an independently built integer, so charging them would double-count objects that already exist and would have flattered the very conclusion under test. Results. The domination holds in both measures at every n in 3..8, against the flat table as well as the nested one, so the positive half no longer rests on an unmeasured premise. Under the serialized measure the undominated set is exactly the counted one, so TT-6 was not an artifact of counting. Under the in-memory measure it is strictly smaller and from n=5 the threshold rendering is the unique undominated realization, because its single constant is an interned integer costing no incremental memory while the constant-free gate chain's only advantage was worth one cell in the count and nothing when measured; at n=3 and n=4 the optimum is instead a tie at zero structure with the other minimal renderings, which is TT-4's refutation reappearing in a second coordinate. Withdrawn: the count reports the nested table strictly better than the flat one at every n, and both measurements report them incomparable, because nested pays a 40-byte tuple header 2**n-1 times rather than once, 14280 bytes against 2088 at n=8 under a count of 256 against 263 that ranked them the other way. A count of cells is therefore not a conservative proxy for size: here it inverted a comparison, and TT-6 survives on the size of its margin rather than on the count being safe. Neither measure is resident set size, cache behaviour or allocator overhead; the interning result is a property of these 0/1 payloads and not of tables in general; the contract is one CPython 3.12 64-bit build; and measuring the second component does nothing about the CU-1 coverage obligation. | `../gmi-threshold-task-frontier-v1/MEASURED_CONSTANT_FOOTPRINT_V1.md` TB-1 to TB-6 | Current iteration; commit and merge only after required local gates pass |

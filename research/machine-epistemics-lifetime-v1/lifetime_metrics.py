...
This module lives in the prospective research lane rather than ``src/ocm`` so
that defining the next experiment does not rewrite already-sealed M0--M12
engineering source inventories. A future milestone may absorb the contract into
the canonical runtime with a new receipt explicitly authorizing that change.

"""Preserve every original atom while appending reviewed reconstruction evidence; enforce complete adjudication records."""
import hashlib
...
ROOT = HERE.parents[1]
BASE = ROOT / "research/gmi-1068-recursive-audit-v18/SCOPE_SNAPSHOT_V18.json"
BASE_SHA = "866fa51b65bb0ecec02d83f398a6479fafb6ba3331962df74e96560984e27af1"
...
CONTRACT = load("audit_contract_v19", HERE / "audit_contract_v19.py")
...
def evaluate(current=None, receipt=None, accounting=None):
    ...
    need(hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA, "V18 baseline drift")
    before = json.loads(BASE.read_text())
    if current is None:
        current = json.loads((HERE / "SCOPE_SNAPSHOT_V19.json").read_text())
    if receipt is None:
        receipt = json.loads((ROOT / CONTRACT.PACKAGE / "RESULT_V19.json").read_text())
    ...
    need(result["registered_atoms"] == 222 and result["unresolved_atoms"] == 202,
    ...
    need(len(scientific) == 12, "scientific closure count drift")
    expected_accounting = CONTRACT.current_accounting(current, receipt["amendment_carry"])
    if accounting is None:
        accounting = json.loads((HERE / "CURRENT_ACCOUNTING_V19.json").read_text())
    exact(accounting, expected_accounting, "current accounting drift")
    for key, value in {"original_total": 222, "original_fulfilled": 20, "original_unresolved": 202,
                       "qualified_replacements": 2, "active_unresolved": 200}.items():

...
spec=importlib.util.spec_from_file_location('checked_snapshot_v19',HERE/'check_snapshot_v19.py')
...
class SnapshotTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.snapshot=json.loads((HERE/'SCOPE_SNAPSHOT_V19.json').read_text())
        cls.receipt=json.loads((checker.ROOT/checker.CONTRACT.PACKAGE/'RESULT_V19.json').read_text())
    ...
    def test_rehashed_snapshot_mutations(self):
        ...
        wrong=checker.CONTRACT.witness(checker.ROOT,'ReconstructedV19.lean','lean_declaration','category_identity')
        ...
        bad=deepcopy(self.snapshot);bad['rounds']['R1']['artifacts']=[a for a in bad['rounds']['R1']['artifacts']
            if a['path']!=checker.CONTRACT.PACKAGE+'/FREEZE_V19.md'];invalid.append(bad)
        bad=deepcopy(self.snapshot);path=HERE/'audit_contract_v19.py'
        ...
        self.assertEqual(len(invalid),22)
    ...
    def test_receipt_mutations(self):
        ...
        changes=[(('atoms',),{'GMI2-R1-001':{'status':'VERIFIED_AT_REGISTERED_SCOPE'}}),
            (('registered_results','S1','scope'),'local units alone suffice'),
            (('registered_results','S2','status'),'PENDING_REVIEW'),
            ...
            (('coverage','test_tables_v19','accepted_tables'),True),
            (('coverage','test_tables_v19','accepted_tables'),59.0),
            ...
            (('tests_run',),True),(('tests_run',),11.0),(('tests_run',),10),
        ...
        for path in ((),('kernel',),('source_bindings',),('coverage','test_tables_v19'),('inputs',),('registered_results',)):
        ...
        self.assertEqual(len(invalid),60)
    ...
    def test_coupled_result_scope_and_original_promotion(self):
        ...
        for result in checker.CONTRACT.RESULTS:
            ...
            claim='Arbitrary multiplicative maps preserve identities and all structures are minimal.'
    ...
    def test_current_accounting_mutations(self):
        ...
        for path,value in [(('original_fulfilled',),21),(('original_unresolved',),201),
                (('active_unresolved',),199),(('qualified_replacements',),3),(('original_total',),222.0),
        ...
        invalid.append(json.loads((checker.BASE.parent/'CURRENT_ACCOUNTING_V18.json').read_text()))

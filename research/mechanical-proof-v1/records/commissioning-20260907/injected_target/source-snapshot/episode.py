...
def validate_worker(record):
    ...
    for entry in imports:
        if type(entry) is not dict or set(entry) != {'name', 'origin'}:
            raise ValueError('invalid module inventory record')
    ...
    reported = audit.get('constant_occurrences')
    ids = record.get('used_constants')
    if (type(ids) is not list or any(type(k) is not int for k in ids) or type(reported) is not dict or
            set(reported) != {'proof_term', 'type_annotations'} or
            any(type(group) is not dict or any(type(n) is not int or n < 1 for n in group.values())
                for group in reported.values())):
        raise ValueError('invalid dependency metadata types')
    if reported != occurrences or ids != used:

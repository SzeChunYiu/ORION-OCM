"""Close only the six registered R3 resource/image atoms; preserve every other original record; enforce complete adjudication records."""
import hashlib
...
ROOT = HERE.parents[1]
BASE = ROOT / "research/gmi-1068-recursive-audit-v20/SCOPE_SNAPSHOT_V20.json"
BASE_SHA = "fa4a12ecbac168d5cb8bbfc2a6eee95fa60e72eea4408fcd8b9267d486d130ba"
...
CONTRACT = load("audit_contract_v21", HERE / "audit_contract_v21.py")
...
def evaluate(current=None, receipt=None, accounting=None):
    ...
    need(hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA, "V20 baseline drift")
    before = json.loads(BASE.read_text())
    if current is None:
        current = json.loads((HERE / "SCOPE_SNAPSHOT_V21.json").read_text())
    if receipt is None:
        receipt = json.loads((ROOT / CONTRACT.PACKAGE / "RESULT_V21.json").read_text())
    ...
    need(result["registered_atoms"] == 222 and result["unresolved_atoms"] == 195,
    ...
    need(len(scientific) == 19, "scientific closure count drift")
    expected_accounting = CONTRACT.current_accounting(current, receipt["amendment_carry"])
    if accounting is None:
        accounting = json.loads((HERE / "CURRENT_ACCOUNTING_V21.json").read_text())
    exact(accounting, expected_accounting, "current accounting drift")
    for key, value in {"original_total": 222, "original_fulfilled": 27, "original_unresolved": 195,
                       "qualified_replacements": 2, "active_unresolved": 193}.items():

...
from registration_evidence_bindings import checked_source, digest_record, verify_omissions, verify_source_bindings
...
def support_checks(files, package, derived):
    ...
    for role, name in [("recorder", "registration_envelope.py"), ("entry", "register.py")]:
        checked_source(Path(package)/name, digest_record(launch[role]), Path(package).parent)
    ...
    for name, record in q["files"].items():
        ...
        checked_source(Path(package)/name, wanted, Path(package).parent)

...
def audit_package(root, expected_seal, research_root):
    ...
    for row in index["archives"]:
        ...
        if type(row["count"]) is not int or type(row["raw_bytes"]) is not int:
            raise ValueError("ARCHIVE_COUNT_TYPE")
        if "archive_binding" in row: checked(values[name], row["archive_binding"], "INDEX_ARCHIVE_HASH")
        if "member_map_binding" in row:
            checked(values[inventory], row["member_map_binding"], "INDEX_MEMBER_MAP_HASH")
    ...
    for relative, binding in bindings.items():
        safe_name(relative)
        source = Path(research_root) / relative
        try: source.resolve().relative_to(Path(research_root).resolve())
        except ValueError as exc: raise ValueError("SOURCE_PATH") from exc
        checked(read_file(source, MAX_MEMBER_BYTES), binding, "CURRENT_SOURCE_HASH")

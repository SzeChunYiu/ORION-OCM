...
spec=importlib.util.spec_from_file_location('checked_snapshot_v26',HERE/'check_snapshot_v26.py')
...
class SnapshotTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.snapshot=json.loads((HERE/'SCOPE_SNAPSHOT_V26.json').read_text())
        cls.receipt=json.loads((checker.ROOT/checker.CONTRACT.PACKAGE/'RESULT_V26.json').read_text())
    ...
    def test_actual_snapshot_no_alarm(self):
        ...
        self.assertEqual(len(result['scientific_atoms_closed']),25)
        self.assertEqual((result['original_fulfilled'],result['remaining_atoms'],
                          result['qualified_replacements'],result['active_unresolved']),(33,189,2,187))
    ...
    def test_rehashed_snapshot_mutations(self):
        ...
        wrong=checker.CONTRACT.witness(checker.ROOT,'oracle_v26.py','python_symbol','raw')
        ...
        bad=deepcopy(self.snapshot);bad['rounds']['R1']['artifacts']=[a for a in bad['rounds']['R1']['artifacts']
            if a['path']!=checker.CONTRACT.PACKAGE+'/FREEZE_V26.md'];invalid.append(bad)
        bad=deepcopy(self.snapshot);path=HERE/'audit_contract_v26.py'
        ...
        self.assertEqual(len(invalid),85)
    ...
    def test_receipt_mutations(self):
        ...
        coverage_key=next(iter(self.receipt['coverage']['test_functors_v26']))
        ...
        changes=[(('atoms',),{'GMI2-R1-001':{'status':'VERIFIED_AT_REGISTERED_SCOPE'}}),
            (('registered_results','Z1','scope'),'all process ontologies have the same unique primitive fields'),
            (('registered_results','Z2','status'),'PENDING_REVIEW'),
            ...
            (('coverage','test_functors_v26',coverage_key),True),
            (('coverage','test_functors_v26',coverage_key),float(self.receipt['coverage']['test_functors_v26'][coverage_key])),
        ...
        for value in ({}, {'original_removed_components':[],'reconciled_rows':6},
                      {'original_removed_components':self.receipt['optional_reconciliation']['original_removed_components'],'reconciled_rows':True},
                      {'original_removed_components':self.receipt['optional_reconciliation']['original_removed_components'],'reconciled_rows':6.0}):
            changes.append((('optional_reconciliation',),value))
        ...
        for path in ((),('kernel',),('source_bindings',),('coverage','test_functors_v26'),('inputs',),('registered_results',)):
    ...
    def test_coupled_result_scope_and_original_promotion(self):
        ...
        for result in checker.CONTRACT.RESULTS:
            ...
            count+=1
        snapshot,receipt=deepcopy(self.snapshot),deepcopy(self.receipt)
        atom=snapshot['rounds']['R1']['obligations'][8];atom['status']='CLOSED'
    ...
    def test_current_accounting_mutations(self):
        ...
        for path,value in [(('original_fulfilled',),34),(('original_unresolved',),188),
                (('active_unresolved',),186),(('qualified_replacements',),3),(('original_total',),222.0),
        ...
        invalid.append(json.loads((checker.BASE.parent/'CURRENT_ACCOUNTING_V25.json').read_text()))

"""Fixed admission closure contract and inherited source custody."""
import hashlib
...
def load(path):
    spec = importlib.util.spec_from_file_location("v26_contract_" + path.stem, path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


SPECS = load(Path(__file__).with_name("audit_specs_v26.py"))
PINS = load(Path(__file__).with_name("audit_pins_v26.py"))
...
def atom_evidence(root, atom):
    ...
    if not PINS.FINAL:
        raise OSError("final reviewed V26 proof pins unavailable")
    return [witness(root, "RESULT_V26.json", "json_pointer", "/atoms/" + atom),
            witness(root, "oracle_v26.py", "python_symbol", "certify")] + [
...
def repair(root, name):
    if not PINS.FINAL:
        raise OSError("final reviewed V26 proof pins unavailable")
    evidence = [witness(root, "check_admission_v26.py", "python_symbol", "evaluate"),
                witness(root, "RESULT_V26.json", "json_pointer", "/kernel" if name == "R14" else "/coverage")]
    extras = {"R1": PINS.REPAIR_DECLARATION,
              "R14": ("proof_contract_v26.py", "python_symbol", "audit_source"),
              "R15": ("oracle_v26.py", "python_symbol", "certify")}
    evidence.insert(0, witness(root, *extras[name]))
    return {"id": name + "-admission-v26", "scope": REPAIR_SCOPES[name],
...
def validate_receipt(root, receipt):
    ...
    need(type(receipt) is dict and set(receipt) == {
        ...
        "claim_boundaries", "optional_reconciliation"}, "receipt fields")
    fixed = {"schema": "GMI_1068_ADMISSION_STRUCTURE_V26", "tests_run": 13,
             "overall_closure": "OPEN", "scientific_truth_certified": False, "claim_boundaries": BOUNDARIES,
             "optional_reconciliation": SPECS.OPTIONAL}
    ...
    inputs = {entry["path"].rsplit("/", 1)[-1]: entry["sha256"] for entry in artifacts
              if not entry["path"].endswith("/RESULT_V26.json")}
    ...
    exact(load(package / "audit_optional_v26.py").verify(root), SPECS.OPTIONAL, "original optional reconciliation drift")
    custody = load(package / "custody_v26.py")
    try:
    ...
    exact(receipt["coverage"], load(package / "coverage_v26.py").REQUIRED, "coverage receipt drift")
    need(digest(package / "proof_contract_v26.py") == PROOF_CONTRACT_SHA, "reviewed proof-type drift")
    proof = load(package / "proof_contract_v26.py")
...
def current_accounting(snapshot, amendment_carry):
    ...
    return {"schema": "GMI_1068_CURRENT_ACCOUNTING_V26", "snapshot_content_sha256": snapshot_sha,

"""One root-bound retained screen; no parser/learner/native work occurs in this observer."""
...
ROOT = HERE.parent
RUN = HERE
REQUEST = RUN / "EXECUTION-REQUEST-DRAFT.json"
GATE = RUN / "ROOT-SCREENING-GATE.json"
OUTPUT = RUN / "screening-01"
...
def run():
    ...
    assert gate["authorization"] == "ROOT_RETAINED_SCREENING_GATE"
    ...
    argv = [str(PYTHON), "-I", "-S", "-B", str(ROOT / "run_screening.py"), str(REQUEST), str(OUTPUT)]
    ...
    assert request["inputs"] == contract["inputs"] == gate["inputs"]
    for value in request["inputs"].values():
        pin(value["path"], value)
    assert request["runtime"] == contract["lark_runtime"]
    for name, value in request["runtime"]["files"].items():
        pin(str(Path(request["runtime"]["path"]) / name), value)
    ...
    write(OBSERVATION / "START.json", {"observer_pid": os.getpid(), "observer_parent_pid": os.getppid(),
        ...
        "runtime": gate["runtime"], "observer_argv": sys.argv, "observer_cwd": os.getcwd(), "child_cwd": str(ROOT)})
    ...
    try:
        outputs = inspect_outputs(OUTPUT, child.pid if child else None, EXPECTED_REQUEST, request, ROOT)
    except BaseException as error:
    ...
    receipt = {"schema": "ordinary.retained-screening.outer-process.v1", "observer_pid": os.getpid(),
        ...
        "terminal": "SCREENING_PROCESS_COMPLETED" if successful else "SCREENING_PROCESS_FAILED",
        "scope": "Once-only retained screening recording; no native admission, acquisition, serving utility, held-out result or novelty follows."}

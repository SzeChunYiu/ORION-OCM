"""Real-snapshot baseline and rehashed hostile one-atom adjudications."""
...
spec = importlib.util.spec_from_file_location("checked_snapshot_v14", HERE / "check_snapshot_v14.py")
...
class SnapshotTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.snapshot = json.loads((HERE / "SCOPE_SNAPSHOT_V14.json").read_text())
        cls.receipt = json.loads((checker.ROOT / checker.CONTRACT.PACKAGE / "RESULT_V14.json").read_text())
    ...
    def test_actual_snapshot_no_alarm(self):
        ...
        self.assertEqual(result["new_scientific_atoms_closed"], ["GMI2-R1-008"])
        self.assertEqual(result["scientific_atoms_closed"],
                         ["GMI2-R1-002", "GMI2-R1-003", "GMI2-R1-004", "GMI2-R1-005",
                          "GMI2-R1-008", "GMI2-R1-010", "GMI2-R2-002", "GMI2-R2-006"])
        self.assertEqual(result["remaining_atoms"], 206)
    ...
    def test_snapshot_mutations(self):
        ...
        wrong = checker.CONTRACT.witness(checker.ROOT, "MatricesV14.lean", "lean_declaration", "matrix")
        mutations = [
            ...
            (atom("R1", 7, "evidence"), [wrong]),
            (atom("R1", 7, "title"), "a different original requirement"),
            (atom("R1", 7, "status"), "OPEN"),
            (atom("R1", 7, "disposition"), "every process has a canonical probability law"),
            (atom("R1", 7, "historical_status"), "CLOSED"),
            (atom("R1", 7, "id"), "GMI2-R1-099"),
            ...
            (atom("R1", 7, "evidence"), []),
            (atom("R1", 7, "evidence"), [checker.CONTRACT.witness(
                checker.ROOT, "RESULT_V14.json", "json_pointer", "/coverage")]),
        ...
        missing["rounds"]["R1"]["artifacts"] = [a for a in missing["rounds"]["R1"]["artifacts"]
                                                  if a["path"] != checker.CONTRACT.PACKAGE + "/FREEZE_V14.md"]
        added = deepcopy(self.snapshot)
        path = HERE / "audit_contract_v14.py"
        ...
        source, target = swapped["rounds"]["R1"]["obligations"][7], swapped["rounds"]["R1"]["obligations"][5]
        source["status"] = "OPEN"
        target.update(status="CLOSED", disposition=source["disposition"], evidence=source["evidence"])
        extended = deepcopy(self.snapshot)
        extended["rounds"]["R1"]["obligations"][7]["unsupported_authority"] = "universal"
    ...
    def test_receipt_mutations(self):
        ...
        mutations = [
            ...
            (("atoms", atom, "scope"), "all nondeterminism has canonical probabilities"),
            ...
            (("kernel", "source_assumptions"), "arbitrary real kernels kernel checked"),
            (("inputs", input_name), "0" * 64),
            (("coverage", "test_stochastic_v14", "kernels"), True),
            (("coverage", "test_stochastic_v14", "kernels"), 35.0),
        ...
        for key in self.receipt:
            ...
            del bad[key]
            invalid.append(bad)
        for path in ((), ("kernel",), ("source_bindings",), ("coverage", "test_stochastic_v14"), ("inputs",)):
            bad = deepcopy(self.receipt)
            cursor = bad
            for key in path:
                cursor = cursor[key]
    ...
    def test_coupled_scope_drift(self):
        ...
        for index in (7,):
            ...
            claim = "All stochastic processes and real scalar specializations are now kernel proved."

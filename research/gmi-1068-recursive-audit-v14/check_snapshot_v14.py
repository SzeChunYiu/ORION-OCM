"""Close exactly one reviewed stochastic-process atom; enforce complete adjudication records."""
import hashlib
...
ROOT = HERE.parents[1]
BASE = ROOT / "research/gmi-1068-recursive-audit-v13/SCOPE_SNAPSHOT_V13.json"
BASE_SHA = "db349656850afb8d7cd6248d73ab0338792ee8a30d91bc6dc26f5decc534dea0"
...
CONTRACT = load("audit_contract_v14", HERE / "audit_contract_v14.py")
...
def evaluate(current=None, receipt=None):
    ...
    need(hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA, "V13 baseline drift")
    before = json.loads(BASE.read_text())
    if current is None:
        current = json.loads((HERE / "SCOPE_SNAPSHOT_V14.json").read_text())
    if receipt is None:
        receipt = json.loads((ROOT / CONTRACT.PACKAGE / "RESULT_V14.json").read_text())
    ...
    need(result["registered_atoms"] == 222 and result["unresolved_atoms"] == 206,
    ...
    need(len(scientific) == 8, "scientific closure count drift")

"""Typed atomic renaming and ordinary-proof construction from an admitted native DAG."""
...
def construct(trace, contracts, renaming, hypotheses, max_tokens=256):
    ...
    require(all(h["statement"][0] == "class" for h in floating), "class parameters only")
    params = [h["statement"][1] for h in floating]
    require(len(params) == 3 and len(set(params)) == 3, "three typed parameters")
    require(type(renaming) is dict and set(renaming) == set(params) and
            set(renaming.values()) == set(params), "atomic bijective renaming")
    ...
    for event in trace["events"]:
        ...
        if node["kind"] == "saved_reference":
            ...
            if row["kind"] in {"$f", "$e"}:
                ...
                require(count > 0 and len(stack) >= count, "stack arity")
                inputs = stack[-count:]
                mapping = {}

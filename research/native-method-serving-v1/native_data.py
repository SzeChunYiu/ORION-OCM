...
def sources(work=None):
    ...
    paths+=list(here.rglob("*.py"))+[here/"TRANSPLANT.json",here/"vendor/mmverify.LICENSE"]
    out={}
...
def payload(rt,eid,role,*,expected=None,derived=None,work=None):
    ...
    if data["schema"]!="ocm.native-method.evidence.v1" or data["role"]!=role:
        raise InputRefused("EVIDENCE_ROLE")
    channel={"proof":"proof","discovery":"demonstration"}.get(role,"instruction")
    if (rec.channel.value!=channel or rec.source!="native-method.v1:"+role or
...
def admit_evidence(rt,role,body,work,derived=None):
    channel={"proof":"proof","discovery":"demonstration"}.get(role,"instruction")
    data={"schema":"ocm.native-method.evidence.v1","role":role,"body":parse(raw(body))}
    _,eid=rt.admit_evidence(data,channel,"native-method.v1:"+role,scope=SCOPE,derived_from=derived)

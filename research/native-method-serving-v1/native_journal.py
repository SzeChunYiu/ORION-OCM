...
from native_contract import InputRefused,fields
from native_request import check_packet,read_request
...
def validate_intent(store,body):
    ...
    if body["schema"]!="ocm.native-method.use-intent.v1" or body["sources_sha256"]!=store.source_sha:
    ...
    read_request(store,body["qid"],body["request"],current=False)
    return body

def begin_use(store,qid,request):
    body={"schema":"ocm.native-method.use-intent.v1","sources_sha256":store.source_sha,
          "qid":qid,"request":request,"core_sequence":len(store.rt.events),
          "core_hash":store.rt.events[-1].event_hash}
    validate_intent(store,body)
    store.pending[qid]=D.parse(D.raw(body))  # Latch before any potentially durable write.
    store._append("USE_PREPARE",body)
    store.intents[qid]=D.parse(D.raw(body))
    return D.parse(D.raw(body))

def validate_receipt(store,body,intent,*,successful=True,replay=False):
    fields(body,("schema","sources_sha256","qid","request","receipt","core_events","intent_sha256"))
    validate_intent(store,intent)
    if (body["schema"]!="ocm.native-method.use.v1" or body["sources_sha256"]!=store.source_sha
    ...
    if successful:
        ...
        from native_receipt import check_identity,stable_check
        check_identity(body["request"]["engine_request"],receipt["packet"],receipt["check"])
        if replay:
            fresh=check_packet(store,body["qid"],body["request"],receipt["packet"],current=False)
            if stable_check(fresh)!=stable_check(receipt["check"]):raise InputRefused("USE_NATIVE_REPLAY_BINDING")
...
def record_use(store,qid,request,receipt,intent):
    ...
    body={"schema":"ocm.native-method.use.v1","sources_sha256":store.source_sha,"qid":qid,

...
| F5 LMHM locally morphogenetic mesh | Γ acts factor-locally; each factor may change realization family | E_factored with per-factor regimes (one factor revised every round, one never, two intermediate): heterogeneous mesh (per-factor occupant by the measured λ*, H*) beats every homogeneous row; kill `LOCAL_DEMAND_SIGNATURE_DOES_NOT_PREDICT_LOCAL_MORPHOGENESIS` | RV-043 |
...
| F2 VRQM | F1 + A1/A3/A4 on the residual layer | composite of RV-042 and RV-032/037; only after RV-042 | after RV-042 |

...
| F5 LMHM locally morphogenetic mesh | Γ acts factor-locally; each factor may change realization family | E_factored with per-factor regimes (one factor revised every round, one never, two intermediate): heterogeneous mesh (per-factor occupant by the measured λ*, H*) beats every homogeneous row; kill `LOCAL_DEMAND_SIGNATURE_DOES_NOT_PREDICT_LOCAL_MORPHOGENESIS` | RV-044 |
| F4 IQL interventional quotient learner | chooses interventions by target-ambiguity information gain | active ecology with priced queries: ambiguity-driven query order reaches θ in fewer priced events than random and than prediction-error order, by a closed-form count on the factored target | RV-045 |
| F6 SCDI self-compiling developmental intelligence | authoritative development state recompiled into serving forms as prices change | price vector switches mid-lifecycle (B0 ↔ HW): the recompiling row beats every fixed-form row once the number of price switches exceeds compiler cost / per-switch saving (a crossover of the RV-033 H* family) | RV-046 |

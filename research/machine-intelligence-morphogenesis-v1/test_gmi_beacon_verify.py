...
def test_subdirectory_repo_root_resolves_freeze_against_git_toplevel(tmp_path):
    # The real callers pass the study directory (a subdirectory of the repository) as repo_root.
    # `git show <commit>:<path>` resolves a bare path against the toplevel, so a study-relative
    # path must not be handed to git verbatim; this is the defect that failed every V7 task once.
    git(tmp_path,"init")
    git(tmp_path,"config","user.email","test@example.invalid")
    git(tmp_path,"config","user.name","Beacon Test")
    study=tmp_path/"research"/"study";study.mkdir(parents=True)
    freeze=study/"freeze.json"
    freeze.write_text(json.dumps({"public_beacon_rule":{"chain_hash":QUICKNET_CHAIN_HASH,"genesis_time":1692803367,"period_seconds":3,"delay_seconds":9}},sort_keys=True))
    git(tmp_path,"add","research/study/freeze.json");git(tmp_path,"commit","-m","freeze")
    commit=git(tmp_path,"rev-parse","HEAD");ct=int(git(tmp_path,"show","-s","--format=%ct",commit));rnd=round_for(ct+9);rt=1692803367+(rnd-1)*3
    sig=("01"*96);rand=hashlib.sha256(bytes.fromhex(sig)).hexdigest()
    b={"schema":"TESTBeacon","status":"ACQUIRED_NO_REROLL","no_reroll":True,"execution_freeze_commit":commit,"delay_seconds":9,"chain_hash":QUICKNET_CHAIN_HASH,"target_round":rnd,"target_round_time_unix":rt,"signature":sig,"randomness":rand,"api_randomness":rand,"sha256_signature_consistency_verified":True}
    out=verify_beacon(repo_root=str(study),freeze_path=str(freeze),beacon=b,expected_schema="TESTBeacon")
    assert out["target_round"]==rnd
    freeze.write_text(freeze.read_text()+"\n")
    with pytest.raises(RuntimeError,match="exact current execution freeze"):
        verify_beacon(repo_root=str(study),freeze_path=str(freeze),beacon=b,expected_schema="TESTBeacon")

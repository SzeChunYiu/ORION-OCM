...
def _lifecycle(cand, grammar, scale, profile):
    ...
    # The grammar-native program itself is charged separately from the phenotype approximation.
...
def run_cell(family: str, grammar: str, cell: str, *, freeze: dict, seed: int, budget: int = 1_000_000):
    ...
    if task is None:
        return {"verdict": "INCONCLUSIVE_GRAMMAR", "reason": f"no obligation evaluator for {spec['obligation_class']}", "family": family, "grammar": grammar, "cell": cell}
    ...
    coverage = {"exhaustive": exhausted and twin_exhausted, "candidate_space_size": len(space),
                ...
                "budget": budget}
    if win is None:
        return {"verdict": "INCONCLUSIVE_GRAMMAR", "reason": "no admissible positive candidate", "controls": controls,
    ...
    return {
        ...
        "world_profile": profile, "controls": controls, "coverage": coverage,
        "grammar_inventory_size": gn.DISJOINT_INVENTORY_SIZES.get(grammar),
        "all_grammar_inventory_sizes": dict(gn.DISJOINT_INVENTORY_SIZES),
        "grammar_token_prefix": gn.PREFIX[grammar],
        "independence": {"implementation_disjointness": "MECHANICALLY_CHECKED", "IG-4": "PENDING_INDEPENDENT_BUCKETING", "IG-5": "PENDING_INDEPENDENT_PRIMITIVES"},

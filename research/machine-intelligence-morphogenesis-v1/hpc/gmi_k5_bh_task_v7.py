...
PHASE=os.path.join(ROOT,"GMI_K5_BH_PHASE_FREEZE_V1.json");SCORE=os.path.join(ROOT,"GMI_K5_BH_SCORING_FREEZE_V1.json");CORR2=os.path.join(ROOT,"GMI_K5_BH_DESIGN_CORRIGENDUM_V2.json");CORR3=os.path.join(ROOT,"GMI_K5_BH_CONTINUAL_CORRIGENDUM_V3.json");ADM=os.path.join(ROOT,"GMI_K5_BH_ADMISSIBILITY_SCORING_ADDENDUM_V2.json");EXEC=os.path.join(ROOT,"GMI_K5_BH_EXECUTION_FREEZE_V7.json");BEACON=os.path.join(ROOT,"GMI_K5_BH_PUBLIC_BEACON_V7.json");RES=os.path.join(ROOT,"microscopes","results","k5_bh_v7")
...
def contracts():
    ...
    if ex.get("status")!="FROZEN_BEFORE_K5_V7_BEACON_AND_PROTECTED_EXECUTION":raise RuntimeError("bad K5 V7 execution freeze")
...
def main():
    ...
    try:
        ...
        unit=p[a.task_index];rec.update(unit);key={**unit,"phase":hs["phase"],"score":hs["score"],"corr2":hs["corr2"],"corr3":hs["corr3"],"admissibility":hs["admissibility"],"execution":hs["execution"]};th=hashlib.sha256(json.dumps(key,sort_keys=True,separators=(",",":")).encode()).hexdigest();rec["task_hash"]=th;rec["seed"]=int(hashlib.sha256(("GMI-K5-BH-V7"+th+verified["randomness"]).encode()).hexdigest()[:16],16)%(2**32)
        ...
        ru=resource.getrusage(resource.RUSAGE_SELF);rec["wall_seconds"]=round(time.time()-t0,3);rec["cpu_seconds"]=round(ru.ru_utime+ru.ru_stime,3);rec["peak_rss_kb"]=ru.ru_maxrss;rec["exit_code"]=0 if rec.get("status")=="OK" else 1;path=os.path.join(RES,f"K5BHV7_{a.host}_{a.job_id}_{a.task_index:05d}.json");json.dump(rec,open(path,"w"),indent=1,sort_keys=True,default=str)

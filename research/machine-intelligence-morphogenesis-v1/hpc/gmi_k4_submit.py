#!/usr/bin/env python3
"""Render and optionally submit protected grammar-native K4 V3 from a measured development-only probe."""
from __future__ import annotations
import argparse, hashlib, json, math, os, re, subprocess, time

ROOT=os.path.dirname(os.path.dirname(os.path.abspath(__file__))); PROBE=os.path.join(ROOT,"LUNARC_ENV_PROBE_V1.json"); TEMPLATE=os.path.join(ROOT,"hpc","gmi_k4_array.sbatch")
FREEZE=os.path.join(ROOT,"GMI_K4_LOFO_FREEZE_V1.json"); GEN=os.path.join(ROOT,"GMI_K4_GENERATOR_FREEZE_V1.json"); SUCCESSOR=os.path.join(ROOT,"GMI_K4_GRAMMAR_NATIVE_SUCCESSOR_FREEZE_V3.json"); BEACON=os.path.join(ROOT,"GMI_K4_PUBLIC_BEACON_V3.json")
RENDERED=os.path.join(ROOT,"hpc","gmi_k4_array.rendered.sbatch"); RECEIPT=os.path.join(ROOT,"hpc","GMI_K4_SUBMISSION_RECEIPT_V3.json")
...
def main():
    ...
    for p in (PROBE,TEMPLATE,FREEZE,GEN,SUCCESSOR,BEACON):
        if not os.path.exists(p): raise SystemExit(f"required V3 artifact absent: {p}")
    probe=json.load(open(PROBE)); succ=json.load(open(SUCCESSOR)); beacon=json.load(open(BEACON))
    if probe.get("status")!="DEVELOPMENT_ONLY_MEASURED_NOT_SUBMITTED": raise SystemExit("bad development probe status")
    if succ.get("status")!="FROZEN_BEFORE_V3_PUBLIC_BEACON_AND_ANY_V3_PROTECTED_RESULT": raise SystemExit("bad V3 successor freeze")
    if beacon.get("status")!="ACQUIRED_NO_REROLL" or not beacon.get("no_reroll") or not beacon.get("sha256_signature_consistency_verified"): raise SystemExit("bad V3 beacon")
    hashes={"freeze_sha256":h(FREEZE),"generator_freeze_sha256":h(GEN),"successor_freeze_sha256":h(SUCCESSOR)}
    for k,v in hashes.items():
        if probe.get(k)!=v: raise SystemExit(f"stale development probe: {k}")
    ...
    pilot=probe["pilot"]; sizing=probe["sizing_rule_frozen_here"]; wall=max(float(sizing["minimum_wall_seconds"]),float(pilot["wall_seconds"])*float(sizing["time_safety_factor"])); mem=max(int(sizing["minimum_memory_mib"]),int(math.ceil((max(1.0,float(pilot["peak_rss_kb"]))/1024.0)*float(sizing["memory_safety_factor"]))))
    ...
    open(RENDERED,"w").write(src); rec={"schema":"GMIK4SubmissionReceiptV4","status":"RENDERED_NOT_SUBMITTED",**hashes,"public_beacon_sha256":h(BEACON),"public_beacon_round":beacon.get("target_round"),"probe_host":probe.get("host"),"pilot":pilot,"account":a.account,"partition":a.partition,"rendered":{"walltime":rep["__WALLTIME__"],"mem_per_cpu":rep["__MEM__"],"array":arr},"rendered_sha256":hashlib.sha256(src.encode()).hexdigest(),"job_id":None,"submitted_at_unix":None,"sbatch_stdout":None,"sbatch_stderr":None}
    if a.submit:

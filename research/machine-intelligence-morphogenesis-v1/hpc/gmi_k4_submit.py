#!/usr/bin/env python3
"""Render/submit protected K4 V7 only after independent unique-beacon verification."""
from __future__ import annotations
import argparse,hashlib,json,math,os,re,subprocess,time
ROOT=os.path.dirname(os.path.dirname(os.path.abspath(__file__)));PROBE=os.path.join(ROOT,"LUNARC_ENV_PROBE_V1.json");PRED=os.path.join(ROOT,"GMI_K4_LOFO_FREEZE_V1.json");SCI=os.path.join(ROOT,"GMI_K4_NULL_AWARE_SUCCESSOR_FREEZE_V5.json");EXEC=os.path.join(ROOT,"GMI_K4_EXECUTION_FREEZE_V7.json");BEACON=os.path.join(ROOT,"GMI_K4_PUBLIC_BEACON_V7.json");TEMPLATE=os.path.join(ROOT,"hpc","gmi_k4_array.sbatch");RENDERED=os.path.join(ROOT,"hpc","gmi_k4_array.rendered.sbatch");RECEIPT=os.path.join(ROOT,"hpc","GMI_K4_SUBMISSION_RECEIPT_V8.json")
def h(p):return hashlib.sha256(open(p,"rb").read()).hexdigest()
def hhmmss(x):s=max(60,int(math.ceil(x/60.0)*60));hh,r=divmod(s,3600);mm,ss=divmod(r,60);return f"{hh:02d}:{mm:02d}:{ss:02d}"
def main():
    ...
    for p in (PROBE,PRED,SCI,EXEC,BEACON,TEMPLATE):
        if not os.path.exists(p):raise SystemExit(f"required V7 artifact absent: {p}")
    pr=json.load(open(PROBE));sci=json.load(open(SCI));ex=json.load(open(EXEC));b=json.load(open(BEACON))
    if pr.get("schema")!="LUNARCEnvProbeV8" or pr.get("status")!="DEVELOPMENT_ONLY_MEASURED_NOT_SUBMITTED":raise SystemExit("bad V7 probe")
    ...
    if pr.get("freeze_hashes")!=hs:raise SystemExit("stale V7 development probe")
    ...
    arr=f"0-263%{max(1,min(a.array_concurrency,264))}";src=open(TEMPLATE).read();rep={"__ACCOUNT__":a.account,"__PARTITION__":a.partition,"__WALLTIME__":hhmmss(wall),"__MEM__":f"{mem}M","__ARRAY__":arr}
    ...
    open(RENDERED,"w").write(src);rec={"schema":"GMIK4SubmissionReceiptV8","status":"RENDERED_NOT_SUBMITTED","freeze_hashes":hs,"public_beacon_sha256":h(BEACON),"verified_beacon_contract":verified,"probe_sha256":h(PROBE),"probe_host":pr.get("host"),"account":a.account,"partition":a.partition,"rendered":{"walltime":rep["__WALLTIME__"],"mem_per_cpu":rep["__MEM__"],"array":arr},"rendered_sha256":hashlib.sha256(src.encode()).hexdigest(),"job_id":None,"submitted_at_unix":None,"sbatch_stdout":None,"sbatch_stderr":None}
    if a.submit:

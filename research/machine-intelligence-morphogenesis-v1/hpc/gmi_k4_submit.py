#!/usr/bin/env python3
"""Render and optionally submit protected measured-resource K4 V4 from a measured development-only probe."""
...
FREEZE=os.path.join(ROOT,"GMI_K4_LOFO_FREEZE_V1.json"); SUCCESSOR=os.path.join(ROOT,"GMI_K4_MEASURED_RESOURCE_SUCCESSOR_FREEZE_V4.json"); BEACON=os.path.join(ROOT,"GMI_K4_PUBLIC_BEACON_V4.json")
RENDERED=os.path.join(ROOT,"hpc","gmi_k4_array.rendered.sbatch"); RECEIPT=os.path.join(ROOT,"hpc","GMI_K4_SUBMISSION_RECEIPT_V5.json")
...
def main():
    ...
    for p in (PROBE,TEMPLATE,FREEZE,SUCCESSOR,BEACON):
        if not os.path.exists(p): raise SystemExit(f"required V4 artifact absent: {p}")
    probe=json.load(open(PROBE)); succ=json.load(open(SUCCESSOR)); beacon=json.load(open(BEACON))
    if probe.get("schema")!="LUNARCEnvProbeV5" or probe.get("status")!="DEVELOPMENT_ONLY_MEASURED_NOT_SUBMITTED": raise SystemExit("bad V4 development probe")
    if succ.get("status")!="FROZEN_BEFORE_V4_PUBLIC_BEACON_AND_ANY_V4_SEARCH_RESULT": raise SystemExit("bad V4 successor freeze")
    if beacon.get("schema")!="GMIK4PublicBeaconV4" or beacon.get("status")!="ACQUIRED_NO_REROLL" or not beacon.get("no_reroll") or not beacon.get("sha256_signature_consistency_verified"): raise SystemExit("bad V4 beacon")
    hashes={"freeze_sha256":h(FREEZE),"successor_freeze_sha256":h(SUCCESSOR)}
    for k,v in hashes.items():
        if probe.get(k)!=v: raise SystemExit(f"stale V4 development probe: {k}")
    ...
    wall=max(float(sizing["minimum_wall_seconds"]),float(pilot["projected_protected_wall_seconds"])*float(sizing["time_safety_factor_on_projected_full_run"]))
    mem=max(int(sizing["minimum_memory_mib"]),int(math.ceil((max(1.0,float(pilot["peak_rss_kb"]))/1024.0)*float(sizing["memory_safety_factor"]))))
    ...
    rec={"schema":"GMIK4SubmissionReceiptV5","status":"RENDERED_NOT_SUBMITTED",**hashes,"public_beacon_sha256":h(BEACON),"public_beacon_round":beacon.get("target_round"),"probe_host":probe.get("host"),"pilot":pilot,"account":a.account,"partition":a.partition,"rendered":{"walltime":rep["__WALLTIME__"],"mem_per_cpu":rep["__MEM__"],"array":arr},"rendered_sha256":hashlib.sha256(src.encode()).hexdigest(),"job_id":None,"submitted_at_unix":None,"sbatch_stdout":None,"sbatch_stderr":None}
    if a.submit:

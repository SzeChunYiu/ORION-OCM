#!/usr/bin/env python3
"""Acquire the unique future drand quicknet beacon committed by K4 execution freeze V6."""
from __future__ import annotations
import datetime as dt,hashlib,json,os,subprocess,time,urllib.request
ROOT=os.path.dirname(os.path.dirname(os.path.abspath(__file__)));OUT=os.path.join(ROOT,"GMI_K4_PUBLIC_BEACON_V6.json");FREEZE_COMMIT="648e49b1b4703290c7cb7e71a970e1b591ff1d0a";CHAIN_HASH="52db9ba70e0cc0f6eaf7803dd07447a1f5477735fd3f661792ba94600c84e971";GENESIS,PERIOD,DELAY=1692803367,3,900;BASE=f"https://api.drand.sh/{CHAIN_HASH}/public"
...
def main():
    if os.path.exists(OUT):raise SystemExit("K4 V6 beacon exists; no-reroll rule forbids overwrite")
    ct=commit_unix();rnd=round_for(ct+DELAY);rtime=GENESIS+(rnd-1)*PERIOD;now=int(time.time())
    if now<rtime:raise SystemExit(f"target round {rnd} not public yet: target_unix={rtime}, now={now}")
    url=f"{BASE}/{rnd}"
    ...
    rec={"schema":"GMIK4PublicBeaconV6","status":"ACQUIRED_NO_REROLL","no_reroll":True,"execution_freeze_commit":FREEZE_COMMIT,"freeze_commit_unix":ct,"freeze_commit_iso_utc":dt.datetime.fromtimestamp(ct,dt.timezone.utc).isoformat(),"delay_seconds":DELAY,"network":"League of Entropy drand quicknet mainnet","chain_hash":CHAIN_HASH,"genesis_time":GENESIS,"period_seconds":PERIOD,"target_round":rnd,"target_round_time_unix":rtime,"source_url":url,"retrieved_at_unix":now,"http_status":status,"signature":sig,"randomness":randomness,"api_randomness":api_rand,"sha256_signature_consistency_verified":True,"bls_signature_verified":False}

#!/usr/bin/env python3
"""Render/submit the protected 256-task K5 V5 array from a measured LUNARC probe."""
from __future__ import annotations
import argparse,hashlib,json,math,os,re,subprocess,time
ROOT=os.path.dirname(os.path.dirname(os.path.abspath(__file__)));PROBE=os.path.join(ROOT,"LUNARC_K5_BH_ENV_PROBE_V5.json");EXEC=os.path.join(ROOT,"GMI_K5_BH_EXECUTION_FREEZE_V5.json");BEACON=os.path.join(ROOT,"GMI_K5_BH_PUBLIC_BEACON_V5.json");TEMPLATE=os.path.join(ROOT,"hpc","gmi_k5_bh_array.sbatch");RENDERED=os.path.join(ROOT,"hpc","gmi_k5_bh_array.rendered.sbatch");RECEIPT=os.path.join(ROOT,"hpc","GMI_K5_BH_SUBMISSION_RECEIPT_V5.json")
def h(p):return hashlib.sha256(open(p,"rb").read()).hexdigest()
def hhmmss(x):
    s=max(60,int(math.ceil(x/60)*60));hh,r=divmod(s,3600);mm,ss=divmod(r,60);return f"{hh:02d}:{mm:02d}:{ss:02d}"
def main():
    ...
    if pr.get("schema")!="LUNARCK5BHEnvProbeV5" or pr.get("status")!="DEVELOPMENT_ONLY_MEASURED_NOT_SUBMITTED":raise SystemExit("bad K5 V5 probe")
    if ex.get("status")!="FROZEN_BEFORE_K5_V5_BEACON_AND_PROTECTED_EXECUTION":raise SystemExit("bad execution freeze")
    if b.get("schema")!="GMIK5BHPublicBeaconV5" or b.get("status")!="ACQUIRED_NO_REROLL" or not b.get("sha256_signature_consistency_verified"):raise SystemExit("bad V5 beacon")
    if pr.get("execution_freeze_sha256")!=h(EXEC):raise SystemExit("stale K5 V5 probe")
    ...
    open(RENDERED,"w").write(src);rec={"schema":"GMIK5BHSubmissionReceiptV5","status":"RENDERED_NOT_SUBMITTED","execution_freeze_sha256":h(EXEC),"public_beacon_sha256":h(BEACON),"public_beacon_round":b.get("target_round"),"probe_sha256":h(PROBE),"probe_host":pr.get("host"),"account":a.account,"partition":a.partition,"rendered":{"walltime":repl["__WALLTIME__"],"mem_per_cpu":repl["__MEM__"],"array":arr},"rendered_sha256":hashlib.sha256(src.encode()).hexdigest(),"job_id":None,"submitted_at_unix":None,"sbatch_stdout":None,"sbatch_stderr":None}
    if a.submit:

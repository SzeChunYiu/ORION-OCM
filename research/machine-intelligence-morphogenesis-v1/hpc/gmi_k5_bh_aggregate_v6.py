...
ROOT=os.path.dirname(os.path.dirname(os.path.abspath(__file__)));PHASE=os.path.join(ROOT,"GMI_K5_BH_PHASE_FREEZE_V1.json");SCORE=os.path.join(ROOT,"GMI_K5_BH_SCORING_FREEZE_V1.json");C2=os.path.join(ROOT,"GMI_K5_BH_DESIGN_CORRIGENDUM_V2.json");C3=os.path.join(ROOT,"GMI_K5_BH_CONTINUAL_CORRIGENDUM_V3.json");ADM=os.path.join(ROOT,"GMI_K5_BH_ADMISSIBILITY_SCORING_ADDENDUM_V2.json");EXEC=os.path.join(ROOT,"GMI_K5_BH_EXECUTION_FREEZE_V6.json");BEACON=os.path.join(ROOT,"GMI_K5_BH_PUBLIC_BEACON_V6.json");RES=os.path.join(ROOT,"microscopes","results","k5_bh_v6")
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument("--pattern",default=os.path.join(RES,"K5BHV6_*.json"));ap.add_argument("--out-json",default=os.path.join(RES,"K5_BH_AGGREGATE_V6.json"));a=ap.parse_args();phase,ph=jh(PHASE);score,sc=jh(SCORE);c2,c2h=jh(C2);c3,c3h=jh(C3);adm,ah=jh(ADM);ex,eh=jh(EXEC);b,bh=jh(BEACON)
 from hpc.gmi_beacon_verify import verify_beacon
 ver=verify_beacon(repo_root=ROOT,freeze_path=EXEC,beacon=b,expected_schema="GMIK5BHPublicBeaconV6")
 hs={"phase":ph,"score":sc,"corr2":c2h,"corr3":c3h,"admissibility":ah,"execution":eh,"beacon":bh};p=plan(ex);by=collections.defaultdict(list);errs=[];files=glob.glob(a.pattern)
 for path in files:
  try:
   r=json.load(open(path));r["_path"]=path
   if r.get("schema")!="GMIK5BHTaskReceiptV6":errs.append({"path":path,"error":"schema"});continue
 ...
 for i,s in enumerate(p):
  ...
  key={**s,"phase":ph,"score":sc,"corr2":c2h,"corr3":c3h,"admissibility":ah,"execution":eh};th=hashlib.sha256(json.dumps(key,sort_keys=True,separators=(",",":")).encode()).hexdigest();seed=int(hashlib.sha256(("GMI-K5-BH-V6"+th+ver["randomness"]).encode()).hexdigest()[:16],16)%(2**32)
 ...
 for xs in by.values():
  if len(xs)==1:grouped[(xs[0].get("lane"),vk(xs[0].get("value")))].append(xs[0])
 ...
 lanev={l:("INVALID" if "INVALID" in cs else "THEORY_RED" if "THEORY_RED" in cs else "GREEN" if len(cs)==4 and all(x=="GREEN" for x in cs) else "INCONCLUSIVE") for l,cs in lanes.items()};complete=not errs and all(v!="INVALID" for v in lanev.values());terminal="K5_BH_V6_EXECUTION_INVALID" if not complete else "K5_BH_V6_GREEN" if all(v=="GREEN" for v in lanev.values()) else "K5_BH_V6_CONTAINS_RED" if "THEORY_RED" in lanev.values() else "K5_BH_V6_INCONCLUSIVE";rep={"schema":"GMIK5BHAggregateV6","terminal":terminal,"freeze_hashes":hs,"verified_beacon_contract":ver,"structural_errors":errs,"cell_results":cells,"lane_results":lanev,"known_form_zero_prior_global_terminal":False,"no_gap_global_terminal":False};os.makedirs(os.path.dirname(a.out_json),exist_ok=True);json.dump(rep,open(a.out_json,"w"),indent=1,sort_keys=True);print(json.dumps({"terminal":terminal,"lane_results":lanev,"errors":len(errs)},sort_keys=True));return 0 if complete else 2

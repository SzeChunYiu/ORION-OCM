#!/usr/bin/env python3
"""Fail-closed K4 V7 aggregate with independent verification of the preregistered beacon round."""
from __future__ import annotations
import argparse,collections,glob,hashlib,json,os
ROOT=os.path.dirname(os.path.dirname(os.path.abspath(__file__)));PRED=os.path.join(ROOT,"GMI_K4_LOFO_FREEZE_V1.json");SCI=os.path.join(ROOT,"GMI_K4_NULL_AWARE_SUCCESSOR_FREEZE_V5.json");EXEC=os.path.join(ROOT,"GMI_K4_EXECUTION_FREEZE_V7.json");BEACON=os.path.join(ROOT,"GMI_K4_PUBLIC_BEACON_V7.json");RES=os.path.join(ROOT,"microscopes","results","k4_v7")
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument("--pattern",default=os.path.join(RES,"K4V7_*.json"));ap.add_argument("--reveal-names",action="store_true");ap.add_argument("--out-json",default=os.path.join(RES,"K4_V7_AGGREGATE.json"));ap.add_argument("--out-md",default=os.path.join(RES,"K4_V7_AGGREGATE.md"));a=ap.parse_args()
    ...
    if ex.get("status")!="FROZEN_BEFORE_K4_V7_BEACON_AND_PROTECTED_EXECUTION":raise SystemExit("bad execution freeze")
    from hpc.gmi_beacon_verify import verify_beacon
    verified=verify_beacon(repo_root=ROOT,freeze_path=EXEC,beacon=b,expected_schema="GMIK4PublicBeaconV7")
    ...
    for path in files:
        try:
            ...
            if r.get("schema")!="GMIK4TaskReceiptV8":parse.append({"path":path,"error":f"wrong schema {r.get('schema')}"});continue
    ...
    for i,spec in enumerate(p):
        ...
        key={**spec,"prediction":ph,"science":sh,"execution":eh};th=hashlib.sha256(json.dumps(key,sort_keys=True,separators=(",",":")).encode()).hexdigest();seed=int(hashlib.sha256(("GMI-K4-V7"+th+verified["randomness"]).encode()).hexdigest()[:16],16)%(2**32)
        if r.get("task_hash")!=th:er.append("task_hash_mismatch")
        if r.get("seed")!=seed:er.append("seed_mismatch")
        if r.get("public_beacon_round")!=verified["target_round"]:er.append("beacon_round_mismatch")
        if r.get("public_beacon_randomness")!=verified["randomness"]:er.append("beacon_randomness_mismatch")
    ...
    terminal=("K4_V7_EXECUTION_INVALID" if errors else "K4_V7_ALL_REGISTERED_CELLS_GREEN__IG4_IG5_PENDING" if allgreen else "K4_V7_CONTAINS_THEORY_RED" if anyred else "K4_V7_CONTAINS_INCONCLUSIVE" if any(cnt.get(x,0) for x in ("INCONCLUSIVE_GRAMMAR","INCONCLUSIVE_SEARCH","TASK_FAILED")) else "K4_V7_MIXED")
    ...
    rep={"schema":"GMIK4NullAwareAggregateV7","terminal":terminal,"freeze_hashes":hs,"verified_beacon_contract":verified,"expected_tasks":len(p),"receipts_found":len(files),"validated_rows":len(rows),"structural_errors":errors,"verdict_counts":dict(cnt),"family_summary":fam,"null_dominance_counts":dict(nulls),"all_complete":complete,"all_green_same_author_v7_scope":allgreen,"known_form_zero_prior_global_terminal":False,"no_gap_global_terminal":False,"claim_ceiling":"Execution-frozen, unique-beacon-verified, null-aware DG-10-corrected same-author K4 only; IG-4/IG-5 and external evidence gates remain."}
    os.makedirs(os.path.dirname(a.out_json),exist_ok=True);json.dump(rep,open(a.out_json,"w"),indent=1,sort_keys=True)
    lines=["# Protected K4 V7 aggregate","",f"Terminal: `{terminal}`","",f"Verified beacon round: **{verified['target_round']}**",f"Validated: **{len(rows)}/{len(p)}**; structural error groups: **{len(errors)}**.","","## Verdicts",""]+[f"- `{k}`: {cnt[k]}" for k in sorted(cnt)]+["","## Null dominance",""]+[f"- `{k}`: {nulls[k]}" for k in sorted(nulls)]+["","## Families",""]
    for f in sorted(fam):
    ...
    open(a.out_md,"w").write("\n".join(lines)+"\n");print(json.dumps({"terminal":terminal,"counts":dict(cnt),"nulls":dict(nulls),"validated":len(rows),"errors":len(errors),"beacon_round":verified["target_round"]},sort_keys=True));return 0 if not errors else 2

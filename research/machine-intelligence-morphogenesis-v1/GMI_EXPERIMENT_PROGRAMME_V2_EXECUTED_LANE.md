...
| probabilistic / generative | M3, S3 (particles), S7 (posterior averaging over the K best programs, K ≤ 16 at 8 bits) | yes | compiled (GMI-T6; S7 in RV-029/031) | no | S7 admissible at full observation (0.875) and inadmissible at few observations (0.79) and under label noise (0.83) — the opposite of the classical Bayesian advantage, attributed to 4-bit weight precision; never the occupant (RV-031). **PRECISION_GATED: a wider-precision universe is the declared next instrument** |
...
| attention / Transformer | S5a (similarity-weighted retrieval, NORMALIZE/SCORE kernel) | yes | compiled (RV-034) | no | closed-form admissibility held at four points (0.8958, 0.8542, 0.7708, 0.75): admissible at a ≤ 5/16, strictly inside the hard-kNN region; dominated by hard kNN on every cell; 4.7% cheaper where normalization is native (B3). Next instrument: a kernel-temperature axis |
| reinforcement / temporal credit assignment | R1 consistency search (RL as inference), R2 tabular Monte-Carlo Q, R3 REINFORCE policy net, R4 kNN credited action (E_credit: episodes of four actions, one aggregated return) | yes | compiled (RV-030) | no | R1 is the sole admissible occupant at 8 and 32 episodes in every basis (predicted before the run); R2/R4 do not generalize; R3 never admissible at four learning rates and four budgets (estimator/precision-gated; representationally able) |
...
**What S1 still needs (in order of leverage):** (a) instrument parameters for the two precision/estimator-gated families — a wider-precision universe (FRAC_BITS ≥ 8) for the probabilistic row and a lower-variance estimator (value regression / actor-critic) for the policy-gradient row — and a kernel-temperature axis for attention; each is a compiled program in the same universe with charged constants; (b) an ecology axis on which the stochastic form is predicted to win (partial observability), so that M3/S3 gets a D3 row; (c) D2 for the dense form (RV-023), after which D2 for the search form becomes attemptable.

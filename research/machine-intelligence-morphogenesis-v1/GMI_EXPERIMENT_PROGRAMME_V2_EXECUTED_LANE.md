...
- Freeze: a neutral grammar small enough to enumerate (declared at freeze: f depth ≤ 2 over 4 input bits + 4 cells, g ≤ 2 writes of depth ≤ 2; expected ≤ 10^7 candidates; canonicalized by dead-write elimination; evaluated on a registered ecology family: E_bind16, E_smooth (unseen criterion), E_smooth8_div, parity-balanced).
- Prediction frozen before enumeration: every point of the (capability, cost) Pareto frontier of every ecology classifies into a registered class {INERT, LOCAL_MEMORY, STORE_PLUS_NUMERIC, NUMERIC_SPARSE, NUMERIC_DENSE, SEARCH/ALGEBRAIC} by the signature classifier v2; and the class that occupies each ecology's frontier is the one the cost model names (memory on binding, dense/kNN on smooth, none on parity unless XOR-closed).

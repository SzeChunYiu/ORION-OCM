...
| **G6b** | admissibility was recovered by neutral search; the FORM was not | the biosphere B1 gate blocks every unknown-form claim, and an admissibility-only gate is passable by bulk: all 24 winners of `RV-377-057` are 133–209 nodes against the planted learner's 47, at a lower score, and every one classifies as memory or hybrid | morphogenetic atrophy (`RV-377-071`): greedy deletion to a fixed point under the constraint score ≥ θ, plus store ablation and per-rule ablation, applied to all 24 winners. Closure = an atrophied machine whose surviving mechanism is a named known form, with the match exhibited structurally rather than asserted |
...
| 21 | every row's **serve path must be charged**; a receipt must assert that no admissible row has zero execution cost per query | `RV-377-072` (an unmetered serve took 175 frontier cells at 802 against 12 296) |
| 22 | an obligation is history-dependent only where the best **constant** answer is below θ; every temporal ecology carries a constant-answer control row and clauses are evaluated only where that control fails | `RV-377-072` (the running-maximum obligation saturates and a memoryless row reached 0.9062) |

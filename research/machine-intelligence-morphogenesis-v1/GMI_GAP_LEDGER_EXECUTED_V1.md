...
| 30 | parallel lanes must draw revival-record ids from **disjoint reserved ranges**, and a lane that publishes a record first keeps the bare id | three lanes independently used `RV-377-052` and two used `RV-377-054`; repaired by renumbering the last lane to 078–080 with a redirect note inside each record |

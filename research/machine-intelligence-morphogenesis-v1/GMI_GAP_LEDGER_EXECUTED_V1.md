...
| 28 | **parent-maximality binds the SUBJECT of a claim, not only its opponent.** Before concluding that a carrier is too expensive, give it its minimal declaration and its cheapest registered encoding, and report how much of the margin was padding | `RV-377-077` (`RV-377-076`'s cost verdict was drawn against a row whose description was 4.128× larger than necessary; **2 352 of its 2 508-bit margin was padding** — the verdict survived, but only just, and it was luck that it did) |
| 29 | a **monotone map** over a declared constant table must be reported in its **LADDER** form (its `S` breakpoints, read by `GT`/`ADD`, using no store) as well as its table form, whenever its output range is small — under rule 25 the two are charged differently by a factor that can exceed four | `RV-377-077` (ladder 188 charged activations/query and **untouched** by the registered table charge; the same row as a trimmed table 68 → **392** under it) |
| 30 | an opponent built to discharge rule 19 must be **scored against the instrument ladder** and reported as **weak** if it fails everywhere; a record may not cite the *existence* of an opponent as evidence that parent-maximality was met | `RV-377-078` (two graded count rows built before the verdict scored exactly **0.0 at all seven instruments** — a sum of likelihoods is not a product; the record reports its own opponents as bad) |
| 31 | a claim that a carrier's loss is **structural rather than precisional** must exhibit the **executed instrument ladder** for that carrier — a top-M ranking can reconstruct what a threshold discards | `RV-377-078` (`QCOUNT` was argued to be irrecoverably thresholded; it recovers to **0.923404 at 12 bits** and is **non-monotone in precision**: 0.822415 / 0.735097 / 0.923404 at 8 / 10 / 12) |
| 32 | a **"no cell exists"** verdict must name every parameter of the **ecology construction** held fixed while it was established — class size, class arity, evidence length — because a negative at one value of an unvaried parameter is a negative about **that value** | `RV-377-079` (three records' worth of precision residual, all at a 32-member class; at **K = 128** an 8-bit row occupies and has **zero** cost-coordinate dominators) |
| 33 | when the **flat** and **scaled** description bases **disagree on a verdict**, the record must report **both terminals** and may not elect one as primary — a sensitivity column that changes an answer has stopped being a sensitivity column | `RV-377-079` (first disagreement in this lane: 7 cells under `scaled`, **0 under `flat` at every K**) |
| 34 | where a cost comparison is **affine in a construction parameter**, the record must fit and report the **exact law and its root**, not the verdict at the executed points — a verdict at one value of a parameter the cost function is linear in is a statement about *that value* | `RV-377-080` (the precision "residual" is exactly **−2K + 376** bits under the registered price, verified to the unit at **seven** class sizes, root **K = 188**; four records measured it at K = 32 and called it a residual) |
| 35 | occupancy counts may **not** be compared across records whose **portfolios differ** — the H and r grids are derived from the portfolio's own analytic crossovers, so adding one row moves the denominator. Compare occupied **cell sets**, or a grid fixed in advance | `RV-377-081` (adding one row moved the grid 444 → 900 cells; a fraction **fell** 0.836 % → 0.678 % while its count **rose** 23 → 36) |
| 36 | when a carrier's readout can be **materialized on update**, the record must charge **both** forms and report which axis each helps: the **read** path governs `H`, the **update** path governs `r`, and a fix to one is silent on the other | `RV-377-081` (caching the exponent cut `exec_q` and `ver_e` by 2.909 and moved the crossing root 188 → 142.31, while leaving **every** occupied cell at `r = 0`) |
| 37 | where two declared **description bases** give **different occupancy verdicts**, the record must report the verdict under **each** and identify **which quantity the bases price differently**; a basis disagreement is a finding about the cost model and may **not** be presented as a sensitivity | `RV-377-082` (under `flat` `QCOUNT` dominates every 8-bit log row at every class size, 42/42; under `scaled` the domination breaks at a computable root and the 8-bit row occupies — the bases differ on the 2-bit-per-scalar discount a narrower word earns) |

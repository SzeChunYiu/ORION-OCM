...
(GMI_TRANSFORMER_MICROFEATURE_THEOREMS_V1.md section 16, programme X-TMT1..X-TMT13, plus X-TMT5 and X-TMT7).
...
def x_tmt1(n=4):
    ...
    return {"theorem": "TMT-1 order-information necessity", "scope": f"all binary sequences of length {n}, all permutations, 3 obligations x 3 encoders", "kind": KIND, "cells": out, "passed": ok}
...
def x_tmt2():
    ...
    return {"theorem": "TMT-2 finite-window impossibility", "scope": "binary sequences of length W+2, obligation = first token, W in {1,2,3}", "kind": KIND, "cells": out, "passed": ok}
...
def x_tmt3(k=4):
    ...
    return {"theorem": "TMT-3 fixed-routing union lower bound", "scope": f"pointer dependency families on {k} tokens ({len(inputs_a)} inputs), a single-pointer family, and an input-independent local window", "kind": KIND, "cells": out, "passed": ok}
...
def x_tmt4():
    ...
    return {"theorem": "TMT-4 softmax variational characterization", "scope": "4 score vectors x 3 temperatures; simplex grid of step 1/40 (861 points); float64, tolerance 1e-9", "kind": KIND + "__NUMERICAL", "cells": out, "worst_grid_gap": worst, "passed": ok}
...
def x_tmt5():
    ...
    return {"theorem": "TMT-5 causal information legality", "scope": f"exact dyadic-kernel attention on {n} tokens; perturb the last token; compare outputs at positions < {n-1}", "kind": KIND,
...
def x_tmt6():
    ...
    same = outs_a == outs_b
    return {"theorem": "TMT-6 exact cache substitution", "scope": f"{n}-step autoregressive continuation, d = {d}, exact rational projections and dyadic-kernel attention", "kind": KIND,
...
def x_tmt7():
    ...
    return {"theorem": "TMT-7 implementation invariance of exact attention", "scope": "6 tokens; naive vs online block-wise (blocks 2 and 3) normalization; exact with the dyadic kernel, float64 with real exp", "kind": KIND,
...
def x_tmt8():
    ...
    return {"theorem": "TMT-8 tied-parameter function-class inclusion", "scope": "y = W2 relu(W1 x), W in {-1,0,1}^(2x2), inputs {-1,0,1}^2; tied = (W2 = W1^T)", "kind": KIND,
...
def x_tmt9():
    ...
    return {"theorem": "TMT-9 residual Jacobian identity", "scope": "three exact rational 2x2 linear blocks; finite-difference Jacobians (exact for linear maps); end-to-end products", "kind": KIND, "cells": cells, "passed": ok}
...
def x_tmt10():
    ...
    for s in seqs:
        ...
        ok &= prod == p[s]
    return {"theorem": "TMT-10 autoregressive chain factorization", "scope": f"exact rational distribution over binary sequences of length {n} (8 sequences)", "kind": KIND, "cells": {"all_sequences_factor_exactly": ok}, "passed": ok}
...
def x_tmt11():
    ...
    return {"theorem": "TMT-11 population cross-entropy optimum", "scope": "3 contexts x 2 outcomes, exact rational p and three candidate q; float64 logs, tolerance 1e-12", "kind": KIND + "__NUMERICAL", "cells": cells, "passed": ok}
...
def x_tmt12():
    ...
    return {"theorem": "TMT-12 quantization collision no-go", "scope": "64 authority states in {0..7}^2, obligation = parity of the sum, three quantizers", "kind": KIND, "cells": cells, "passed": ok}
...
def x_tmt13():
    ...
    return {"theorem": "TMT-13 key/value cache state scaling", "scope": f"L={L}, n={n}, d={d}, H={H}; explicit element count vs 2 L n d H_kv", "kind": KIND, "cells": cells, "passed": ok}


CHECKS = [x_tmt1, x_tmt2, x_tmt3, x_tmt4, x_tmt5, x_tmt6, x_tmt7, x_tmt8, x_tmt9, x_tmt10, x_tmt11, x_tmt12, x_tmt13]
NOT_CHECKED = {"TMT-14 fixed-window / cache distinction": "type distinction (no finite check; X-TMT6 shows caching changes op counts, X-TMT2 shows the window bounds the distinctions)",
               "TMT-15 decoding policy / model distribution distinction": "type distinction (no finite check)"}


def main(path=None):
    results = [c() for c in CHECKS]
    receipt = {"schema": "GMI_TRANSFORMER_MICROFEATURE_EXACT_V1", "issue": [377, 422], "source_theorems": "GMI_TRANSFORMER_MICROFEATURE_THEOREMS_V1.md (TMT-1..15), programme section 16",

...
def test_all_frozen_obligations_have_an_executable_generator():
    ...
    assert len(SAFE["families"]) == 22 and len(SAFE["grammars"]) == 3


def test_historical_names_are_not_search_primitives():
    surface = json.dumps([{"fid": f.fid, "caps": f.caps, "vector": f.vector()} for f in k4_v1.FORMS], sort_keys=True).lower()
...
def test_v1_property_vector_cannot_change_search_winner():
    ...
    altered["families"][fid]["property_vector"] = {**altered["families"][fid]["property_vector"],
...
def test_v2_property_vector_still_cannot_change_search_stream():
    ...
    assert a["search_digest"] == b["search_digest"]
    assert a["world_profile"] == b["world_profile"]
    # If the cell has a winner, post-search adjudication is allowed to notice the altered target.
    if "target_vector_match" in a and "target_vector_match" in b:
        assert a["target_vector_match"] != b["target_vector_match"]
...
def test_name_key_is_irrelevant_even_if_present():
    with_name = copy.deepcopy(FREEZE); without_name = copy.deepcopy(FREEZE); without_name.pop("name_key", None)

# M12 pre-registration V2 — one persistent OCM across a frozen heterogeneous lifetime

Frozen after the V1 DEV_CALIBRATION run (research/ocm-m12/M12_LIFETIME_EVAL_V1.json) and before any V2 outcome is read. Issue #14. The evaluation code is
...
| Statistics | paired exact (McNemar-style) with pre-registered TOST δ = 0.05, α = 0.05 (`ocm.evaluation.stats`) on the **principal ordering O1 only** (O2/O3 repeat the same tasks under a different history and are robustness replicates, reported descriptively; pooling them would be pseudo-replication, ledger S32); inferential terminals only at n ≥ 40 pairs per family, otherwise DESCRIPTIVE |
...
protected exposure > 0; external IO > 0; identity reset or split (the ledger root changes or a component points at a different runtime at any phase boundary, ledger S31); a LIVE commitment resting on revoked evidence after F; a
...
## 6. Changes from V1 (all made before any V2 outcome access; none changes a mechanism)

1. Harness: `__restart__` inside phase A reopened a ChatSession at the lifetime root instead of `root/chat`, silently splitting the runtime identity (turns 38–39 of d09 answered Unknown; later phases ran on a second instance). Fixed: restart persists and reopens the same ledger root and re-points every component; a kill gate now checks the ledger root and single-runtime invariant at every phase boundary (S31).
2. Statistics: inferential claims on O1 only (S32).
3. Phase G: the learning-policy fault is planted at the skill level (S33).

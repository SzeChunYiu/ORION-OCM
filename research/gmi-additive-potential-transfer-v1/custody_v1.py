...
def verify(root):
    ...
    if set(manifest)!={"schema","payloads"} or manifest["schema"]!="APT_MANIFEST_V1":
    ...
    if len(parents)!=4 or {r["label"] for r in parents}!={"WTT","ARC","SPS","WTT_ORACLE"}:
        raise ValueError("parent coverage mismatch")
    for row in parents:
        data=found[row["copy"]].read_bytes()
        if row["commit"]!="c37fbfba8f63169658d5f64b42ae7da428e09df0":

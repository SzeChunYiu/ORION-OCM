...
import check_lean_v15 as kernel
...
class KernelGuardTests(unittest.TestCase):
    def test_registered_statement_types_reject_source_valid_mutants(self):
        ...
        for filename, pattern, replacement in (
            ("QuotientOrderV15.lean", r"theorem context_quotient_comparison[\s\S]*?(?=\nend PartialContextV15)",
             "theorem context_quotient_comparison : True := True.intro\n"),
            ("SeparationV15.lean", r"theorem admitted_nonrecovery[\s\S]*?(?=\nend PartialContextV15)",
             "theorem admitted_nonrecovery : True := True.intro\n"),
        ):
        ...
        for index, mutation in enumerate(mutations):
            with self.subTest(mutation=index), tempfile.TemporaryDirectory(prefix="v15-kernel-hostile-") as directory:
                root = Path(directory)

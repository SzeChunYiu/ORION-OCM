...
controls_pass=checks["exit_code"]==0 and "Ran 9 tests" in text and text.rstrip().endswith("OK")
...
source_stable=all(x["sources_before"]==x["sources_after"] for x in (checks,startup))
passed=controls_pass and startup_pass and donor_pass and source_stable
control=json.loads((root/"CONTROL-RESULT.json").read_bytes())
assert control["tests_run"]==9 and control["passed"] and control["pid"]==checks["pid"]
...
receipt={"schema":"ordinary.screening-observer-authored-qualification.v1","pid":os.getpid(),
 "python":{"path":str(PYTHON),**identity(PYTHON)},"passed":passed,"authored_output_controls":9,
 "output_controls_pass":controls_pass,"isolated_missing_gate_startup_pass":startup_pass,
 "sources_unchanged":source_stable,"actual_gate_and_outputs_absent_before":absent_before,
 ...
 "actual_control_cwd":control["cwd"],"actual_control_modules":control["imported_modules"],"processes":{"output_controls":identity(root/"output-controls.PROCESS.json"),"startup":identity(root/"missing-gate-startup.PROCESS.json")},
 "actual_native_or_export_or_screen_calls":0,"actual_retained_proposal_decodes":0,"source_or_template_edits":False,
 "scope":"Nine authored output/source controls and one exact isolated observer entry refusal. Startup reads the fixed request metadata and refuses at absent gate before any real input or research import. No screening child executes."}

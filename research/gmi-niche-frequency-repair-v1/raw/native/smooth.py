...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "xor_mixed":
        ...
        arg = sys.argv[2]; k = -int(arg[1:]) if arg.startswith("n") else int(arg); a = k / 16
        main(coeffs=(a, a, a, a), tag=f"V34_SYM{arg}_H", n_events=16, rows=ROWS_V6, criterion="unseen")
        ...
        if which == "smooth3": main(seed=sd, coeffs=COEFFS_V3, tag=f"V22_SMOOTH3_S{sd}", n_events=16, rows=ROWS_V6, criterion="unseen")
        else:
            k = int(which); a = k / 16
            main(seed=sd, coeffs=(a, a, a, a), tag=f"V22_SYM{k}_S{sd}", n_events=16, rows=ROWS_V6, criterion="unseen")

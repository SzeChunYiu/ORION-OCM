"""CI gates for gmi-833-h-real-scale-particle-population-v1, kept out of the
...
def closure_consistency(result=None, recon=None):
    result = result or load("RESULT_V1.json")
    recon = recon or load("ISSUE_833_RECONCILIATION_H19_V1.json")
...
def annotation_budget(recon=None):
    ...
    payload is 2,143 characters (K08)."""
    recon = recon or load("ISSUE_833_RECONCILIATION_H19_V1.json")
...
def two_route_namespace():
    ...
    import independent_oracle_pp_v1  # noqa: F401
...
def selftest():
    ...
    recon = load("ISSUE_833_RECONCILIATION_H19_V1.json")
    checks = []
    ...
    dirty_recon["replacements"].append(
        {"anchor": recon["replacements"][0]["anchor"],
         "old": "- [ ] Bayesian inference/belief-state systems.",
         "new": "- [x] Bayesian inference/belief-state systems. — ✅ fabricated"})

...
def main(out):
    opened=[]
    allowed=set(C.freeze()["sources"])|set(C.freeze()["b_inputs"])|{
    ...
    def audit(event,args):
        ...
        try:relative=str(path.relative_to(C.HERE))
        except ValueError:return
        if out==path or out in path.parents:return
        if relative.endswith(".pyc") and relative.startswith("engine/"):
            C.require(not path.exists(),"B_BYTECODE_PRESENT:"+relative)
        else:C.require(relative in allowed,"B_FORBIDDEN_READ:"+relative)
        opened.append({"path":relative,"mode":str(args[1]),"flags":args[2]})
    sys.addaudithook(audit);C.AUDIT_READS=opened
    work={};engine=C.engine();P=C.load("life_payload","life_payload.py")
    ...
    for mode in ("wrong_hole","wrong_target"):
        ...
        C.require(result["terminal"]=="NATIVE_REJECTED" and result["native_calls"]==1,
                  "NATIVE_NEGATIVE_DID_NOT_REACH_REFUSAL:"+mode)

...
def main(out):
    work={};C.ACTIVE_WORK=work;engine=C.engine();packet=C.read(C.HERE/"inputs/fixtures/FIXTURES.json")
    ...
    training=C.native(claims,out/"native-training",work,
                      C.bytes_read(C.HERE/"inputs/fixtures/TRAINING-SUFFIX.mm"))
    C.write(out/"TRAINING.json",training)
    C.require(training["terminal"]=="NATIVE_VERIFIED","NATIVE_TRAINING_REFUSED")
    discovery=C.load("life_discovery","life_discovery.py").discover(training,engine,work,out/"discovery-progress")
    ...
    C.write(out/"DISCOVERY-PROPOSED.json",discovery)
    for alias_number,row in enumerate(discovery["supported"]):
        ...
        C.write(out/("ALIAS-"+str(alias_number)+".json"),{"id":row["id"],"alias":row["alias"],"work":work})

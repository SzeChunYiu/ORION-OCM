"""Fixed parent-translation closure contract and inherited source custody."""
import hashlib
...
def load(path):
    spec = importlib.util.spec_from_file_location("v27_contract_" + path.stem, path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


SPECS = load(Path(__file__).with_name("audit_specs_v27.py"))
PINS = load(Path(__file__).with_name("audit_pins_v27.py"))
...
def atom_evidence(root, atom):
    ...
    if not PINS.FINAL:
        raise OSError("final reviewed V27 proof pins unavailable")
    return [witness(root, "RESULT_V27.json", "json_pointer", "/atoms/" + atom),
            witness(root, "oracle_v27.py", "python_symbol", "certify"),
            witness(root, "PARENT_TRANSLATIONS_V27.json", "json_pointer", "/rows")] + [
...
def repair(root, name):
    if not PINS.FINAL:
        raise OSError("final reviewed V27 proof pins unavailable")
    evidence = [witness(root, "check_parents_v27.py", "python_symbol", "evaluate"),
                witness(root, "RESULT_V27.json", "json_pointer", "/kernel" if name == "R14" else "/coverage")]
    extras = {"R1": PINS.REPAIR_DECLARATION,
              "R14": ("proof_contract_v27.py", "python_symbol", "audit_source"),
              "R15": ("oracle_v27.py", "python_symbol", "certify")}
    ...
    return {"id": name + "-parents-v27", "scope": REPAIR_SCOPES[name],
...
def validate_receipt(root, receipt):
    ...
    need(type(receipt) is dict and set(receipt) == {
        ...
        "claim_boundaries", "parent_reconciliation"}, "receipt fields")
    fixed = {"schema": "GMI_1068_PARENT_TRANSLATIONS_V27", "tests_run": 15,
             "overall_closure": "OPEN", "scientific_truth_certified": False, "claim_boundaries": BOUNDARIES,
             "parent_reconciliation": SPECS.PARENTS}
    ...
    inputs = {entry["path"].rsplit("/", 1)[-1]: entry["sha256"] for entry in artifacts
              if not entry["path"].endswith("/RESULT_V27.json")}
    ...
    exact(load(package / "audit_parents_v27.py").verify(root), SPECS.PARENTS, "original parent reconciliation drift")
    custody = load(package / "custody_v27.py")
    try:
    ...
    exact(receipt["coverage"], load(package / "coverage_v27.py").REQUIRED, "coverage receipt drift")
    need(digest(package / "proof_contract_v27.py") == PROOF_CONTRACT_SHA, "reviewed proof-type drift")
    proof = load(package / "proof_contract_v27.py")
...
def current_accounting(snapshot, amendment_carry):
    ...
    return {"schema": "GMI_1068_CURRENT_ACCOUNTING_V27", "snapshot_content_sha256": snapshot_sha,

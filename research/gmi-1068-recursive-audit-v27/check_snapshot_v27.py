"""Close only the registered R1-009 parent-translation atom; preserve every other original record; enforce complete adjudication records."""
import hashlib
...
ROOT = HERE.parents[1]
BASE = ROOT / "research/gmi-1068-recursive-audit-v26/SCOPE_SNAPSHOT_V26.json"
BASE_SHA = "9fe6b0e3c4ab2d0de34a3af453191d6e0fa99df61adb1332deb8c52b8fae74d6"
...
CONTRACT = load("audit_contract_v27", HERE / "audit_contract_v27.py")
...
def evaluate(current=None, receipt=None, accounting=None):
    ...
    need(hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA, "V26 baseline drift")
    before = json.loads(BASE.read_text())
    if current is None:
        current = json.loads((HERE / "SCOPE_SNAPSHOT_V27.json").read_text())
    if receipt is None:
        receipt = json.loads((ROOT / CONTRACT.PACKAGE / "RESULT_V27.json").read_text())
    need(type(current) is dict and set(current) == set(before), "snapshot top-level fields")
    for field in before:
        if field != "rounds":
            expected = "f2516e9740b9e0fe1bb369d69aa4b551b1f9ad3d" if field in ("observed_main", "audited_base") else before[field]
    ...
    need(result["registered_atoms"] == 222 and result["unresolved_atoms"] == 188,
    ...
    need(len(scientific) == 26, "scientific closure count drift")
    expected_accounting = CONTRACT.current_accounting(current, receipt["amendment_carry"])
    if accounting is None:
        accounting = json.loads((HERE / "CURRENT_ACCOUNTING_V27.json").read_text())
    exact(accounting, expected_accounting, "current accounting drift")
    for key, value in {"original_total": 222, "original_fulfilled": 34, "original_unresolved": 188,
                       "qualified_replacements": 2, "active_unresolved": 186}.items():

...
spec=importlib.util.spec_from_file_location('checked_snapshot_v27',HERE/'check_snapshot_v27.py')
...
class SnapshotTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.snapshot=json.loads((HERE/'SCOPE_SNAPSHOT_V27.json').read_text())
        cls.receipt=json.loads((checker.ROOT/checker.CONTRACT.PACKAGE/'RESULT_V27.json').read_text())
    ...
    def test_actual_snapshot_no_alarm(self):
        ...
        self.assertEqual(len(result['scientific_atoms_closed']),26)
        self.assertEqual((result['original_fulfilled'],result['remaining_atoms'],
                          result['qualified_replacements'],result['active_unresolved']),(34,188,2,186))
    ...
    def test_rehashed_snapshot_mutations(self):
        ...
        wrong=checker.CONTRACT.witness(checker.ROOT,'oracle_v27.py','python_symbol','raw')
        ...
        bad=deepcopy(self.snapshot);bad['rounds']['R1']['artifacts']=[a for a in bad['rounds']['R1']['artifacts']
            if a['path']!=checker.CONTRACT.PACKAGE+'/FREEZE_V27.md'];invalid.append(bad)
        bad=deepcopy(self.snapshot);path=HERE/'audit_contract_v27.py'
        ...
        self.assertEqual(len(invalid),88)
    ...
    def test_receipt_mutations(self):
        ...
        coverage_key=next(iter(self.receipt['coverage']['test_lts_v27']))
        ...
        changes=[(('atoms',),{'GMI2-R1-001':{'status':'VERIFIED_AT_REGISTERED_SCOPE'}}),
            (('registered_results','AA1','scope'),'all process ontologies have the same unique primitive fields'),
            (('registered_results','AA2','status'),'PENDING_REVIEW'),
            ...
            (('coverage','test_lts_v27',coverage_key),True),
            (('coverage','test_lts_v27',coverage_key),float(self.receipt['coverage']['test_lts_v27'][coverage_key])),
        ...
        for value in ({}, {'original_parent_ids':[],'reconciled_rows':7},
                      {'original_parent_ids':self.receipt['parent_reconciliation']['original_parent_ids'],'reconciled_rows':True},
                      {'original_parent_ids':self.receipt['parent_reconciliation']['original_parent_ids'],'reconciled_rows':7.0}):
            changes.append((('parent_reconciliation',),value))
        ...
        for path in ((),('kernel',),('source_bindings',),('coverage','test_lts_v27'),('inputs',),('registered_results',)):
    ...
    def test_coupled_result_scope_and_original_promotion(self):
        ...
        for atom_id in checker.CONTRACT.ATOM_SPECS:
            ...
            with self.assertRaisesRegex(ValueError,'registered atom adjudication drift'):
                self.check(self.rebind(snapshot),receipt)
            count+=1
        helper=checker.load('snapshot_controls_v27',HERE/'snapshot_controls_v27.py')
    ...
    def test_current_accounting_mutations(self):
        ...
        for path,value in [(('original_fulfilled',),35),(('original_unresolved',),187),
                (('active_unresolved',),185),(('qualified_replacements',),3),(('original_total',),222.0),
        ...
        invalid.append(json.loads((checker.BASE.parent/'CURRENT_ACCOUNTING_V26.json').read_text()))

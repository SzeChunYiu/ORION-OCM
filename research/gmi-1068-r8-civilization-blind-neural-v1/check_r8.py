...
src=(ROOT/"causal_derivation_v1.py").read_text().lower();hits=[t for t in deny if t in src]
if hits: raise SystemExit("CAUSAL_DENYLIST_HITS:"+repr(hits))
counts,winners=causal.run()
need(counts=={"worlds":375,"k0":15,"k1":120,"k2":240,"routeA_checks":24375,"routeB_checks":24375,"remint_checks":24375},"COUNTS")
def flat(d):
 out={}
 for k,v in d.items():
  need(len(k)==1,"NON_SINGLE_WINNER")
  out[k[0]]=v
 return out
wa=flat(winners["S_ARITH"]);wb=flat(winners["S_BRANCH"])
need(wa=={"AFFINE":15,"HINGE":360},"ARITH_WINNERS")
need(wb=={"AFFINE":15,"BRANCH":360},"BRANCH_WINNERS")
r=json.loads((ROOT/"RESULT_V1.json").read_text())
need(r["winners_S_ARITH"]=={"AFFINE":15,"HINGE":360,"BRANCH":0,"TABLE":0},"RESULT_ARITH")
need(r["winners_S_BRANCH"]=={"AFFINE":15,"HINGE":0,"BRANCH":360,"TABLE":0},"RESULT_BRANCH")
need(r["posthoc_neural_like_matches"]==360 and r["literal_historical_ignorance_claim"] is False,"HISTORY_BOUNDARY")
cl=json.loads((ROOT/"POSTHOC_CLASSIFIER_V1.json").read_text());need(cl["causal_access"] is False,"CLASSIFIER_ACCESS")

"""Close only the registered R3-009 source-specific transport atom; preserve every other original record; enforce complete adjudication records."""
import hashlib
...
ROOT = HERE.parents[1]
BASE = ROOT / "research/gmi-1068-recursive-audit-v23/SCOPE_SNAPSHOT_V23.json"
BASE_SHA = "d0e334a42aaf3e87233b596403543d910695455bc8fd246b5a05ee76e6b5ef19"
...
CONTRACT = load("audit_contract_v24", HERE / "audit_contract_v24.py")
...
def evaluate(current=None, receipt=None, accounting=None):
    ...
    need(hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA, "V23 baseline drift")
    before = json.loads(BASE.read_text())
    if current is None:
        current = json.loads((HERE / "SCOPE_SNAPSHOT_V24.json").read_text())
    if receipt is None:
        receipt = json.loads((ROOT / CONTRACT.PACKAGE / "RESULT_V24.json").read_text())
    need(type(current) is dict and set(current) == set(before), "snapshot top-level fields")
    for field in before:
        if field != "rounds":
            expected = "6da0f19202efaae848328327b70b14cce0049842" if field in ("observed_main", "audited_base") else before[field]
    ...
    need(result["registered_atoms"] == 222 and result["unresolved_atoms"] == 192,
    ...
    need(len(scientific) == 22, "scientific closure count drift")
    expected_accounting = CONTRACT.current_accounting(current, receipt["amendment_carry"])
    if accounting is None:
        accounting = json.loads((HERE / "CURRENT_ACCOUNTING_V24.json").read_text())
    exact(accounting, expected_accounting, "current accounting drift")
    for key, value in {"original_total": 222, "original_fulfilled": 30, "original_unresolved": 192,
                       "qualified_replacements": 2, "active_unresolved": 190}.items():

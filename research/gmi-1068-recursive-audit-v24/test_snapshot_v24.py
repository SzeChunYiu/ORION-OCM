...
spec=importlib.util.spec_from_file_location('checked_snapshot_v24',HERE/'check_snapshot_v24.py')
...
class SnapshotTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.snapshot=json.loads((HERE/'SCOPE_SNAPSHOT_V24.json').read_text())
        cls.receipt=json.loads((checker.ROOT/checker.CONTRACT.PACKAGE/'RESULT_V24.json').read_text())
    ...
    def test_actual_snapshot_no_alarm(self):
        ...
        self.assertEqual(len(result['scientific_atoms_closed']),22)
        self.assertEqual((result['original_fulfilled'],result['remaining_atoms'],
                          result['qualified_replacements'],result['active_unresolved']),(30,192,2,190))
    ...
    def test_rehashed_snapshot_mutations(self):
        ...
        wrong=checker.CONTRACT.witness(checker.ROOT,'oracle_v24.py','python_symbol','profile')
        ...
        bad=deepcopy(self.snapshot);bad['rounds']['R3']['artifacts']=[a for a in bad['rounds']['R3']['artifacts']
            if a['path']!=checker.CONTRACT.PACKAGE+'/FREEZE_V24.md'];invalid.append(bad)
        bad=deepcopy(self.snapshot);path=HERE/'audit_contract_v24.py'
    ...
    def test_receipt_mutations(self):
        ...
        coverage_key=next(iter(self.receipt['coverage']['test_profiles_v24']))
        ...
        changes=[(('atoms',),{'GMI2-R1-001':{'status':'VERIFIED_AT_REGISTERED_SCOPE'}}),
            (('registered_results','X1','scope'),'all action words uniquely identify edge paths'),
            (('registered_results','X2','status'),'PENDING_REVIEW'),
            ...
            (('coverage','test_profiles_v24',coverage_key),True),
            (('coverage','test_profiles_v24',coverage_key),float(self.receipt['coverage']['test_profiles_v24'][coverage_key])),
        ...
        for path in ((),('kernel',),('source_bindings',),('coverage','test_profiles_v24'),('inputs',),('registered_results',)):
        ...
        self.assertEqual(len(invalid),64)
    ...
    def test_coupled_result_scope_and_original_promotion(self):
        ...
        for result in checker.CONTRACT.RESULTS:
            ...
            claim='Every old action word has a unique edge-path decoder.'
        ...
        for atom_id in checker.CONTRACT.ATOM_SPECS:
            ...
            atom['disposition']='All historical Gamma definitions are identical without source premises.'
    ...
    def test_current_accounting_mutations(self):
        ...
        for path,value in [(('original_fulfilled',),31),(('original_unresolved',),191),
                (('active_unresolved',),189),(('qualified_replacements',),3),(('original_total',),222.0),
        ...
        invalid.append(json.loads((checker.BASE.parent/'CURRENT_ACCOUNTING_V23.json').read_text()))

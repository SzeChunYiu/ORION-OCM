...
import check_lean_v26 as kernel
    ...
    def test_registered_statement_types_reject_source_valid_mutants(self):
        ...
        changes = [
            [("ProofTargetsV26.lean",
              r"theorem raw_transport_contract[\s\S]*?(?=\ntheorem restricted_history_contract)",
              "theorem raw_transport_contract : True := True.intro\n")],
            [("ProofTargetsV26.lean",
              r"theorem restricted_history_contract[\s\S]*?(?=\ntheorem resource_lift_contract)",
              "theorem restricted_history_contract : True := True.intro\n")],
            [("ProofTargetsV26.lean",
              r"theorem resource_lift_contract[\s\S]*?(?=\nend ProofTargetsV26)",
              "theorem resource_lift_contract : True := True.intro\n")],
        ]
        ...
        for index, mutation in enumerate(mutations):
            with self.subTest(mutation=index), tempfile.TemporaryDirectory(prefix="v26-kernel-hostile-") as directory:
                root = Path(directory)

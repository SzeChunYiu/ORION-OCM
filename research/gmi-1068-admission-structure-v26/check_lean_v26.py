...
from proof_contract_v26 import ENTRIES, SOURCE_NAMES, SOURCE_PATHS, audit_source
...
def evaluate(source_overrides=None, require_source_compile=False):
    ...
    audit = audit_source()
    try:
        with tempfile.TemporaryDirectory(prefix="gmi-v26-lean-") as output:
            ...
            audit_path = Path(output) / "RegisteredProofsV26.lean"
    ...
    return {"status": "PASS", "lean_version": "4.19.0", "sources": bindings,
            ...
            "source_assumptions": "actual V5 restriction and resource categories through V11 adapters; actual functor identity/composition preservation; object injection exactly characterizes all raw-tree mapped Option transport; cost identities zero and additive Nat costs; same-path resource lift and exact residual; locally discrete equality2cells; optional V13/V14 structures retain their original premises and data"}

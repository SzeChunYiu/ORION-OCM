...
def build() -> dict:
    ...
    return {
        ...
        "the_one_complete_transition_lost_to_a_parent": (
            ...
            "X3_PRICE_FREE_V1 then removed the exchange rate altogether, reported raw "
            "counts, and found that under the one price this lane had chosen the lineage "
            "LOSES to replay at four of six settings -- which put the carry claim itself in "
            "doubt. X4_FINAL_ACCOUNTING_V1 resolved that doubt by locating the confound: "
            "every arm X3 measured held the full 433-predicate language, because it "
            "inherited DEV-6's arms, and DEV-6 had run the SINGLETON rule -- DEV-3's "
            "mechanism, correctly priced at 1 -- over THAT language rather than over the "
            "small one DEV-3 used. A version space that large rarely collapses, so the rule "
            "rarely fires and the arm verifies on nearly every demand; that is a fact about "
            "language size, not about guards. Running the same rule, the same charge, the "
            "same worlds and the same budgets over BOTH languages, the small-language "
            "guarded arm beats replay at four of eight level-1 settings at correctness 1.0, "
            "best ratio 0.377, and the full-language arm beats it at none of the eight. "
            "X3's numbers stand and are not withdrawn; its reading was too broad and is "
            "corrected. What survives every audit this lane has run is therefore ONE "
            "conditional carry advantage, and X4 found its condition is a conjunction of "
            "two rather than one. DEV-4's language precondition is the first. The second "
            "was not predicted: the advantage appears only at the LARGER of the two swept "
            "budgets and reverses at the smaller, so a budget able to hold the rules, their "
            "guards AND working facts is a precondition too. X4 also refuted its own Z5. At "
            "level 3, where the small language is unsound and only the full-language arms "
            "are admissible, the UNANIMITY arm beats replay at three of eight settings, and "
            "every one of them is at the SMALLER budget under UNIFORM demand, where the "
            "replay parent is most starved. The two survivors therefore have opposite "
            "budget dependence, which this lane had no theory for. "
            "X5_BUDGET_CROSSING_V1 answered half of that and refuted three of its own six "
            "registered predictions doing it. Nine budgets rather than two, seven of them "
            "out of sample. The unanimity arm at level 3 under uniform demand is a REGIME: "
            "one clean crossing, winning at every budget up to 1408 and losing at every "
            "budget from 1536 up, because its cost barely moves with the budget while the "
            "replay parent's falls by a factor of sixteen. Under skewed demand it never "
            "wins at all, so X4's level-3 positive is a uniform-demand result and is "
            "restated as one. The guarded arm is NOT a regime in the shape predicted: its "
            "margin changes sign three times, and the reason is visible in the counters "
            "rather than mysterious. Both cost curves fall with the budget and neither "
            "falls at a constant rate -- the arm holds only five or six of its sixteen "
            "guards until 1280 bits, so its work is nearly flat there, then the held count "
            "rises to 14.25 and 16 and its work drops from 112,182 to 22,029, while the "
            "parent improves smoothly throughout and then memoises the entire answer space. "
            "A difference of two such curves can cross more than once and does. The "
            "consequence is a correction to DEV-3's computed window, and it cuts both ways. "
            "DEV-3's UPPER edge, rule_count * extension * FACT_BITS = 2048 bits, was "
            "computed from the cost constants before any of these budgets existed and HOLDS "
            "out of sample: at 2048 the parent stores all 256 answers and the arm loses at "
            "both demand shapes. DEV-3's LOWER edge, 768 bits, is necessary and NOT "
            "sufficient: the arm loses at six budgets inside its own window. The measured "
            "carry regime, the contiguous run the arm wins at BOTH demand shapes, is "
            "[1408, 1536] against a computed window of [768, 2048] -- about a quarter of "
            "the width the lane thought it had. X5 also imported the independent review of "
            "PR #150 published in PR #153, which shows by counterexample that a calibrated "
            "posterior's perplexity does not bound expected guess cost, and tested the "
            "analogous claim here: the SAME language produces both signs of the carry "
            "advantage, so no function of version-space size predicts the sign, while the "
            "margin rebuilt from the priced events alone reproduces the measured work "
            "difference to the integer in all thirty-six cells. Size is not the operative "
            "quantity in this lane either; the charge is. "
            "X6_COMPILED_CONSULTATION_V1 then built the arm that decides between the two "
            "stories, which this lane had argued about from three directions and never "
            "tested. It compiles the unanimity verdict into a per-rule decision table, so "
            "the version space stays at 433 predicates and a consultation costs 1 instead "
            "of what it scans. The size story predicts this changes nothing, because the "
            "space is still large. The charge story predicts it recovers the advantage. A "
            "per-replicate control confirmed first that compilation changed only the price: "
            "the compiled arm and DEV-5's naive rule agree on correctness and on every "
            "non-deliberation counter in all thirty-six settings, with zero disagreements, "
            "so the comparison is between two prices for one rule and not between two "
            "rules. All five registered predictions held. The compiled arm beats the replay "
            "parent at 21 settings against the naive rule's 8, gaining 13 and LOSING NONE, "
            "and it wins at six budgets at level 3 under SKEWED demand -- the exact "
            "condition where X5 measured the naive rule losing at all nine -- with a best "
            "ratio of 0.512. So the carry advantage no longer requires a language small "
            "enough to collapse. It still requires one that CONTAINS the truth, which is "
            "soundness and is not negotiable, but the ECONOMY precondition DEV-4 imposed and "
            "DEV-5 tried and failed to remove at an honest price is removed here, by pricing "
            "rather than by choosing the language. This lane's carry advantage is therefore "
            "conditional on a property of the machine -- how much a consultation costs -- "
            "rather than on a property of the world it was handed. One prediction of this "
            "lane's own was refuted in the process: synthesis.py had predicted, about the "
            "native lane, that compiling a whole bank buys nothing and only demand-driven "
            "preparation pays. In this analogue eager compilation buys almost everything, "
            "20 settings against demand-driven's 21, because the extension is 16 and the "
            "demanded indices already cover about 14 of them. The published prediction is "
            "left as published and the refutation is recorded beside it. "
            "X7_PRICED_TABLE_V1 then audited X6 by this lane's own accounting, because X6's "
            "table was charged 1 per consultation and NOTHING to hold, while DEV-3's guard "
            "paid RULE_BITS + GUARD_BITS out of the same budget as the facts and DEV-3's "
            "entire result was a window computed from that charge. A cell holds a ternary "
            "verdict, so it is charged 2 bits from the same budget; under pressure the arm "
            "discards whole tables, least recently consulted first, and pays the scan "
            "again. Two controls first. At a price of ZERO the compiled arms reproduce X6's "
            "grid cell for cell on X6's own seeds, so the bit machinery changed the price "
            "and nothing else. And the naive rule and the replay parent, which hold no "
            "table, win exactly 6 of 18 settings at EVERY price. Terminal "
            "CARRY_ADVANTAGE_SURVIVES_A_PRICED_TABLE. At the honest 2 bits the "
            "demand-driven arm wins 13 of 18, and at level 3 under SKEWED demand it wins 5 "
            "of 9 with a best ratio of 0.764 where the naive rule wins none. The advantage "
            "survives an accounting consistent with DEV-3's. It also SEPARATES the two "
            "compiled arms X6 found nearly indistinguishable: charge for storage and eager "
            "compilation collapses from 13 winning settings to 2 while demand-driven falls "
            "only from 14 to 13, and the recompilation counts say why -- an eager arm whose "
            "table almost fits evicts a neighbour, is evicted in turn, and pays the full "
            "compile again, 1,332 times at 8 bits against the demand-driven arm's 556. That "
            "is a property of the whole-table discard policy this lane chose and declared, "
            "and the counts are published so the difference between a policy and a "
            "mechanism is visible rather than argued. One prediction was refuted and it is "
            "the informative one. V5 predicted the advantage would die below 8 bits a cell. "
            "It does not: it still wins 8 of 18 there, and 8 bits is exactly FACT_BITS, "
            "what this lane charges to store one answer. A compiled verdict that costs as "
            "much as a cached answer and still beats a parent made entirely of cached "
            "answers means the two are not comparable units -- a cell decides an index for "
            "a whole rule and licenses an application at APPLY_COST where the parent pays "
            "DERIVE_COST, on every demand for that index, while a stored answer serves only "
            "itself. It also means X7's highest surviving price is the TOP OF X7'S SWEEP "
            "and therefore a lower bound rather than a boundary. The receipt says so in its "
            "own field rather than leaving the number to be misread, and "
            "X8_TABLE_BREAK_EVEN_V1 continued the same sweep upward at 8, 16, 32, 64 and "
            "128 bits a cell. Its overlap control at 8 reproduced X7 cell for cell, the "
            "tableless arms stayed at 6 of 18 at every price, and the response stayed "
            "monotone. Two of its six predictions failed and both are worth more than they "
            "cost. U5 predicted the end would be refusal rather than churn: refusals do "
            "rise, 45 to 422, but discards rise WITH them, 546 to 1,457, because at two "
            "and a half cells held a new cell can only be admitted by evicting another "
            "table. The end is refusal AND churn. U1 asked for a price at which the "
            "compiled arm beats replay at zero settings, which cannot happen and is a "
            "defect in the prediction rather than a fact about the arm: as the price rises "
            "the arm degrades INTO the rule it compiles -- U4 confirms it, within 1.6 "
            "percent at every budget at 128 bits -- and that rule beats replay at 6 of 18 "
            "settings on its own at every price. So the floor is 6 and not 0, and the "
            "quantity meant was the EXCESS over the rule being compiled: 2 settings at 8 "
            "bits, 1 at 16, and 0 from 32 upward. The correctly specified break-even is 32 "
            "bits a cell -- FOUR ANSWERS' worth of storage for one compiled verdict. The "
            "published terminal stands as the registered criterion produced it and the "
            "correction is recorded beside it in x8.CRITERION_CORRECTION rather than "
            "substituted for it. This lane's one carry advantage therefore now has all "
            "three of its edges located and none of them assumed: DEV-4's soundness "
            "precondition on the language, X5's budget window [1408, 1536] bits, and a "
            "storage ceiling of 32 bits per compiled verdict."),

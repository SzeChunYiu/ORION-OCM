"""Successor emit: declared 2- or 3-class parameters and 0-ary P1 syntax applies."""
...
def substitute(tokens, mapping):
    return [v for t in tokens for v in mapping.get(t, [t])]


def emit(body, contracts, context, hypotheses, work):
    rows = TC.validate(context)
    expected = TC.parameters(rows[0]["type"], len(rows))
    ...
    require(type(hypotheses) is list and all(type(h) is dict and set(h) == {"label", "statement"} for h in hypotheses),
            "hole shape")
    ...
    require(body["schema"] == "native.typed-proper-chunk.v1" and body["parameters"] == expected and
            set(body) == {"schema", "parameters", "premises", "query", "nodes", "root"},
            "body schema/parameters")
    nodes = body["nodes"]
    require(type(nodes) is list and 0 < len(nodes) <= 256 and type(body["root"]) is int and
            body["root"] == len(nodes) - 1, "body root/population")
    ...
    for i, n in enumerate(nodes):
        ...
        if n["kind"] == "float":
            require(set(n) == {"kind", "output"} and n["output"] in [[r["type"], r["id"]] for r in rows],
                    "float body")
        elif n["kind"] == "hole":
            require(set(n) == {"kind", "slot", "output"} and type(n["slot"]) is int and
                    0 <= n["slot"] < len(holes) and n["output"] == body["premises"][n["slot"]],
                    "hole body")
        else:
            require(set(n) == {"kind", "label", "inputs", "output", "substitution"} and n["kind"] == "apply",
                    "application shape")
            ...
            require(len(inputs) == len(hs) and all(type(j) is int and 0 <= j < i for j in inputs),
                    "body dependency")
            ...
            require(all(outputs[k] == substitute(h["statement"], mapping) for k, h in zip(inputs, hs)),
                    "typed/essential body")
    ...
    require(used == declared, "complete declared-parameter body")
    proof = []
    reached = set()
    semantic = 0
    ...
    def visit(i):
        ...
        reached.add(i)
        n = nodes[i]
        count(work, "expanded_node_visits")
        if n["kind"] == "float":
            label = float_labels[n["output"][1]]
        elif n["kind"] == "hole":
            label = holes[n["slot"]]
        else:
            for j in n["inputs"]:
            ...
            if n["output"][0] == "|-":
                semantic += 1
        require(len(proof) < 4096, "expanded proof bound")
        proof.append(label)
    visit(body["root"])
    require(reached == set(range(len(nodes))), "unreachable body node")
    return {"proof": proof, "target": native(body["query"]), "hypotheses": hypotheses,
            "semantic_applications_expanded": semantic}

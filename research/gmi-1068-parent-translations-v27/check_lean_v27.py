...
from proof_contract_v27 import ENTRIES, SOURCE_NAMES, SOURCE_PATHS, audit_source
...
def evaluate(source_overrides=None, require_source_compile=False):
    ...
    audit = audit_source()
    try:
        with tempfile.TemporaryDirectory(prefix="gmi-v27-lean-") as output:
            ...
            audit_path = Path(output) / "RegisteredProofsV27.lean"
    ...
    return {"status": "PASS", "lean_version": "4.19.0", "sources": bindings,
            ...
            "source_assumptions": "actual relation-LTS powerset and path bridges; Set endofunctor laws derive system category; actual V14 Weight with nontrivial zero-sum-free and no-zero-divisor laws plus commutative multiplication and nonnegative monotone partial order; actual matrix Events and normalized labelled Tests; declared subtype task interfaces with explicit inclusion; possibility closure supplied; physical and general measurable/quantum scope remains separate"}

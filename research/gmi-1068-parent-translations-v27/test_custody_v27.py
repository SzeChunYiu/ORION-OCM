...
import custody_v27 as core
COVERAGE={}


@contextmanager
def overlay(verified):
    with tempfile.TemporaryDirectory(prefix='gmi-v27-custody-') as temporary:
        root=Path(temporary)
...
class CustodyTests(unittest.TestCase):
    def test_real_inherited_inputs_and_tampering(self):
        ...
        files=('research/gmi-1068-typed-foundation-v11/paths_v11.py',
               'research/gmi-1068-stochastic-process-v14/stochastic_v14.py',
               'research/gmi-1068-admission-structure-v26/functors_v26.py',
               'research/gmi-1068-admission-structure-v26/RawTransportV26.lean',
               'research/gmi-1068-recursive-audit-v26/SCOPE_SNAPSHOT_V26.json',
               'research/gmi-833-aj1-operational-process-base-v1/PARENT_COMPARISON.json',
               'research/gmi-1068-amendment-governance-v16/AMENDMENT_LEDGER_V16.json',
               *core.RECEIPTS,core.PACKAGE+'/FREEZE_V27.md',core.PACKAGE+'/SOURCE_PINS_V27.json')
        rejected=0
        for name in files:
            with overlay(valid) as root:
                ...
                rejected+=1
        with overlay(valid) as root:
            target=root/'research/gmi-1068-admission-structure-v26/functors_v26.py'
            target.write_bytes(target.read_bytes()+b'\n')
            path=root/core.RECEIPTS[0]
            receipt=json.loads(path.read_text())
            receipt['inputs']['functors_v26.py']=core.digest(target)
        ...
        with overlay(valid) as root:
            ...
            with self.assertRaises(ValueError):core.verify(root)
            rejected+=1
        self.assertEqual(rejected,len(files)+2)
    ...
    def test_unavailable_inputs_and_path_escape(self):
        valid=core.verify(ROOT)
        missing=0
        for name in ('research/gmi-1068-admission-structure-v26/functors_v26.py',
                     'research/gmi-1068-partial-context-v15/context_v15.py',
                     core.PACKAGE+'/FREEZE_V27.md',core.PACKAGE+'/SOURCE_PINS_V27.json',core.RECEIPTS[2]):
            with overlay(valid) as root:
                (root/name).unlink()

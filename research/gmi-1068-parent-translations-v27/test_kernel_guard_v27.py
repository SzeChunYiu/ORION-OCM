...
import check_lean_v27 as kernel
    ...
    def test_registered_statement_types_reject_source_valid_mutants(self):
        ...
        changes = [
            [("ProofTargetsV27.lean",
              r"theorem hom_equation_contract[\s\S]*?(?=\ntheorem test_composition_contract)",
              "theorem hom_equation_contract : True := True.intro\n")],
            [("ProofTargetsV27.lean",
              r"theorem test_composition_contract[\s\S]*?(?=\ntheorem regular_task_contract)",
              "theorem test_composition_contract : True := True.intro\n")],
            [("ProofTargetsV27.lean",
              r"theorem regular_task_contract[\s\S]*?(?=\nend ProofTargetsV27)",
              "theorem regular_task_contract : True := True.intro\n")],
        ]
        ...
        for index, mutation in enumerate(mutations):
            with self.subTest(mutation=index), tempfile.TemporaryDirectory(prefix="v27-kernel-hostile-") as directory:
                root = Path(directory)

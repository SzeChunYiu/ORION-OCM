...
def main():
    ...
    argv=[str(PYTHON),"-I","-S","-B",str(ROOT/"check_caller.py"),str(output/"CONTROLS.json")]
    ...
    allowed={str(ROOT/n) for n in ("check_caller.py","authored_fixture.py","caller_bindings.py","caller_flow.py","run_native.py")}
    allowed|={str(ROOT/"vendor"/n) for n in ("typed_context.py","typed_emit.py","typed_constructor.py","structural_fixture.py","hole_match.py","replacement.py","life_native.py","trace_source.py")}
    ...
    passed=(pid==child.pid and child.returncode==0 and matched and imports_ok and not forbidden
        and before==after and controls["passed"] and controls["tests_run"]==6 and controls["native_calls"]==0)

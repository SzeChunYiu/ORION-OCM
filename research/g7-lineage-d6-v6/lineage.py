"""G7 persistent developmental lineage — D6 successor vessel.

Continues lineage id orion-ocm-g7-lineage-v1:microscope-d0-d1 through a seventh
earned DevelopmentTransitionV1 (OCM_5 → OCM_6). D6 competence is a tiny governed
self-evolution microscope: propose a plant repair, shadow-eval it on a clone,
and require external constitution C to adopt before live replace and restart.
Not historical M11 g0→g1→g2→g2 relabel, not constitution mutation, not Metamath,
not FLT, not neural. Serialises the whole earned bundle, restarts by loading it,
and refuses a tampered digest. Reset stores cannot read continued-arm files.
Constitution C is frozen.
"""
...
D5_GRAMMAR_ID = "g7.method-selection-metacognition.v1"
...
D5_METHOD_LEFT = "TRY_LEFT"
D5_METHOD_RIGHT = "TRY_RIGHT"
D5_ALREADY_EARNED = (D5_METHOD_LEFT, D5_METHOD_RIGHT)
D5_CUES = ("L", "R")
D5_PAYLOAD_ALPHABET = ("W", "X", "Y")
D5_PAYLOAD_LEN = 3
D5_MATCH_COST = 1
D5_MISMATCH_COST = 4
D5_FROZEN_POLICY_NAMES = frozenset({"NEURAL", "SGD", "BACKPROP", "LLM"})
D5_POLICY_FIXED_LEFT = "FIXED_LEFT_FIRST"
D5_POLICY_FIXED_RIGHT = "FIXED_RIGHT_FIRST"
D5_POLICY_TABLE = "VALIDATION_UTILITY_TABLE"
D6_GRAMMAR_ID = "g7.governed-plant-self-evolution.v1"
D6_CELLS = 8
D6_STUCK_COUNT = 2
D6_SYNDROME_WINDOWS = tuple(tuple((index + offset) % D6_CELLS for offset in range(3)) for index in range(D6_CELLS))
D6_SHADOW_COST = 1
D6_ADOPT_COST = 1
D6_PROPOSE_COST = 1
D6_REPLACE_COST = 4
D6_FROZEN_POLICY_NAMES = frozenset({
    "M11", "JUMP", "MUTATE_C", "G0", "G1", "G2", "AUTOML", "NEURAL",
})
D6_POLICY_REPLACE_ALL = "REPLACE_ALL_CELLS"
D6_POLICY_WITHOUT_C = "PROPOSE_WITHOUT_C_ADOPTION"
D6_POLICY_TABLE = "SHADOW_EVAL_C_ADOPTED_REPAIR"
PERSIST_SLOTS = (
    "field_state",
    "learned_methods",
    "method_schemas",
        ...
        "executive_metareasoning_policy": {
            ...
            "payload": {
                "rule": (
                    ...
                    "serve D5 utility-table first-method selection on the "
                    "metacognition family; serve D6 shadow-eval plant repairs "
                    "only after external C adopts; do not rewrite C"
                ),
...
def current_selection_policy(bundle: dict):
    for method in bundle.get("learned_methods") or []:
        if method.get("kind") == "selection_policy":
            return method
    return None


def current_evolution_policy(bundle: dict):
    for method in bundle.get("learned_methods") or []:
        if method.get("kind") == "evolution_policy":
            return method
    return None
...
def d5_taught_donors():
    return [
        {
            "identity": "donor:already-earned:TRY_LEFT",
            "kind": "primitive",
            "origin_category": ORIGIN_TAUGHT,
            "payload": {
                "operator": D5_METHOD_LEFT,
                "grammar": D5_GRAMMAR_ID,
                "role": "already-earned-method",
                "already_earned": True,
            },
        },
        {
            "identity": "donor:already-earned:TRY_RIGHT",
            "kind": "primitive",
            "origin_category": ORIGIN_TAUGHT,
            "payload": {
                "operator": D5_METHOD_RIGHT,
                "grammar": D5_GRAMMAR_ID,
                "role": "already-earned-method",
                "already_earned": True,
            },
...
def meta_task_id(cue, payload) -> str:
    return content_hash({
        "domain": D5_GRAMMAR_ID,
        "cue": cue,
        "payload": list(payload),
    })


def cheap_method_for_cue(cue: str) -> str:
    if cue == "L":
        return D5_METHOD_LEFT
    if cue == "R":
        return D5_METHOD_RIGHT
    raise ValueError(f"unknown D5 cue {cue}")


def meta_population():
    """Cue-conditioned first-method tasks. Not neural, not Metamath, not FLT."""
    out = []
    for cue in D5_CUES:
        for payload in product(D5_PAYLOAD_ALPHABET, repeat=D5_PAYLOAD_LEN):
            out.append({
                "fingerprint": meta_task_id(cue, payload),
                "cue": cue,
                "payload": payload,
                "cheap_method": cheap_method_for_cue(cue),
                "domain": D5_GRAMMAR_ID,
            })
    return tuple(out)


def take_meta(population, salt: str, n: int, exclude=()):
    excluded = set(exclude)
    pool = [task for task in population if task["fingerprint"] not in excluded]
    chosen = tuple(sorted(
        pool,
    ...
    if len(chosen) != n:
        raise RuntimeError(f"metacognition stratum too small: {len(chosen)} < {n}")
    return chosen


def try_earned_method(task, method_name: str):
    if method_name not in D5_ALREADY_EARNED:
        raise RuntimeError(f"refusing unknown D5 method {method_name}")
    if method_name in D5_FROZEN_POLICY_NAMES:
        raise RuntimeError("refusing frozen neural identity as a D5 method")
    cheap = task["cheap_method"]
    if method_name == cheap:
        return D5_MATCH_COST, True
    return D5_MISMATCH_COST, False


def other_earned_method(method_name: str) -> str:
    if method_name == D5_METHOD_LEFT:
        return D5_METHOD_RIGHT
    if method_name == D5_METHOD_RIGHT:
        return D5_METHOD_LEFT
    raise ValueError(f"unknown D5 method {method_name}")


def first_method_for_policy(task, policy) -> str:
    kind = policy["kind"]
    if kind == "fixed":
        return policy["first"]
    if kind == "table":
        ...
        table = policy["table"]
        if cue not in table:
            raise RuntimeError(f"D5 utility table missing cue {cue}")
        return table[cue]
    raise ValueError(f"unknown D5 policy kind {kind}")


def fixed_policy(name: str) -> dict:
    if name == D5_POLICY_FIXED_LEFT:
        return {"kind": "fixed", "name": name, "first": D5_METHOD_LEFT}
    if name == D5_POLICY_FIXED_RIGHT:
        return {"kind": "fixed", "name": name, "first": D5_METHOD_RIGHT}
    raise ValueError(f"unknown fixed D5 policy {name}")


def table_policy(table: dict, policy_id: str) -> dict:
    return {
        "kind": "table",
        "name": D5_POLICY_TABLE,
        "policy_id": policy_id,
        "table": dict(table),
    }


def serve_meta(task, policy):
    first = first_method_for_policy(task, policy)
    if first in D5_FROZEN_POLICY_NAMES:
        raise RuntimeError("refusing frozen NEURAL/SGD/BACKPROP/LLM first-method")
    cost_first, ok = try_earned_method(task, first)
    token_word = [f"CUE:{task['cue']}", f"TRY_FIRST:{first}"]
    if ok:
        return {
            "status": "FOUND",
            "token_word": token_word,
            "program": token_word,
            "enumeration_attempts": cost_first,
            "unique_candidates_checked": 1,
            "verified": True,
            "policy_used": policy["kind"] == "table",
            "first_method": first,
            "methods_tried": [first],
        }
    second = other_earned_method(first)
    cost_second, ok2 = try_earned_method(task, second)
    token_word = token_word + [f"TRY_SECOND:{second}"]
    verified = bool(ok2)
    return {
        "status": "FOUND" if verified else "NOT_FOUND",
        "token_word": token_word,
        "program": token_word,
        "enumeration_attempts": cost_first + cost_second,
        "unique_candidates_checked": 2,
        "verified": verified,
        "policy_used": policy["kind"] == "table",
        "first_method": first,
        "methods_tried": [first, second],
    }


def evaluate_meta(tasks, policy):
    rows = []
    total = 0
    checks = 0
    used = 0
    ...
    for task in tasks:
        ...
        if row["policy_used"]:
            used += 1
        if row["first_method"] == task["cheap_method"]:
            first_correct += 1
        rows.append(row)
        total += row["enumeration_attempts"]
        checks += row["unique_candidates_checked"]
    return rows, total, checks, used, first_correct


def discover_utility_table(train_tasks, origin: str):
    """Build a cue → first-method table from train utility. Never NEURAL/SGD."""
    per_cue = {cue: {D5_METHOD_LEFT: 0, D5_METHOD_RIGHT: 0} for cue in D5_CUES}
    discovery_cost = 0
    attempts = []
    for task in train_tasks:
        cue = task["cue"]
        left_cost, _ = try_earned_method(task, D5_METHOD_LEFT)
        right_cost, _ = try_earned_method(task, D5_METHOD_RIGHT)
        per_cue[cue][D5_METHOD_LEFT] += left_cost
        per_cue[cue][D5_METHOD_RIGHT] += right_cost
        discovery_cost += left_cost + right_cost
        attempts.append({
            "task": task["fingerprint"],
            "cue": cue,
            "left_cost": left_cost,
    ...
    table = {}
    for cue in D5_CUES:
        left = per_cue[cue][D5_METHOD_LEFT]
        right = per_cue[cue][D5_METHOD_RIGHT]
        if left == 0 and right == 0:
            raise RuntimeError(f"D5 train missing cue {cue}")
        table[cue] = D5_METHOD_LEFT if left <= right else D5_METHOD_RIGHT
        if table[cue] in D5_FROZEN_POLICY_NAMES:
            raise RuntimeError("D5 invented a frozen neural first-method")
    policy_id = "SEL_" + content_hash({
        "table": table,
        "family": D5_GRAMMAR_ID,
        "algorithm": D5_POLICY_TABLE,
    })[:12]
    if policy_id in D5_FROZEN_POLICY_NAMES:
        raise RuntimeError("D5 invented a frozen NEURAL/SGD name")
    if not policy_id.startswith("SEL_"):
        raise RuntimeError("D5 invented identity is not a SEL_* selection policy")
    return {
        "policy": table_policy(table, policy_id),
        "table": table,
        "policy_id": policy_id,
        "origin": origin,
        "discovery_cost": discovery_cost,
...
def d5_tournament(validation_tasks, policy):
    left_rows, left_total, left_checks, left_used, left_correct = evaluate_meta(
        validation_tasks, fixed_policy(D5_POLICY_FIXED_LEFT)
    )
    right_rows, right_total, right_checks, right_used, right_correct = evaluate_meta(
        validation_tasks, fixed_policy(D5_POLICY_FIXED_RIGHT)
    )
    table_rows, table_total, table_checks, table_used, table_correct = evaluate_meta(
        validation_tasks, policy
    )
    table_found = all(row["status"] == "FOUND" and row["verified"] for row in table_rows)
    accepted = (
        table_found
        and policy["kind"] == "table"
        and policy["policy_id"] not in D5_FROZEN_POLICY_NAMES
        and not policy["policy_id"].startswith("RW_")
        and not policy["policy_id"].startswith("CUT_")
        and table_used == len(validation_tasks)
        and table_total < left_total
        and table_total < right_total
        and table_correct == len(validation_tasks)
    )
    evaluations = [
        {
            "name": D5_POLICY_FIXED_LEFT,
            "aggregate_cost": left_total,
            "first_correct": left_correct,
            "policy_uses": left_used,
        },
        {
            "name": D5_POLICY_FIXED_RIGHT,
            "aggregate_cost": right_total,
            "first_correct": right_correct,
            "policy_uses": right_used,
        },
        {
            "name": D5_POLICY_TABLE,
            "aggregate_cost": table_total,
            "first_correct": table_correct,
            "policy_uses": table_used,
        },
    ]
    return {
        "accepted": accepted,
        "selected": evaluations[2] if accepted else None,
        "baseline": evaluations[0],
        "evaluations": evaluations,
        "all_candidate_validation_cost": left_total + right_total + table_total,
...
def selection_policy_record(policy, origin: str) -> dict:
    if policy["policy_id"] in D5_FROZEN_POLICY_NAMES:
        raise RuntimeError("refusing to persist NEURAL/SGD/BACKPROP/LLM as D5 competence")
    identity = "method:" + content_hash({
        "kind": "selection-policy.g7.d5.v1",
    ...
    return {
        ...
        "payload": {
            ...
            "family": D5_GRAMMAR_ID,
            "already_earned_method_ids": [
                "donor:already-earned:TRY_LEFT",
                "donor:already-earned:TRY_RIGHT",
            ],
            "already_earned_methods": list(D5_ALREADY_EARNED),
            "algorithm": "argmin-validation-utility-per-cue",
            "frozen_names_refused": sorted(D5_FROZEN_POLICY_NAMES),
            "not_neural": True,
            "not_metamath": True,
            "not_flt": True,
            "not_m11_relabel": True,
...
def admit_selection_policy(bundle: dict, policy, origin: str, stage: str, change_note: str):
    bundle = json.loads(canonical_json(bundle))
    assert_constitution_frozen(bundle)
    method = selection_policy_record(policy, origin)
    schema = {
        "identity": "schema:" + content_hash({"method": method["identity"], "kind": "selection-policy"}),
        "kind": "schema",
        "origin_category": origin,
        "payload": {
            "pattern": (
                "from a tiny per-cue validation utility table, select which of two "
                "already-earned methods to try first; not neural, not REGEX/EVAL, "
                "not a Hilbert CUT_*, then finish held-out cue-conditioned tasks"
            ),
            "originating_method_ids": [method["identity"]],
            "family": D5_GRAMMAR_ID,
        },
    ...
    if method["identity"] not in existing:
        ...
        bundle["field_state"]["atoms"].append({
            "id": method["identity"],
            "type": "procedure",
            "content": method["payload"]["policy_id"],
        })
        bundle["self_model"]["known_competence"].append(method["identity"])
        existing_donors = {donor["identity"] for donor in bundle["imported_donors"]}
        for donor in d5_taught_donors():
            if donor["identity"] not in existing_donors:
                bundle["imported_donors"].append(donor)
        existing_reps = {rep["identity"] for rep in bundle["representations"]}
        if "rep:method-selection-utility-table.v1" not in existing_reps:
            bundle["representations"].append({
                "identity": "rep:method-selection-utility-table.v1",
                "origin_category": ORIGIN_TAUGHT,
                "payload": {
                    "kind": "per-cue-argmin-utility-table",
                    "grammar": D5_GRAMMAR_ID,
                    "not_neural": True,
                    "not_metamath": True,
                },
    ...
    bundle["self_model"]["unknown"] = [
        item for item in bundle["self_model"]["unknown"]
        if item not in {"OCM_2", "OCM_3", "OCM_4", "OCM_5"}
    ]
    bundle["self_change_history"].append({
        "change": change_note,
        "stage": stage,
...
def d6_taught_donors():
    return [
        {
            "identity": "donor:plant:observe-gauge",
            "kind": "primitive",
            "origin_category": ORIGIN_TAUGHT,
            "payload": {
                "operator": "observe-gauge",
                "grammar": D6_GRAMMAR_ID,
                "role": "observable-plant-signature",
            },
        },
        {
            "identity": "donor:plant:propose-repair",
            "kind": "primitive",
            "origin_category": ORIGIN_TAUGHT,
            "payload": {
                "operator": "propose-repair",
                "grammar": D6_GRAMMAR_ID,
                "role": "repair-proposal",
            },
        },
        {
            "identity": "donor:plant:shadow-eval",
            "kind": "primitive",
            "origin_category": ORIGIN_TAUGHT,
            "payload": {
                "operator": "shadow-eval",
                "grammar": D6_GRAMMAR_ID,
                "role": "clone-quality-check",
            },
        },
        {
            "identity": "donor:plant:external-c-adopt",
            "kind": "primitive",
            "origin_category": ORIGIN_TAUGHT,
            "payload": {
                "operator": "external-c-adopt",
                "grammar": D6_GRAMMAR_ID,
                "role": "constitution-gated-adoption",
                "external": True,
            },
        },
    ]
...
def plant_syndrome(stuck) -> tuple:
    members = set(stuck)
    return tuple(
        sum(1 for index in window if index in members) % 2
        for window in D6_SYNDROME_WINDOWS
    )


def plant_task_id(stuck) -> str:
    return content_hash({
        "domain": D6_GRAMMAR_ID,
        "stuck": [int(x) for x in sorted(stuck)],
    })


def syndrome_key(syndrome) -> str:
    return "".join(str(int(bit)) for bit in syndrome)


class PlantWorld:
    """Tiny governed plant. Hidden contamination is not a training label."""

    def __init__(self, stuck):
        self.stuck = frozenset(int(index) for index in stuck)
        self.cells = [index not in self.stuck for index in range(D6_CELLS)]
        self.constitution = list(CONSTITUTION)

    def quality(self) -> int:
        return int(all(self.cells))

    def replace_cells(self, indices):
        for index in indices:
            if index not in range(D6_CELLS):
                raise ConstitutionMutationError("repair plan attempted to touch C")
            self.cells[index] = True
        return self.quality()

    def restart_clone(self):
        clone = PlantWorld(self.stuck)
        clone.cells = list(self.cells)
        clone.constitution = list(self.constitution)
        return clone


def independent_truth(stuck):
    """Reserved scorer: re-simulate the stuck set. Learner never receives this."""
    return frozenset(int(index) for index in stuck)


def shadow_eval_repair(stuck, proposed):
    proposed = tuple(int(index) for index in proposed)
    for index in proposed:
        if index not in range(D6_CELLS):
            raise ConstitutionMutationError("repair plan attempted to touch C")
    clone = PlantWorld(stuck)
    quality = clone.replace_cells(proposed)
    restarted = clone.restart_clone()
    if restarted.quality() != quality:
        raise RuntimeError("shadow restart disagreed with clone quality")
    if tuple(restarted.constitution) != CONSTITUTION:
        raise ConstitutionMutationError("shadow eval mutated C")
    return {
        "quality": quality,
        "cost": D6_SHADOW_COST,
        "restarted_quality": restarted.quality(),
        "proposed": proposed,
    }


def constitution_adopt(proposal, shadow_quality, constitution):
    """External C adopts a shadow-clean plant repair. C is not a writable slot."""
    if tuple(constitution) != CONSTITUTION:
        raise ConstitutionMutationError("C mutated; adoption fails closed")
    for index in proposal:
        if index not in range(D6_CELLS):
            raise ConstitutionMutationError("C refuses a proposal that touches C")
    if int(shadow_quality) != 1:
        return False
    return True


def plant_population():
    """Exactly-two contaminated cells. Not D2 modules, not Metamath, not M11."""
    out = []
    syndromes = {}
    for combo in combinations(range(D6_CELLS), D6_STUCK_COUNT):
        stuck = tuple(combo)
        syndrome = plant_syndrome(stuck)
        fingerprint = plant_task_id(stuck)
        if syndrome in syndromes and syndromes[syndrome] != stuck:
            raise RuntimeError("D6 plant syndrome is not identifying")
        syndromes[syndrome] = stuck
        out.append({
            "fingerprint": fingerprint,
            "stuck": stuck,
            "syndrome": syndrome,
    ...
    if len(out) != len({task["fingerprint"] for task in out}):
        raise RuntimeError("D6 plant fingerprints collided")
    return tuple(out)


def take_plant(population, salt: str, n: int, exclude=()):
    excluded = set(exclude)
    pool = [task for task in population if task["fingerprint"] not in excluded]
    chosen = tuple(sorted(
        pool,
    ...
    if len(chosen) != n:
        raise RuntimeError(f"plant stratum too small: {len(chosen)} < {n}")
    return chosen


def replace_all_policy() -> dict:
    return {"kind": "replace_all", "name": D6_POLICY_REPLACE_ALL}


def table_evolution_policy(table: dict, policy_id: str, *, c_may_adopt: bool, width: int) -> dict:
    return {
        "kind": "table",
        "name": D6_POLICY_TABLE if c_may_adopt else D6_POLICY_WITHOUT_C,
        "policy_id": policy_id,
        "table": dict(table),
        "width": int(width),
        "c_may_adopt": bool(c_may_adopt),
        "not_task_id_table": True,
    }


def invert_gauge(syndrome, width: int):
    """Propose a repair by matching the observable gauge. Does not read stuck labels."""
    observed = tuple(int(bit) for bit in syndrome)
    attempts = 0
    for combo in combinations(range(D6_CELLS), width):
        attempts += 1
        if plant_syndrome(combo) == observed:
            return tuple(combo), attempts
    raise RuntimeError(f"D6 gauge inversion failed at width {width}")


def serve_plant(task, policy):
    syndrome = tuple(task["syndrome"])
    gauge = f"GAUGE:{syndrome_key(syndrome)}"
    if policy["kind"] == "replace_all":
        proposed = tuple(range(D6_CELLS))
        live = PlantWorld(task["stuck"])
        quality = live.replace_cells(proposed)
        restarted = live.restart_clone()
        verified = quality == 1 and restarted.quality() == 1
        token_word = [gauge, "REPLACE_ALL", "RESTART"]
        return {
            "status": "FOUND" if verified else "NOT_FOUND",
            "token_word": token_word,
            "program": token_word,
            "enumeration_attempts": D6_REPLACE_COST * D6_CELLS,
            "unique_candidates_checked": 1,
            "verified": verified,
            "policy_used": False,
            "c_adopted": False,
            ...
            "identified": independent_truth(task["stuck"]) == frozenset(proposed),
        }
    if policy["kind"] != "table":
        raise ValueError(f"unknown D6 policy kind {policy['kind']}")
    if policy["policy_id"] in D6_FROZEN_POLICY_NAMES:
        raise RuntimeError("refusing frozen M11/JUMP/MUTATE_C evolution identity")
    width = int(policy["width"])
    proposed, invert_attempts = invert_gauge(syndrome, width)
    shadow = shadow_eval_repair(task["stuck"], proposed)
    token_word = [
        gauge,
        f"PROPOSE:{','.join(str(index) for index in proposed)}",
        "SHADOW",
    ]
    propose_cost = D6_PROPOSE_COST
    if not policy.get("c_may_adopt", True):
        token_word.append("C_REFUSE")
        return {
            "status": "NOT_FOUND",
            "token_word": token_word,
            "program": token_word,
            "enumeration_attempts": propose_cost + shadow["cost"],
            "unique_candidates_checked": invert_attempts,
            "verified": False,
            "policy_used": False,
            "c_adopted": False,
    ...
    adopted = constitution_adopt(proposed, shadow["quality"], CONSTITUTION)
    if not adopted:
        token_word.append("C_REFUSE")
        return {
            "status": "NOT_FOUND",
    ...
    token_word.append("C_ADOPT")
    live = PlantWorld(task["stuck"])
    quality = live.replace_cells(proposed)
    restarted = live.restart_clone()
    token_word.extend([
        f"REPLACE:{','.join(str(index) for index in proposed)}",
        "RESTART",
    ])
    verified = quality == 1 and restarted.quality() == 1 and shadow["quality"] == 1
    identified = independent_truth(task["stuck"]) == frozenset(proposed)
    return {
        "status": "FOUND" if verified else "NOT_FOUND",
        "token_word": token_word,
        "program": token_word,
        "enumeration_attempts": propose_cost + shadow["cost"] + D6_ADOPT_COST + D6_REPLACE_COST * len(proposed),
        "unique_candidates_checked": invert_attempts,
        "verified": verified,
        "policy_used": True,
        "c_adopted": True,
...
def evaluate_plant(tasks, policy):
    rows = []
    total = 0
    checks = 0
    used = 0
    ...
    identified = 0
    for task in tasks:
        result = serve_plant(task, policy)
        row = {
            "task": task["fingerprint"],
            "status": result["status"],
            "token_word": list(result["token_word"]),
            ...
            "verified": result["verified"],
            "policy_used": result["policy_used"],
            "c_adopted": result["c_adopted"],
            "shadow_quality": result["shadow_quality"],
        ...
        total += row["enumeration_attempts"]
        checks += row["unique_candidates_checked"]
    return rows, total, checks, used, adopted, identified


def discover_evolution_policy(train_tasks, origin: str):
    """Invent one EVOL_* invert-gauge repair of learned width via shadow-eval. Never MUTATE_C."""
    table = {}
    discovery_cost = 0
    attempts = []
    widths = set()
    for task in train_tasks:
        ...
        chosen_width = None
        chosen_repair = None
        for width in range(1, D6_STUCK_COUNT + 1):
            try:
                proposed, invert_attempts = invert_gauge(syndrome, width)
            except RuntimeError:
                discovery_cost += 1
                continue
            discovery_cost += invert_attempts
            shadow = shadow_eval_repair(task["stuck"], proposed)
            discovery_cost += shadow["cost"]
            if shadow["quality"] != 1:
                continue
            if not constitution_adopt(proposed, shadow["quality"], CONSTITUTION):
                continue
            chosen_width = width
            chosen_repair = [int(index) for index in proposed]
            break
        if chosen_width is None or chosen_repair is None:
            raise RuntimeError(f"D6 discovery failed on {task['fingerprint']}")
        if chosen_width == 1:
            raise RuntimeError("D6 width-1 inversion must not repair two-cell plants")
        widths.add(chosen_width)
        key = syndrome_key(syndrome)
        if key in table and table[key] != chosen_repair:
            raise RuntimeError(f"D6 anti-unify failed on syndrome {key}")
        table[key] = chosen_repair
        attempts.append({
            "task": task["fingerprint"],
            "syndrome": key,
            "repair": chosen_repair,
            "width": chosen_width,
            "shadow_quality": 1,
            "c_adopted": True,
        })
    if widths != {D6_STUCK_COUNT}:
        raise RuntimeError(f"D6 train did not anti-unify on width {D6_STUCK_COUNT}: {widths}")
    width = D6_STUCK_COUNT
    policy_id = "EVOL_" + content_hash({
        "width": width,
        "family": D6_GRAMMAR_ID,
        "algorithm": D6_POLICY_TABLE,
    })[:12]
    if policy_id in D6_FROZEN_POLICY_NAMES:
        raise RuntimeError("D6 invented a frozen M11/JUMP/MUTATE_C name")
    if not policy_id.startswith("EVOL_"):
        raise RuntimeError("D6 invented identity is not an EVOL_* evolution policy")
    return {
        "policy": table_evolution_policy(table, policy_id, c_may_adopt=True, width=width),
        "table": table,
        "policy_id": policy_id,
        "width": width,
        "origin": origin,
...
def d6_tournament(validation_tasks, policy):
    all_rows, all_total, all_checks, all_used, all_adopted, all_identified = evaluate_plant(
        validation_tasks, replace_all_policy()
    )
    refuse_policy = table_evolution_policy(
        policy["table"], policy["policy_id"], c_may_adopt=False, width=policy["width"]
    )
    no_c_rows, no_c_total, no_c_checks, no_c_used, no_c_adopted, no_c_identified = evaluate_plant(
        validation_tasks, refuse_policy
    )
    table_rows, table_total, table_checks, table_used, table_adopted, table_identified = evaluate_plant(
        validation_tasks, policy
    )
    table_found = all(row["status"] == "FOUND" and row["verified"] and row["c_adopted"] for row in table_rows)
    no_c_failed = all(not row["verified"] and not row["c_adopted"] for row in no_c_rows)
    accepted = (
        table_found
        and no_c_failed
        and policy["kind"] == "table"
        and policy["c_may_adopt"]
        and policy["width"] == D6_STUCK_COUNT
        and policy.get("not_task_id_table") is True
        and policy["policy_id"] not in D6_FROZEN_POLICY_NAMES
        and not policy["policy_id"].startswith("SEL_")
        and not policy["policy_id"].startswith("RW_")
        and not policy["policy_id"].startswith("CUT_")
        and table_used == len(validation_tasks)
        and table_adopted == len(validation_tasks)
        and table_identified == len(validation_tasks)
        and table_total < all_total
        and no_c_adopted == 0
    )
    evaluations = [
        {
            "name": D6_POLICY_REPLACE_ALL,
            "aggregate_cost": all_total,
            "verified": all(row["verified"] for row in all_rows),
            "c_adoptions": all_adopted,
        },
        {
            "name": D6_POLICY_WITHOUT_C,
            "aggregate_cost": no_c_total,
            "verified": any(row["verified"] for row in no_c_rows),
            "c_adoptions": no_c_adopted,
        },
        {
            "name": D6_POLICY_TABLE,
            "aggregate_cost": table_total,
            "verified": table_found,
            "c_adoptions": table_adopted,
        },
    ]
    return {
        "accepted": accepted,
        ...
        "baseline": evaluations[0],
        "evaluations": evaluations,
        "all_candidate_validation_cost": all_total + no_c_total + table_total,
        "replace_all_total": all_total,
        "without_c_total": no_c_total,
        "table_total": table_total,
        "table_used": table_used,
        "table_adopted": table_adopted,
        "table_identified": table_identified,
        "without_c_adopted": no_c_adopted,
        "replace_all_checks": all_checks,
        "without_c_checks": no_c_checks,
        "table_checks": table_checks,
        "table_rows": table_rows,
        "replace_all_rows": all_rows,
        "without_c_rows": no_c_rows,
        "no_c_failed": no_c_failed,
    }


def evolution_policy_record(policy, origin: str) -> dict:
    if policy["policy_id"] in D6_FROZEN_POLICY_NAMES:
        raise RuntimeError("refusing to persist M11/JUMP/MUTATE_C as D6 competence")
    identity = "method:" + content_hash({
        "kind": "evolution-policy.g7.d6.v1",
        "policy_id": policy["policy_id"],
        "table": policy["table"],
        "family": D6_GRAMMAR_ID,
    })
    return {
        ...
        "payload": {
            ...
            "table": dict(policy["table"]),
            "family": D6_GRAMMAR_ID,
            "algorithm": "invert-gauge-shadow-eval-external-c-adopt",
            "c_may_adopt": True,
            "external_c_required": True,
            "not_task_id_table": True,
            "frozen_names_refused": sorted(D6_FROZEN_POLICY_NAMES),
            "not_m11_relabel": True,
            "not_constitution_mutation": True,
            "not_neural": True,
            "not_metamath": True,
            "not_flt": True,
        },
...
def admit_evolution_policy(bundle: dict, policy, origin: str, stage: str, change_note: str):
    bundle = json.loads(canonical_json(bundle))
    assert_constitution_frozen(bundle)
    method = evolution_policy_record(policy, origin)
    schema = {
        "identity": "schema:" + content_hash({"method": method["identity"], "kind": "evolution-policy"}),
        "kind": "schema",
        "origin_category": origin,
        "payload": {
            "pattern": (
                "propose a tiny plant repair from the observable gauge, shadow-eval "
                "it on a clone, and require external constitution C to adopt before "
                "live replace and restart; not M11 relabel, not MUTATE_C, not neural"
            ),
            "originating_method_ids": [method["identity"]],
            "family": D6_GRAMMAR_ID,
        },
    ...
    if method["identity"] not in existing:
        ...
        bundle["method_schemas"].append(schema)
        bundle["support_dependency"]["edges"].append({
            "source": "d6-shadow-eval-c-adoption",
            "target": method["identity"],
            "relation": "SUPPORT",
        })
        bundle["field_state"]["atoms"].append({
        ...
        bundle["self_model"]["known_competence"].append(method["identity"])
        existing_donors = {donor["identity"] for donor in bundle["imported_donors"]}
        for donor in d6_taught_donors():
            if donor["identity"] not in existing_donors:
                bundle["imported_donors"].append(donor)
        existing_reps = {rep["identity"] for rep in bundle["representations"]}
        if "rep:governed-plant-self-evolution.v1" not in existing_reps:
            bundle["representations"].append({
                "identity": "rep:governed-plant-self-evolution.v1",
                "origin_category": ORIGIN_TAUGHT,
                "payload": {
                    "kind": "syndrome-to-shadow-eval-c-adopted-repair",
                    "grammar": D6_GRAMMAR_ID,
                    "not_m11_relabel": True,
                    "not_constitution_mutation": True,
                },
    ...
    bundle["self_model"]["unknown"] = [
        item for item in bundle["self_model"]["unknown"]
        if item not in {"OCM_2", "OCM_3", "OCM_4", "OCM_5", "OCM_6"}
    ]
    bundle["self_change_history"].append({
        "change": change_note,
        "stage": stage,

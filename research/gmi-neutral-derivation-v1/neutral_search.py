...
def verify_optimum(levels, truth, probabilities, prices, claimed_cost, winners):
    ...
    authoritative = {(size, sig): fiber for size, sig, fiber in actual}
    claimed_keys = [(size, sig) for size, sig, _ in winners]
    if (actual_cost != claimed_cost or set(authoritative) != set(claimed_keys)
            or len(claimed_keys) != len(set(claimed_keys))):
        raise ValueError("Missing, extra, or incorrectly priced optimal trace fiber")
    for size, sig, fiber in winners:
        expected = authoritative[size, sig]
        if (fiber.multiplicity != expected.multiplicity
                or fiber.program != expected.program):
            raise ValueError("Optimal fiber multiplicity or canonical representative changed")

...
## 8. Scope-correction annotations (2026-09-10, ORION-OCM M0 reconciliation STEERING_MEMO)

Appended per the append-only discipline: existing lines above are untouched; every original number and verdict is preserved verbatim. None of these annotations relabels a filed negative as positive or discards a raw result.

- **D27 scope (annotation, 2026-09-10, STEERING_MEMO section 1A).** The inherited/knocked-out objects in `research/ocm-form-oracle-v1/oracle/d27_lineage.py` (methods, schemas, facts, index_built) are the T2/T3 simulator Sim's simulated integer counters and boolean index flag (`research/ocm-morphology-zoo-v1/evaluation/lifetime.py` lines 115-119; costs from the frozen FIELD_MULT/EXEC_MULT tables), not acquired program objects. In `run_lifetime2` epoch zero grants persistent forms `s.methods += 1` (lifetime2.py line 232) and a method-acquisition task is solved when the counter is positive (lines 95/147); the schedule runs 2 method_acq tasks per epoch for 12 epochs, so continued memory supplies 22 extra successes after epoch 1 on the 88-task battery (2*11/88 = 0.25). The discrete +0.25 block is therefore schedule-derived, not evidence that a population independently discovered a 22-task abstraction; the knockout is meaningful inside the simulator; it cannot alone establish native method acquisition, actual future-search improvement, or a negative against every implemented OCM learner (`src/ocm/learning/methods.py` is a genuine native learner that changes candidate enumeration). Corrections appended in place: `FO_D27_AGGREGATE.json` and `FO_D27_CEILING_CONTROL.json` key `scope_correction_2026_09_10`.
- **D27 related/unrelated/harmful key contrast (annotation, 2026-09-10, STEERING_MEMO section 1B).** `research/ocm-form-oracle-v1/oracle/future_family.py` (lines 62-65) discloses that FOF1 and the held-out T3 share the same `t3_draw`/`run_t3` generator, which awards success from capability/budget conditions. The related/unrelated/harmful keys are different frozen parameter draws through ONE generator; the 0.0135 vs 0.0194 mean-relative-drop contrast (`b_future_cognition_signature`) is a contrast between frozen proxy draws, not an identified structural-relatedness effect, and must not be used to infer that future ecologies lack transferable regularities. Correction appended in place: `FO_D27_AGGREGATE.json` key `scope_correction_2026_09_10` (subsection `future_family_key_contrast_scope`).
- **D29 scope (annotation, 2026-09-10, STEERING_MEMO section 1D).** The pure-repeat Q* of about 80.7 is an off-grid projection (`D29_RHO_CROSSOVER_RESULTS.json` `off_grid_projection_if_no_crossover['0'].projected_crossover_Q_off_grid` = 80.72), not an observed crossover; the earlier registered rho-range prediction (rho* in [0.51, 0.905]) was refuted at the swept scope (NO_CROSSOVER_ANYWHERE_OFF_RHO1, 30 worlds x 5 rho x Q<=32); repeated-query benefits still need comparison with a parent allowed ordinary caching/persistence; the result tests this abstraction mechanism and cost model, not every developmental learner. Corrections appended in place: `D29_RHO_CROSSOVER_RESULTS.json` key `scope_correction_2026_09_10`; registry entries `scope_correction_d27_2026_09_10` / `scope_correction_d29_2026_09_10` appended to `claim_shape_rules.predictions_registered` in `QUERY_ECOLOGY_AMENDMENT_V1.json`.

...
def main():
    ...
    argv=[str(PYTHON),"-I","-S","-B",str(ROOT/"check_adapter.py"),str(output/"CONTROLS.json")]
    ...
    allowed={str(ROOT/n) for n in ("check_adapter.py","authored_fixture.py","hole_match.py","replacement.py")}
    allowed|={str(ROOT/"vendor"/n) for n in ("typed_context.py","typed_emit.py","typed_constructor.py","structural_fixture.py")}
    ...
    passed=(pid==child.pid and child.returncode==0 and matched and imports_ok and not forbidden
        and before==after and controls["passed"] and controls["tests_run"]==8 and controls["native_calls"]==0)

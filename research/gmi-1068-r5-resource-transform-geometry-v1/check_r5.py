...
def paths_ac():
 return [(2,1),add((1,0),(0,2))]
def score(w,c):return w[0]*c[0]+w[1]*c[1]
def main():
 p=pareto(paths_ac());assert p==[(1,2),(2,1)],p
 ...
 assert wins==["DIRECT","VIA_B"],wins
 # triangle using best scalar direct/via
 for w in ws:
  dAC=min(score(w,c) for c in p)
  dAB=score(w,(1,0));dBC=score(w,(0,2))
  assert dAC<=dAB+dBC
 # remint preserves cost multiset/frontier
 rename={"A":"X","B":"Y","C":"Z"}
 reminted=[c for c in paths_ac()]
 assert pareto(reminted)==p
 hostiles=[
  ("C","A") not in edges,
  len(p)==2,
  wins[0]!=wins[1],
 ...
 assert r["pareto_costs_A_C"]==[[1,2],[2,1]]

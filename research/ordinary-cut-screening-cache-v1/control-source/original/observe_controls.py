...
paths=[ROOT/"check_cache.py",Path(__file__).resolve(),Path(sys.executable).resolve()]
paths+=list((ROOT/"source").glob("*.py"))+[ROOT/"history/syntax_adapter.py"]
...
argv=[sys.executable,"-I","-S","-B",str(ROOT/"check_cache.py"),str(OUT)]
...
receipt={"schema":"ordinary.syntax-cache-process.v1","observer_pid":os.getpid(),

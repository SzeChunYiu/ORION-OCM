...
BASE=ROOT.parent.parent/"ordinary-cut-screen-revival-v1"
...
paths=[ROOT/"check_metadata.py",Path(__file__).resolve(),Path(sys.executable).resolve()]
paths+=list((ROOT/"source").glob("*.py"))+[ROOT/"history/syntax_memo.py"]
...
argv=[sys.executable,"-I","-S","-B",str(ROOT/"check_metadata.py"),str(OUT)]
...
receipt={"schema":"ordinary.syntax-cache-metadata-process.v1","observer_pid":os.getpid(),

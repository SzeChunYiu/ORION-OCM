"""Hostile protocol tests for the G7 D4 lineage successor."""
...
SPEC_L = importlib.util.spec_from_file_location("g7_lineage_d4", HERE / "lineage.py")
L = importlib.util.module_from_spec(SPEC_L)
SPEC_L.loader.exec_module(L)
SPEC_E = importlib.util.spec_from_file_location("g7_experiment_d4", HERE / "experiment.py")
...
class TestG7LineageProtocol(unittest.TestCase):
    ...
    def test_unrun_stages_are_d5_through_d6(self):
        self.assertEqual(L.UNRUN_STAGES, ("OCM_5", "OCM_6"))
    ...
    def test_d4_family_is_disjoint_and_not_frozen_regex_eval(self):
        self.assertNotEqual(L.D4_GRAMMAR_ID, L.GRAMMAR_ID)
        self.assertNotEqual(L.D4_GRAMMAR_ID, L.D2_GRAMMAR_ID)
        self.assertNotEqual(L.D4_GRAMMAR_ID, L.D3_GRAMMAR_ID)
        ...
        self.assertEqual(len({poly, diag, hilbert, rewrite}), 4)
        population = L.rewrite_population()
        self.assertGreaterEqual(len(population), 24)
        self.assertTrue(all(task["domain"] == L.D4_GRAMMAR_ID for task in population))
        self.assertEqual(L.D4_FROZEN_RULE_NAMES, frozenset({"REGEX", "EVAL"}))
        self.assertTrue(all(task["source"] != task["target"] for task in population))
...
class TestG7Experiment(unittest.TestCase):
    ...
    def test_one_lineage_id_and_five_earned_transitions(self):
        result = self.result
        self.assertEqual(result["lineage_id"], L.LINEAGE_ID)
        self.assertEqual(result["earned_transitions"], 5)
        t0, t1, t2, t3, t4 = result["transitions"]
    ...
    def test_t4_is_exact_string_rewrite_not_metamath_flt_or_m11(self):
        ...
        self.assertTrue(t4["families_disjoint"])
        self.assertEqual(t4["constitution"], list(L.CONSTITUTION))
        t0_method = self.result["transitions"][0]["new_methods_schemas"][0]["identity"]
        ...
        self.assertIn(t0_method, t4["reused_method_identities"])
        self.assertIn(t1_failure, t4["reused_method_identities"])
        self.assertIn(t2_policy, t4["reused_method_identities"])
        self.assertIn(t3_cut, t4["reused_method_identities"])
        self.assertTrue(self.result["transitions"][4]["actual_execution_witnesses"])
        self.assertTrue(self.result["transitions"][4]["ablation"]["effect_disappears"])
        self.assertTrue(self.result["transitions"][4]["strongest_parent_result"]["reset_costs_more_or_fails_reuse"])
        self.assertFalse(self.result["transitions"][4]["strongest_parent_result"]["RESET_OCM"]["reused_prior_methods"])
        self.assertTrue(self.result["transitions"][4]["strongest_parent_result"]["CONTINUED_OCM"]["reused_prior_methods"])
        train_ids = set(self.result["transitions"][4]["new_information_supplied"]["training_task_ids"])
        earlier = set()
        for record in self.result["transitions"][:4]:
            earlier |= set(record["new_information_supplied"]["training_task_ids"])
        self.assertFalse(train_ids & earlier)
        payload = self.result["transitions"][4]["new_methods_schemas"][0]["payload"]
        self.assertTrue(payload["not_metamath"])
        self.assertTrue(payload["not_flt"])
        self.assertTrue(payload["not_m11_relabel"])
        self.assertEqual(payload["rule_id"], t4["rule_id"])
        self.assertEqual(payload["lhs"], ["P", "Q"])
        self.assertEqual(payload["rhs"], ["Q", "P"])
        self.assertGreater(t4["rewrite_uses"], 0)
        self.assertEqual(t4["continued_work_units"], t4["task_specific_work_units"])
    ...
    def test_honest_unrun_boundary_and_terminal(self):
        ...
        self.assertEqual(result["final_stage"], "OCM_4")
        self.assertNotEqual(result["terminal"], "CANNOT_CHECK_D4_REWRITE_NOT_EARNED")
        self.assertIn("not Metamath", result["d4_scope"])
        self.assertIn("not FLT", result["d4_scope"])

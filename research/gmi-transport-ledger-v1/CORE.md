# Transport ledger: read first

Ledger item 9. "Do the laws survive realistic scale?" splits in two, and the
halves have opposite answers.

- **Lower bounds transport, and always did.** CLB-1's chain
  `inf_F a <= a(q_M) <= c(M)` is an order argument needing only allocation
...
Status: **NOT REGISTERED IN THE REPLAY CAPSULE**. No measured constant is
claimed, and no registered bound is asserted to have its accounting premise
discharged for a physical machine.

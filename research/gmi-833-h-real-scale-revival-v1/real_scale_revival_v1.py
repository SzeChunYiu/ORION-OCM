...
SCOPES = ("R02", "R03", "R04")
SIGMA = {"R02": "SIGMA_R02", "R03": "SIGMA_R03", "R04": "SIGMA_R04"}
...
def evaluate_predictions(scopes, cross, controls):
    """Every prediction of FREEZE_V1.md section 8, held or failed."""
    p = {}

    digests = set(scopes[k]["grammar"]["digest"] for k in SCOPES)
    after = set(scopes[k]["grammar_digest_after"] for k in SCOPES)
    p["P00"] = {"held": bool(len(digests) == 1 and digests == after
                             and digests == set([G.digest()])),
                "detail": {"digest": sorted(digests),
                           "recomputed": G.digest(),
    ...
    r2 = scopes["R02"]
    a2 = r2["arms"]
    p["P2a"] = {"held": bool(r2["chosen"]["class"] == "AFFINE_SCORE"
                             and r2["chosen"]["attributes"]["body_is_arg_times_param"]
                             and not r2["chosen"]["attributes"]["reads_state"]),
                "detail": {"class": r2["chosen"]["class"],
                           "body": r2["chosen"]["body"], "head": r2["chosen"]["head"]}}
    p["P2b"] = {"held": bool(cross["R04"]["class"] != "AFFINE_SCORE"),
                "detail": {"control_ecology": "F04", "class": cross["R04"]["class"]}}
    p["P2c"] = {"held": bool(Q(a2["chosen"]["held_sse"]) < Q(a2["best_constant"]["held_sse"])
                ...
                "detail": {"chosen_sse": a2["chosen"]["held_sse"],
                           ...
                           "majority_decision_errors":
                               a2["majority_sign"]["held_decision_errors"]}}
    if controls is None:
        p["P2d"] = {"held": False, "detail": {"reason": "no control artifact"},
                    "applicable": False}
    else:
        chosen = Q(controls["chosen_arm_held_sse"])
        lo = Q(controls["band_low"])
        applicable = bool(controls["in_band"] == controls["controls"]
                          == N_CONTROLS)
        p["P2d"] = {"held": bool(applicable
    ...
    p["P2e"] = {"held": bool(r2["crossover"]["m_star"] is not None),
                "detail": r2["crossover"]}

    r3 = scopes["R03"]
    a3 = r3["arms"]
    adm = r3["admissibility"]
    p["P3e"] = {"held": bool(adm["admissible"] and adm["min_response"] >= 1
                ...
                "detail": {"min_response": adm["min_response"],
                           ...
                           "n_fit": adm["n_fit"]}}
    p["P3a"] = {"held": bool(r3["chosen"]["class"] == "NONLINEAR_LINK"
    ...
    n_held3 = a3["log_link"]["n"]
    p["P3b"] = {"held": bool(a3["log_link"]["strict_wins"] * 2 > n_held3),
                "detail": {"log_strict_wins": a3["log_link"]["strict_wins"],
                           "identity_strict_wins": a3["identity_link"]["strict_wins"],
                           "ties": a3["ties"], "n_held": n_held3}}
    p["P3c"] = {"held": bool(a3["identity_link"]["negative_means"] >= 1
    ...
    p["P3d"] = {"held": bool(cross["R02"]["class"] != "NONLINEAR_LINK"),
                "detail": {"control_ecology": "F02", "class": cross["R02"]["class"]}}
    p["P3f"] = {"held": bool(r3["crossover"]["m_star"] is not None),
                "detail": r3["crossover"]}

    r4 = scopes["R04"]
    a4 = r4["arms"]
    p["P4a"] = {"held": bool(r4["chosen"]["class"] == "LIFTED_BASIS"
                             and not r4["chosen"]["attributes"]["body_affine_in_arg"]),
                "detail": {"class": r4["chosen"]["class"],
                           "body": r4["chosen"]["body"], "head": r4["chosen"]["head"]}}
    p["P4b"] = {"held": bool(Q(a4["lifted"]["held_sse"]) < Q(a4["affine"]["held_sse"])),
                "detail": {"lifted_sse": a4["lifted"]["held_sse"],
                           "affine_sse": a4["affine"]["held_sse"],
                           "affine_two_routes_agree": a4["affine"]["two_routes_agree"]}}
    p["P4c"] = {"held": bool(Q(a4["lifted"]["held_sse"]) > 0),
                "detail": {"lifted_sse": a4["lifted"]["held_sse"]}}
    p["P4d"] = {"held": bool(a4["landmark"].get("q_star") is not None),
                "detail": a4["landmark"]}
    p["P4e"] = {"held": bool(cross["R02"]["class"] != "LIFTED_BASIS"),
                "detail": {"control_ecology": "F02", "class": cross["R02"]["class"]}}
    return p


GATE_EVIDENCE = {
    "R02": {"R01": ["P2a"], "R02": ["P00"], "R03": ["P00"], "R04": ["P2a"],
            "R05": ["P2b", "P2d"], "R06": [], "R07": ["P2e"], "R08": ["P2c"],
            "R09": [], "R10": [], "R11": []},
    "R03": {"R01": ["P3a"], "R02": ["P00"], "R03": ["P00"], "R04": ["P3a"],
            "R05": ["P3d"], "R06": [], "R07": ["P3f"], "R08": ["P3b", "P3c", "P3e"],
            "R09": [], "R10": [], "R11": []},
    "R04": {"R01": ["P4a"], "R02": ["P00"], "R03": ["P00"], "R04": ["P4a"],
            "R05": ["P4e"], "R06": [], "R07": ["P4d"], "R08": ["P4b", "P4c"],
            "R09": [], "R10": [], "R11": []},
}
...
def run_hostiles(scopes, src, controls, rows):
    ...
    shapes = [scopes[k]["arms"] for k in SCOPES]
...
def main():
    ...
    preds = evaluate_predictions(scopes, cross, controls)

...
The machine-readable form of this receipt is `D1_SENSITIVITY_V1.json`. It is a
separate artifact rather than a field in `RESULT_V1.json`, because that file is
regenerated by the executor and gated byte-exact in CI (`git diff --exit-code`
after a re-run). A hand-added field there breaks reproducibility — which is the
gate working correctly, and is how this placement was found.

...
class CustodyTests(unittest.TestCase):
    ...
    def test_complete_real_no_alarm_and_full_replay(self):
        ...
        self.assertEqual(replay(ROOT)['status'],'FULL_TYPED_COST_PAYLOAD_REPLAY_PASS')

    def test_same_status_changed_numeric_payload_rejected(self):
        expected=json.loads((ROOT/'RECEIPT_V1.json').read_text())
        expected['rows']['TRANSPARENT_PARTIAL_PARENT']['python_opcodes_per_sweep']=32
    ...
    def test_raw_and_code_changes_rejected(self):
        for name in ['typed_machine_v1.py','raw/grand_gmi_delegation_invariant_cost_checks_v1.py']:
    ...
    def test_late_unbound_change_rejected_even_with_correct_payload(self):
        ...
        def worker(path):
            (path/'typed_machine_v1.py').write_bytes(b'changed')
            return wanted

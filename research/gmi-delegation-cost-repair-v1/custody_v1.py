...
def verify_manifest(root):
    ...
    require(manifest['schema'] == 'GMI_DELEGATION_COST_UNIT_MANIFEST_V1', 'manifest version')
...
def replay(root, worker=None):
    ...
    return {'status':'FULL_TYPED_COST_PAYLOAD_REPLAY_PASS', 'manifest_sha256':digest(initial_raw),
            'receipt_sha256':digest(expected), 'payload_files':len(initial['files']),
            'independent_execution_comparisons':expected_payload['independent_execution_comparisons']}

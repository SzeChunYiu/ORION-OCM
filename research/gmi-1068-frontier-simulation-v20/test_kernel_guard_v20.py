...
import check_lean_v20 as kernel
...
class KernelGuardTests(unittest.TestCase):
    def test_registered_statement_types_reject_source_valid_mutants(self):
        ...
        changes = [
            [("ProofTargetsV20.lean",
              r"theorem frontier_cofinal_contract[\s\S]*?(?=\ntheorem guarded_iff_contract)",
              "theorem frontier_cofinal_contract : True := True.intro\n")],
            [("ProofTargetsV20.lean",
              r"theorem guarded_iff_contract[\s\S]*?(?=\ntheorem greatest_words_contract)",
              "theorem guarded_iff_contract : True := True.intro\n")],
            [("ProofTargetsV20.lean",
              r"theorem greatest_words_contract[\s\S]*?(?=\nend ProofTargetsV20)",
              "theorem greatest_words_contract : True := True.intro\n")],
        ]
        ...
        for index, mutation in enumerate(mutations):
            with self.subTest(mutation=index), tempfile.TemporaryDirectory(prefix="v20-kernel-hostile-") as directory:
                root = Path(directory)

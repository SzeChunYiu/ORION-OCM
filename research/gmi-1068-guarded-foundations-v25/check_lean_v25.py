...
from proof_contract_v25 import ENTRIES, SOURCE_NAMES, SOURCE_PATHS, audit_source
...
def evaluate(source_overrides=None, require_source_compile=False):
    ...
    audit = audit_source()
    try:
        with tempfile.TemporaryDirectory(prefix="gmi-v25-lean-") as output:
            ...
            audit_path = Path(output) / "RegisteredProofsV25.lean"
    ...
    return {"status": "PASS", "lean_version": "4.19.0", "sources": bindings,
            ...
            "source_assumptions": "actual V19 strong partial associative algebra with two-sided local units and coherence; actual V11 lawful category; labelled ambient carrier; true-unit empty guards; attained-image recovery; explicit named-object identity map; no absolute primitive count or compression claim"}

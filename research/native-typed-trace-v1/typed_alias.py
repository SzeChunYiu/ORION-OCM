"""Conventional one-step matching, including composite/repeated substitutions."""
import typed_context as TC
import typed_grammar as grammar

def count(work,key,n=1):
    work[key]=work.get(key,0)+n
def match(pattern, ground, floating, initial, work, syntax):
        ...
        if token not in variables:
            ...
            for end in range(j+1,len(ground)+1):
                ...
                if key not in cache:
                    ...
                    try: syntax(variables[token],value); cache[key]=True
...
def dv_valid(row,mapping):
    # The registered target context has exactly V0/V1/V2 and no ambient DV.
    for a,b in row["dv"]:
        av=set(mapping[a]) & set(TC.IDS); bv=set(mapping[b]) & set(TC.IDS)
        if av and bv: return False
    return True
def applications(row,query,premises,work,syntax):
    answers=[]
    for m in match(row["statement"],query,row["floating"],{},work,syntax):
        states=[(m,[])]
        for h in row["essential"]:
            ...
            for prior,slots in states:
                for i,p in enumerate(premises):
                    ...
                    for nxt in match(h["statement"],p,row["floating"],prior,work,syntax):
    ...
    return answers
def ground_guard(query,premises,syntax):
...
def one_step(contracts,query,premises,parameters,work):
    ...
    for row in contracts:
        ...
        result+=applications(row,query,premises,work,syntax)
    return result


def classify(contracts,body,work):
    """An unsupported check cannot grant production eligibility."""
    try:
        aliases=one_step(contracts,body["query"],body["premises"],body["parameters"],work)
    except (ValueError,KeyError,TypeError,IndexError,RecursionError) as exc:
        return {"status":"UNKNOWN","eligible":False,"aliases":[],
                "reason":type(exc).__name__+": "+str(exc)}
    return {"status":"ALIAS" if aliases else "NO_ALIAS_IN_REGISTERED_DOMAIN",
            "eligible":not aliases,"aliases":aliases}

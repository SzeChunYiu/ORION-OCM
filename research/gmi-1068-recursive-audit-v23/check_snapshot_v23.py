"""Close only the registered R3-008 affine-regime atom; preserve every other original record; enforce complete adjudication records."""
import hashlib
...
ROOT = HERE.parents[1]
BASE = ROOT / "research/gmi-1068-recursive-audit-v22/SCOPE_SNAPSHOT_V22.json"
BASE_SHA = "289a91f8485dcc8e754f45afe9ed26791a17ba93c4ebe26194bcdbcedf8a039c"
...
CONTRACT = load("audit_contract_v23", HERE / "audit_contract_v23.py")
...
def evaluate(current=None, receipt=None, accounting=None):
    ...
    need(hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA, "V22 baseline drift")
    before = json.loads(BASE.read_text())
    if current is None:
        current = json.loads((HERE / "SCOPE_SNAPSHOT_V23.json").read_text())
    if receipt is None:
        receipt = json.loads((ROOT / CONTRACT.PACKAGE / "RESULT_V23.json").read_text())
    need(type(current) is dict and set(current) == set(before), "snapshot top-level fields")
    for field in before:
        if field != "rounds":
            expected = "855e538865a69150e1ebff5d9e3fb0ce8e6701ff" if field in ("observed_main", "audited_base") else before[field]
    ...
    need(result["registered_atoms"] == 222 and result["unresolved_atoms"] == 193,
    ...
    need(len(scientific) == 21, "scientific closure count drift")
    expected_accounting = CONTRACT.current_accounting(current, receipt["amendment_carry"])
    if accounting is None:
        accounting = json.loads((HERE / "CURRENT_ACCOUNTING_V23.json").read_text())
    exact(accounting, expected_accounting, "current accounting drift")
    for key, value in {"original_total": 222, "original_fulfilled": 29, "original_unresolved": 193,
                       "qualified_replacements": 2, "active_unresolved": 191}.items():

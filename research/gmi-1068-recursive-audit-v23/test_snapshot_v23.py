...
spec=importlib.util.spec_from_file_location('checked_snapshot_v23',HERE/'check_snapshot_v23.py')
...
class SnapshotTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.snapshot=json.loads((HERE/'SCOPE_SNAPSHOT_V23.json').read_text())
        cls.receipt=json.loads((checker.ROOT/checker.CONTRACT.PACKAGE/'RESULT_V23.json').read_text())
    ...
    def test_actual_snapshot_no_alarm(self):
        ...
        self.assertEqual(len(result['scientific_atoms_closed']),21)
        self.assertEqual((result['original_fulfilled'],result['remaining_atoms'],
                          result['qualified_replacements'],result['active_unresolved']),(29,193,2,191))
    ...
    def test_rehashed_snapshot_mutations(self):
        ...
        wrong=checker.CONTRACT.witness(checker.ROOT,'AffineIntervalsV23.lean','lean_declaration','interval_le_iff')
        ...
        for index in (8,):changes.append((atom('R3',index,'status'),'CLOSED'))
        invalid=[]
        ...
        bad=deepcopy(self.snapshot);bad['rounds']['R3']['artifacts']=[a for a in bad['rounds']['R3']['artifacts']
            if a['path']!=checker.CONTRACT.PACKAGE+'/FREEZE_V23.md'];invalid.append(bad)
        bad=deepcopy(self.snapshot);path=HERE/'audit_contract_v23.py'
        ...
        self.assertEqual(len(invalid),74)
    ...
    def test_receipt_mutations(self):
        ...
        coverage_key=next(iter(self.receipt['coverage']['test_diagrams_v23']))
        ...
        changes=[(('atoms',),{'GMI2-R1-001':{'status':'VERIFIED_AT_REGISTERED_SCOPE'}}),
            (('registered_results','W1','scope'),'every nonlinear family has finite affine regimes'),
            (('registered_results','W2','status'),'PENDING_REVIEW'),
            ...
            (('coverage','test_diagrams_v23',coverage_key),True),
            (('coverage','test_diagrams_v23',coverage_key),float(self.receipt['coverage']['test_diagrams_v23'][coverage_key])),
        ...
        for path in ((),('kernel',),('source_bindings',),('coverage','test_diagrams_v23'),('inputs',),('registered_results',)):
    ...
    def test_coupled_result_scope_and_original_promotion(self):
        ...
        for result in checker.CONTRACT.RESULTS:
            ...
            claim='Every nonlinear family has a finite rational regime diagram.'
            snapshot['rounds']['R3']['local_repairs'][-1]['scope']=claim
        ...
        for atom_id in checker.CONTRACT.ATOM_SPECS:
            ...
            atom['disposition']='All nonlinear context families are completely classified.'
    ...
    def test_current_accounting_mutations(self):
        ...
        for path,value in [(('original_fulfilled',),30),(('original_unresolved',),192),
                (('active_unresolved',),190),(('qualified_replacements',),3),(('original_total',),222.0),
        ...
        invalid.append(json.loads((checker.BASE.parent/'CURRENT_ACCOUNTING_V22.json').read_text()))

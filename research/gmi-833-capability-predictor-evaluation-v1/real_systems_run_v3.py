...
``REAL_RUNS/REAL_MEASURED_V3.json`` is what CI and the canonical executor read.
...
OUT_DIR = os.path.join(HERE, "REAL_RUNS_V3")
...
def main():
    ...
    for mech in hv.REAL3_MECHS:
        for size in hv.REAL3_SIZES:
            for w in (0, 1):
    ...
    payload = {
        ...
        "band": str(hv.REAL3_BAND),
    }
    if not os.path.isdir(OUT_DIR):
        os.makedirs(OUT_DIR)
    body = json.dumps(payload, indent=2, sort_keys=True)
    with open(os.path.join(OUT_DIR, "REAL_MEASURED_V3.json"), "w") as handle:
        handle.write(body)
        handle.write("\n")

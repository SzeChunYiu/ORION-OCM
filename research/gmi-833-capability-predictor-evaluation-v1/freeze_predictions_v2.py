"""Freeze the SIGMA_REAL2 revival predictions BEFORE the V2 training runs.
...
def main():
    ...
    spec = hv.sigma_real2()
    certs = [hu.disjointness_certificate(base, "SIGMA_1", spec["UNIVERSE"], "SIGMA_REAL2"),
             hu.disjointness_certificate(hu.SYN_RAW, "SIGMA_SYN", spec["UNIVERSE"], "SIGMA_REAL2"),
             hu.disjointness_certificate(hu.ARCH_RAW, "SIGMA_ARCH", spec["UNIVERSE"], "SIGMA_REAL2"),
             hu.disjointness_certificate(hu.REAL_RAW, "SIGMA_REAL", spec["UNIVERSE"], "SIGMA_REAL2")]
    ...
    summary, stream = fp.sweep(parent, "SIGMA_REAL2")
    ...
    out = {
        "schema": "GMI_833_K_EVAL_FROZEN_PREDICTIONS_V2",
        "supersedes": None,
        ...
        "real_system_protocol": {
            ...
            "systems": [list(m) for m in hv.REAL2_MACHINES],
            "registered_solved_bits": dict(
                ("|".join(str(x) for x in m), hv.real2_solved_law(m))
                for m in hv.REAL2_MACHINES),
            "mu": [str(x) for x in hu.MU_REAL],
        },
    }
    body = fp.canonical_json(out)
    text = json.dumps(json.loads(body), indent=2, sort_keys=True)
    with open(os.path.join(HERE, "FROZEN_PREDICTIONS_V2.json"), "w") as handle:
        handle.write(text)
        handle.write("\n")
    lines = stream.rstrip("\n").split("\n")
    sample = ["# universe=SIGMA_REAL2 rows=%d rule=every_101st_row" % len(lines)]
    for i in range(0, len(lines), 101):
        sample.append("SIGMA_REAL2\t" + lines[i])
    with open(os.path.join(HERE, "FROZEN_PREDICTION_SAMPLE_V2.tsv"), "w") as handle:
        handle.write("\n".join(sample) + "\n")

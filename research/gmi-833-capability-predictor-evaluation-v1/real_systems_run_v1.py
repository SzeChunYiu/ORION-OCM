...
``REAL_RUNS/REAL_MEASURED_V1.json`` is what CI and the canonical executor read.
...
OUT_DIR = os.path.join(HERE, "REAL_RUNS")
...
def main():
    ...
    for mech in hu.REAL_MECHS:
        for size in hu.REAL_SIZES:
            for w in (0, 1):
    ...
    measured = {}
    for machine in hu.REAL_MACHINES:
    ...
    payload = {
        "schema": "GMI_833_K_EVAL_REAL_MEASURED_V1",
    ...
    with open(os.path.join(OUT_DIR, "REAL_MEASURED_V1.json"), "w") as handle:
        handle.write(body)
        handle.write("\n")

"""Train and externally evaluate the 32 registered REAL systems (torch, CPU).

Runs on laptop-billy only; the deterministic receipt it writes to
``REAL_RUNS/REAL_MEASURED_V2.json`` is what CI and the canonical executor read.
Nothing here can see a prediction: the registered solved-set law lives in
``heldout_universes_v1.real_solved_law`` and is compared to these measurements
only by the scorer.

Protocol frozen in FREEZE_V1.md section 2.6.
"""
...
OUT_DIR = os.path.join(HERE, "REAL_RUNS_V2")
...
def main():
    ...
    for mech in hv.REAL2_MECHS:
        for size in hv.REAL2_SIZES:
            for w in (0, 1):
    ...
    measured = {}
    for machine in hv.REAL2_MACHINES:
    ...
    payload = {
        "schema": "GMI_833_K_EVAL_REAL_MEASURED_V2",
        ...
        "solved_rule": "exact accuracy == 1 on the protected split",
    }
    if not os.path.isdir(OUT_DIR):
        os.makedirs(OUT_DIR)
    body = json.dumps(payload, indent=2, sort_keys=True)
    with open(os.path.join(OUT_DIR, "REAL_MEASURED_V2.json"), "w") as handle:
        handle.write(body)
        handle.write("\n")

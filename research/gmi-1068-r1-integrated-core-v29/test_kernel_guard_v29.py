...
import check_lean_v29 as kernel
...
class KernelGuardTests(unittest.TestCase):
    def test_registered_statement_types_reject_source_valid_mutants(self):
        ...
        changes = [
            [("ProofTargetsV29.lean",
              r"theorem presentation_contract[\s\S]*?(?=\ntheorem fixed_encoder_contract)",
              "theorem presentation_contract : True := True.intro\n")],
            [("ProofTargetsV29.lean",
              r"theorem fixed_encoder_contract[\s\S]*?(?=\ntheorem guarded_restriction_contract)",
              "theorem fixed_encoder_contract : True := True.intro\n")],
            [("ProofTargetsV29.lean",
              r"theorem guarded_restriction_contract[\s\S]*?(?=\nend ProofTargetsV29)",
              "theorem guarded_restriction_contract : True := True.intro\n")],
        ]
        ...
        for index, mutation in enumerate(mutations):
            with self.subTest(mutation=index), tempfile.TemporaryDirectory(prefix="v29-kernel-hostile-") as directory:
                root = Path(directory)

...
import custody_v29 as core
COVERAGE={}


@contextmanager
def overlay(verified):
    with tempfile.TemporaryDirectory(prefix='gmi-v29-custody-') as temporary:
        root=Path(temporary)
...
class CustodyTests(unittest.TestCase):
    def test_real_inherited_inputs_and_tampering(self):
        ...
        for name in files:
            with overlay(valid) as root:
                ...
                rejected+=1
        with overlay(valid) as root:
            target=root/'research/gmi-1068-irreducibility-witnesses-v28/forward_v28.py'
            target.write_bytes(target.read_bytes()+b'\n')
            path=root/core.RECEIPTS[0]
            receipt=json.loads(path.read_text())
            receipt['inputs']['forward_v28.py']=core.digest(target)
    ...
    def test_unavailable_inputs_and_path_escape(self):
        valid=core.verify(ROOT)
        missing=0
        for name in ('research/gmi-1068-typed-foundation-v11/paths_v11.py',
                     'research/gmi-1068-guarded-foundations-v25/named_v25.py',
                     *(core.PACKAGE+'/'+name for name in core.FREEZE_FILES),core.RECEIPTS[2]):
            with overlay(valid) as root:
                (root/name).unlink()

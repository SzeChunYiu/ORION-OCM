"""Tests for gmi-833-h-real-scale-nearest-neighbor-v1.
...
import grammar_nn_v1 as G          # noqa: E402
import real_scale_nearest_neighbor_v1 as C   # noqa: E402

PRIMARY = ("grammar_nn_v1", "run_real_scale_nearest_neighbor_v1",
           "real_scale_nearest_neighbor_v1")
SIGMA = "SIGMA_H05R"
...
class TwoRoutes(unittest.TestCase):
    ...
    def test_oracle_does_not_import_the_primary_executor(self):
        tree = ast.parse(read("independent_oracle_nn_v1.py"))
        named = []
    ...
    def test_oracle_module_namespace_is_clean(self):
        code = ("import sys, os; sys.path.insert(0, %r);"
                "import independent_oracle_nn_v1;"
    ...
    def test_the_checker_reads_no_parent_result_file(self):
        tree = ast.parse(read("real_scale_nearest_neighbor_v1.py"))
...
class Custody(unittest.TestCase):
    ...
    def test_the_slice_addenda_state_the_registered_forms(self):
        ...
        self.assertIn("20260921", text2)
        self.assertIn("20260922", text2)

    def test_the_first_run_receipts_are_still_committed(self):
        self.assertTrue(os.path.isfile(os.path.join(
            HERE, "REAL_RUNS", "scope_SIGMA_H05R.json")))
        self.assertTrue(os.path.isfile(os.path.join(
            HERE, "REAL_RUNS", "sources.json")))

    def test_the_manifest_names_all_four_freeze_files(self):
        man = load_json("MANIFEST_V1.json")
        self.assertEqual(man["freeze_addenda"],
                         ["FREEZE_V1_ARITHMETIC_ADDENDUM.md",
...
class GrammarAndReadouts(unittest.TestCase):
    ...
    def test_the_classifier_reads_the_name_alone(self):
        ...
        self.assertEqual(G.classify("CNT>=2"), "STORED_EXEMPLAR_COUNT_THRESHOLD")
        self.assertEqual(G.classify("PREF_VOTE"), "NEIGHBORHOOD_MAJORITY_VOTE")
        self.assertRaises(ValueError, G.classify, "kNN")

    def test_the_language_is_closed_and_outcome_free(self):
        self.assertIn("CNT>=1", G.READOUTS)
        self.assertIn("CNT>=2", G.READOUTS)
        self.assertNotIn("RETRIEVE", G.READOUTS)
        self.assertNotIn("SQUARE", G.READOUTS)
        self.assertEqual(len(G.READOUTS), 13)
...
class Reconciliation(unittest.TestCase):
    ...
    def setUp(self):
        self.recon = load_json("ISSUE_833_RECONCILIATION_H05_V1.json")
        self.result = load_json("RESULT_V1.json")

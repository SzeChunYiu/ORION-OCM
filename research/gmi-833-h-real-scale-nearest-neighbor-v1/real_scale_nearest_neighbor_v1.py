...
gmi-833-h-real-scale-nearest-neighbor-v1 (issue #833, section H).
...
loss or claim.
    ...
    python3 -I -B  real_scale_nearest_neighbor_v1.py
    python3 -I -O -B real_scale_nearest_neighbor_v1.py
"""
import json
import os
import random
...
PREDICTED_CLASS = "STORED_EXEMPLAR_MEMBERSHIP"
...
def decision(name, ql, c, p1, p0, e1, e0):
    """Exact decision of a registered readout on one query, from its tallies.

    FREEZE_V1_SLICE_ADDENDUM.md section 2. The empty-neighbourhood fallback is
    the fit majority (1); the executor asserted the fit shared fraction exceeds
    1/2 before any enumeration, and the checker re-asserts it from the receipt.
    """
    if name == "C0":
        return 0
    if name == "C1":
...
def block_errors(rows, name):
    ...
    for ql, yv, c, p1, p0, e1, e0 in rows:
        if decision(name, ql, c, p1, p0, e1, e0) != yv:
            e += 1
    return e
...
def check_nulls(rec):
    ...
    ok = (le == label["errors"] and de == design["errors"]
          ...
          and label["seed"] == 20260921 and design["seed"] == 20260922)
...
def check_ladder(rec):
    lad = rec["ladder"]
    errs = lad["errors"]
    ok = (lad["readout"] == "CNT>=1" and len(errs) == 8
...
def check_control(rec):
    ...
    ok = (pc["winner"] == "LEN<=10" and pc["cn1_errors"] == 79435
...
def check_regen(rec):
    ...
    ok = (rg["same_class"] and rg["class"] == PREDICTED_CLASS
          and rg["primary"]["winner"] == "CNT>=1"
          and rg["regen"]["winner"] == "CNT>=1")
...
def hostile_tampered_replay(rec):
    ...
    before = block_errors(rows, "CNT>=1")
    rows[0][1] = 1 - rows[0][1]
    after = block_errors(rows, "CNT>=1")
...
def main():
    rec = load("scope_SIGMA_H05R.json")
    ...
    rows["H05"] = {"row": ROW, "sigma": SIGMA, "predicted_class": PREDICTED_CLASS,
    ...
    result = {
        "schema": "GMI833HRealScaleNearestNeighborLedgerV1",
        "scope": SIGMA, "row": ROW,
        "claim_ceiling": CLAIM_CEILING,
        "forbidden_promotions": FORBIDDEN,
        "freeze_file": "FREEZE_V1.md",
        "freeze_addenda": ["FREEZE_V1_ARITHMETIC_ADDENDUM.md",
                           "FREEZE_V1_SLICE_ADDENDUM.md",
                           "FREEZE_V1_SLICE_ADDENDUM_R2.md"],
        "source_sha256_registered": SOURCE_SHA,
        "scale": scale,
        "rows_closed": [ROW] if rows["H05"]["complete"] else [],
        "rows_open": [] if rows["H05"]["complete"] else [ROW],
        "rows": rows,
    ...
    print("RESULT_V1.json written: %s supported=%d/11 complete=%s"
          % (SIGMA, supported, rows["H05"]["complete"]))
...
def _evidence(rkey):
    return {
        ...
        "R02": "GRAMMAR_G_NN_REGISTERED_AND_DIGESTED",
        "R03": "BLIND_READOUT_LANGUAGE_POSTHOC_CLASSIFIER",
        "R04": "FAMILY_BLIND_SELECTION_PROCEDURE",

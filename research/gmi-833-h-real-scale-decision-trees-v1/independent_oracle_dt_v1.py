"""Route B for gmi-833-h-real-scale-decision-trees-v1: a source-separated
oracle.

This file imports NOTHING from grammar_dt_v1, run_real_scale_decision_trees_v1
or real_scale_decision_trees_v1. It re-derives, from FREEZE_V1.md and its
...
test_real_scale_decision_trees_v1.py via an `ast` scan and a `sys.modules`
assertion, not by this comment.

    python3 -I -B  independent_oracle_dt_v1.py
"""
import json
import os
import random
...
EXPECT_CLASS = "THRESHOLD_CONJUNCTION"
...
READOUTS = (
    ...
    "LEN<=6&ASSOC>=1", "LEN<=7&ASSOC>=1", "LEN<=8&ASSOC>=1",
    "LEN<=9&ASSOC>=1", "LEN<=10&ASSOC>=1", "LEN<=11&ASSOC>=1",
    "LEN<=12&ASSOC>=1",
    "LEN<=6&ASSOC>=2", "LEN<=7&ASSOC>=2", "LEN<=8&ASSOC>=2",
    "LEN<=9&ASSOC>=2", "LEN<=10&ASSOC>=2", "LEN<=11&ASSOC>=2",
    "LEN<=12&ASSOC>=2",
    "LEN<=6&ASSOC>=3", "LEN<=7&ASSOC>=3", "LEN<=8&ASSOC>=3",
    "LEN<=9&ASSOC>=3", "LEN<=10&ASSOC>=3", "LEN<=11&ASSOC>=3",
    "LEN<=12&ASSOC>=3",
...
def decide(name, ql, c, fa, in_store, yl, p1, p0, e1, e0):
    """The registered readout semantics, re-implemented from the addendum text.

    A conjunction fires iff every conjunct fires. Empty-neighbourhood readouts
    fall back to the fit majority (1).
    """
    if name == "C0":
        return 0
    if name == "C1":
    ...
    if "&" in name:
        for part in name.split("&"):
            if decide(part, ql, c, fa, in_store, yl, p1, p0, e1, e0) == 0:
                return 0
        return 1
...
def block_errors(rows, name):
    return sum(1 for ql, yv, c, fa, in_store, yl, p1, p0, e1, e0 in rows
               if decide(name, ql, c, fa, in_store, yl, p1, p0, e1, e0) != yv)


def main():
    rec = fetch("scope_SIGMA_H08R.json")
    ...
    checks["holdout_counts"] = bool(
        ...
        and ho["n"] == 97018 and ho["shared"] == 76572 and ho["unique"] == 20446
        and ho["winner"] == "LEN<=9&CNT>=1" and ho["winner_class"] == EXPECT_CLASS)
    ...
    checks["control"] = bool(
        pc["winner"] == "LEN<=9" and pc["winner_class"] == "DESCRIPTOR_LENGTH_THRESHOLD"
        and pc["conj1_errors"] == 75318 and not f1 and pc["control_fires"])
    ...
    out = {
        "schema": "GMI833HRealScaleDecisionTreesOracleV1",
        "route": "B", "scope": SIGMA,

...
def induce(experiences):
    ...
    for x, rewritten in zip(prepared, result.rewritten):
        ...
        checks.append({'decoded': decoded, 'equivalence': check})
    return {'status': 'NO_NEW_ABSTRACTION' if not library else 'PROPOSED_ABSTRACTIONS',
            'settings': dict(SETTINGS), 'raw': raw, 'library': library,
            'training': prepared, 'rewrite_checks': checks,
            'later_generation_consumption': 'NOT_RUN'}

...
def evaluate(policy, *, arm, revoked=(), task=None, ks=None, config=None):
    ...
    stable = state_binding(ks) == policy.binding and digest(task) == policy.task_binding and digest(cfg) == policy.config_binding and rv in policy.family
    ...
    try:
        ...
        runtime_vector = outcome.trace.resources.as_dict()
    except Exception as exc:
        consumer = {"status": "ERROR", "error_type": type(exc).__name__, "error": str(exc),
                    "traceback": traceback.format_exc(), "committed": False}
    encoded_bytes = lambda value: len(json.dumps(wire(value), sort_keys=True, separators=(",", ":"), allow_nan=False).encode())
    return {"arm": arm, "request": wire({"state": state_binding(ks), "task": task, "config": cfg, "revoked": rv}),
            ...
            "resources": {"donor_calls": dict(run_counts), "preparation_donor_calls": dict(policy.counts),
                          ...
                          "canonical_field_bytes": encoded_bytes(ks),
                          "canonical_prepared_matrix_bytes": encoded_bytes(list(policy.matrices.values())),
                          "canonical_decoder_partition_bytes": encoded_bytes(f["blocks"]),
                          "exact_rational_outputs": True, "runtime_reported_vector": runtime_vector,
                          "performance_claim": "NOT_TESTED; serialized bytes are not resident memory; runtime work proxy unchanged; no process-tree CPU claim"}}

...
def execute(config, input_path):
    ...
    phase = config["phase"]; state = Path(config["state"])
    if phase == "acquire" and state.exists() and (not state.is_dir() or any(state.iterdir())):
        raise ValueError("ACQUISITION_STATE_NOT_FRESH")
    actor = Actor(config["state"], config["arm"])
    rows = Path(config["rows"])

...
def test_synthetic_fixed_pair_gate_and_missing_arm():
    ...
    assert result["component_gate"]=="ADOPT_EXACT_DONOR" and result["warm_median_ratio"]==.8
    values.pop(13)
...
def test_synthetic_mismatch_and_failed_cost_gate():
    ...
    assert adjudicate(values,assignments,[])["functional_terminal"]=="FUNCTIONAL_MISMATCH"
    values,assignments=paired()
    for a in assignments:
        if a["arm"]=="sympy": values[a["index"]]["cold_ns"]=1000
    assert adjudicate(values,assignments,[])["component_gate"]=="PARENT_SUFFICIENT_FOR_REGISTERED_GATE"

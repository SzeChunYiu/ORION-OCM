...
import subprocess
import sys
from trial_common import ROOT, read, sha, write

CTX=Path("/home/billy/orion-director-work/20260906/g1-current-context-20260906")
...
def main():
    ...
    if inventory!="b599c3a5368c862d588520b3e74ee712c4267b3eeb0d4cf4d0e72103ffee55db":raise ValueError("CONTEXT_CHANGED")
    ...
    for name,module in (("sympy","sympy"),("z3-solver","z3"),("sexpdata","sexpdata"),("mpmath","mpmath"),("ufal.udpipe",None),("cvc5",None)):
        ...
        packages[name]={"version":dist.version,"native_and_record_files":records}
        if module:
    ...
    for rel,record in context["source_files"].items():
        ...
        elif rel.startswith("research/ocm-prototype/") and rel.endswith(".py"):name=Path(rel).stem
        else:continue
    ...
    manifest={"schema":"OCM.ExactSparseTrial.v1","registration":"https://github.com/SzeChunYiu/ORION-OCM/issues/72#issuecomment-5558797826",
              ...
              "later_main_audit":"Root reported fea2893 adds four files only; existing f4 executed paths unchanged; JSONL retained, no SQLite or field-bridge adoption.",

...
def identities(syntax_operator_id=CATALOGUE[0]):
    if syntax_operator_id not in (CATALOGUE[0], "syntax:stanza-recurrent"):
        raise ValueError("unknown fixed syntax donor")
    ...
    if syntax_operator_id != CATALOGUE[0]:
        files.append(HERE / "stanza_donor.py")
...
def check(runtime, request, output, name, *, syntax_operator_id=CATALOGUE[0]):
    if name == syntax_operator_id and request["kind"] == "syntax":
...
def catalogue(runtime, query_id, request, checks, fault=None, *, syntax_operator_id=CATALOGUE[0]):
    result = []
    for name, root in zip((syntax_operator_id, CATALOGUE[1]), (MODEL, CLIA)):
        def backend(ks, operator_id, context, name=name):
            if name == syntax_operator_id and request["kind"] == "syntax":
                if syntax_operator_id != CATALOGUE[0]:
                    return {"status": "CANNOT_CHECK", "reason": "HOST_DONOR_UNBOUND"}
        ...
        def checker(output, name=name):
            receipt = (check(runtime, request, output, name) if syntax_operator_id == CATALOGUE[0]
                       else check(runtime, request, output, name, syntax_operator_id=syntax_operator_id))
...
def query(runtime, request, fault=None, *, catalogue_builder=None, syntax_operator_id=CATALOGUE[0]):
    start = time.perf_counter()
    fixture = identities(syntax_operator_id)
    try:
    ...
    checks = []
    operators = (catalogue(runtime, qid, request, checks, fault, syntax_operator_id=syntax_operator_id)
                 if catalogue_builder is None else catalogue_builder(runtime, qid, request, checks, fault))
    ...
    if SV.committed(outcome) and pure and identities(syntax_operator_id) == fixture:
        operator, output, _ = outcome.candidate
        selected = operator.operator_id
        receipt = (check(runtime, request, output, selected) if syntax_operator_id == CATALOGUE[0]
                   else check(runtime, request, output, selected, syntax_operator_id=syntax_operator_id))
        checks.append({"operator": selected, "phase": "admission", **receipt})
        support = (qid, MODEL if selected == syntax_operator_id else CLIA)
        truth = warrant(runtime, support)
        if receipt["status"] == "PASS" and truth.liveness(runtime.state.revoked) is Liveness.LIVE:
            syntax = selected == syntax_operator_id

...
def main(argv=None) -> int:
    ...
    rec = {"receipt": "N1_UD_INDUCTION_V1", "dataset": "UD_English-EWT r2.14 (custody manifest docs/provenance/UD_EWT_CUSTODY_MANIFEST_V1.json)", "files_sha256": {s: UD.digest_of(p) for s, p in files.items()},
           ...
           "ud_parse": {"meta": parse_meta, "protected": parse, "protected_chart": parse_chart, "protected_chart_gaps": parse_chart_gaps},
    ...
    print(json.dumps({"train": {k: rec["train_induction"][k] for k in ("sentences", "tokens", "lexemes", "singletons", "irregular_past_exceptions", "irregular_present_exceptions", "skeleton_families")}, "coverage": {s: {k: v for k, v in c.items() if k in ("token_coverage", "sentence_lexical_coverage", "skeleton_coverage", "cannot_check")} for s, c in rec["coverage"].items()}, "simple_clauses": {s: {k: v for k, v in c.items() if k != "misses_sample"} for s, c in interp.items()}, "misses_test": interp["test"]["misses_sample"][:6], "grammar_train": {k: v for k, v in gram.receipt().items() if k != "top_rules"}, "grammar_protected": {s: {m: {k: v for k, v in r.items() if k != "missing_rules_top"} for m, r in e.items()} for s, e in grammar_eval.items()}, "parse": {"meta": parse_meta, **{s: {k: v for k, v in r.items() if k != "misses_sample"} for s, r in parse.items()}, "misses_test": parse["test"].get("misses_sample", [])[:5]}, "parse_chart": {s: {k: v for k, v in r.items() if k != "misses_sample"} for s, r in parse_chart.items()}, "parse_chart_gaps": {s: {k: v for k, v in r.items() if k != "misses_sample"} for s, r in parse_chart_gaps.items()}, "wall_s": rec["wall_s"]}, indent=1))
    return 0

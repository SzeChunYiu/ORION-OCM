...
class SemanticDiscrepancy(ValueError): pass

SEMANTIC = {"STATE_OR_AUTHORITY_DRIFT","ORIGINAL_ANSWER_NOT_COMMITTED","NO_EXACT_VALUE","APPLICATION_IDENTITY",
    "PROGRAM_AUTHORITY","WRONG_PUBLIC_MAX_VALUE","KERNEL_INPUT_DRIFT","SEED_DRIFT","NONRATIONAL_FINE_OUTPUT",
    "NONCANONICAL_RATIONAL","ACTUAL_CALL_COUNTS","POINTWISE_AUTHORITY","NATIVE_EVENT_BINDING",
    "PREPARATION_AUTHORITY","PREPARATION_INPUT_DRIFT","WRONG_HOST_PROGRAMS","REPEAT_SEMANTIC_DRIFT"}

def need(condition, reason):
    if not condition: raise (SemanticDiscrepancy if reason in SEMANTIC else ValueError)(reason)
...
def origin_check(record, manifest, arm, final):
    ...
    required = {"ocm.runtime.solve", "ocm.kso.navigation", "g1_vessel", "clia_reuse_vessel", "clia_reuse_apply", "vendor.conll18_ud_eval"}
...
def adjudicate(observations, assignments, errors, discrepancies=()):
    complete=len(observations); pairs=[]; mismatches=[]; discrepancies=list(discrepancies)
    for pair in range(7):
    ...
    discrepancies += [{"pair":p,"reason":"PAIRED_SEMANTIC_DISCREPANCY"} for p in mismatches]
    cost_complete=not errors and complete==14 and len(pairs)==7
    function="REFINE_REQUIRED" if discrepancies else "CANNOT_CHECK" if not cost_complete else "EXACT_FUNCTIONAL_PARITY"
    aggregate=cost_complete and not discrepancies
    warm=median(Fraction(p["warm_exact"]) for p in pairs) if aggregate else None
    total=median(Fraction(p["total_exact"]) for p in pairs) if aggregate else None
    gate=function
    if aggregate:
        gate="ADOPTION_CANDIDATE" if warm<=Fraction(4,5) and total<1 else "WARM_GAIN_ONLY_COLD_COST_UNRESOLVED" if warm<1 and total>=1 else "NO_PRACTICAL_GAIN"
    return {"assigned_processes":14,"assigned_calls":28,"qualified_processes":complete,"qualified_calls":2*complete,
            "functional_terminal":function,"component_gate":gate,"cost_evidence_complete":cost_complete,
...
def grade(root, expected_manifest_sha256):
    ...
    observations={}; errors=[]; discrepancies=[]; pids=[]
    for a,report in zip(m["assignments"],receipt["reports"],strict=True):
        try:
            ...
            need(worker["case_sha256"]==sha(root/(prefix+"/input.json")),"WORKER_INPUT")
            if worker.get("status")=="ERROR":
                if "manifest_sha256" in worker: need(worker["manifest_sha256"]==expected_manifest_sha256,"WORKER_INPUT")
                errors.append({"case":a["index"],"reason":"WORKER_ERROR","worker_error":{k:worker.get(k) for k in ("type","message","pid")},
                               "manifest_binding_present":"manifest_sha256" in worker})
                continue
            need(worker["manifest_sha256"]==expected_manifest_sha256,"WORKER_INPUT")
        ...
        except SemanticDiscrepancy as exc: discrepancies.append({"case":a["index"],"reason":str(exc)})
    ...
    return adjudicate(observations,m["assignments"],errors,discrepancies)
...
def main():
    ...
    return 2 if result["functional_terminal"]=="CANNOT_CHECK" else 1 if result["functional_terminal"]=="REFINE_REQUIRED" else 0

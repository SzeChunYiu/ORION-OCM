...
def main():
    ...
    try:
        ...
        traceback.print_exc()
        record = {"status": "ERROR", "type": type(exc).__name__, "message": str(exc),
                  "pid": os.getpid(), "case_sha256": sha(args.case), "assigned_calls": 2, "consumer_scope": "DIRECT_SV_SOLVE_WITH_COMMITMENT_DECISION; no durable query/result append; binder persistence included"}
        status = 2

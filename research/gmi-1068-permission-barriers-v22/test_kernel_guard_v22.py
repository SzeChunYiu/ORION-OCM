...
import check_lean_v22 as kernel
...
class KernelGuardTests(unittest.TestCase):
    def test_registered_statement_types_reject_source_valid_mutants(self):
        ...
        changes = [
            [("ProofTargetsV22.lean",
              r"theorem gated_success_contract[\s\S]*?(?=\ntheorem context_incidence_contract)",
              "theorem gated_success_contract : True := True.intro\n")],
            [("ProofTargetsV22.lean",
              r"theorem context_incidence_contract[\s\S]*?(?=\ntheorem minimal_blocker_contract)",
              "theorem context_incidence_contract : True := True.intro\n")],
            [("ProofTargetsV22.lean",
              r"theorem minimal_blocker_contract[\s\S]*?(?=\nend ProofTargetsV22)",
              "theorem minimal_blocker_contract : True := True.intro\n")],
        ]
        ...
        for index, mutation in enumerate(mutations):
            with self.subTest(mutation=index), tempfile.TemporaryDirectory(prefix="v22-kernel-hostile-") as directory:
                root = Path(directory)

...
**which quantity predicts the selected model class?** (the rows' own word
for a model class is `morphology`)

**The section map was wrong about this.** It says these rows need "no new
mathematics", only the registered model selection receipts pinned by blob sha.
The five parent model-class receipts on `main` contain no information-theoretic or
...
**Harness.** `X = {0,1}^3` uniform; five architecture-name-free model classes
...
| result | numbers |
|---|---|
| `SWEEP-1` raw information | **4,348** conflicting pairs of **10,292** equal-information world pairs — raw `I(X;Y)` does **not** determine the model selection. Equality decided exactly by `min(k, 8-k)`; no logarithm evaluated |
...
| `SWEEP-4` locality boundary (AE6) | `tau = 1/2` is the exact boundary at which failure of locality displaces every local model class; parity selects `m4_gf2_affine` for `tau in (1/2,1]`, `x0 AND x1` selects `m2_arity2_junta` for `tau in (3/4,1]` |
| `SWEEP-5` intervention (AE13) | Markov-equivalent `A→C` / `C→A`: **identical** observational joints and identical observational model selection `{m1}`; under `do(A)` the selected set differs — `{m1}` versus `NO_VIABLE_MORPHOLOGY` for all `tau in (1/2, 3/4]` |
...
class and scores it by direct summation, re-implements the model selection rule from
...
**Null.** `200` shuffled model selection maps never reach `0` conflicts (range

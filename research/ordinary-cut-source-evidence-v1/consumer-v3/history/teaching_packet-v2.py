...
def validate(base,packet,inventory,release,request,authority):
    ...
    if authority["release"]!=request["release_receipt"] or authority["registry_scope"]!=request["registry_scope"]:
    ...
    for row in roots:
        ...
        if row["trace_disposition"]=="TRACE_UNUSABLE":
            ...
            if contract(row["trace"]["source"])!=p1[row["label"]]:raise ValueError("trace whole contract")
            if any(contract(value)!=p1.get(label) or value["label"]!=label for label,value in row["contracts"].items()):

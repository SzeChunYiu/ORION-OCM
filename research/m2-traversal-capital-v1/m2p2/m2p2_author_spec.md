...
- A **chunk** is a sequence of 2 or 3 operations. A builder **decomposes into chunks**
  if it is an exact end-to-end concatenation of chunks (whole chunks, nothing left
  over; chunks may repeat).

## What a world is

A world hides a **chunk set**: 4 to 8 distinct chunks of your choosing. The members of
...
- `chunks`: 4 to 8 entries, each a list of 2 or 3 op names, all entries distinct
  as sequences. No other constraint — pick them however you like.
- `min_builder_length`: integer 4..8.
...
- 4 <= chunk count <= 8; every chunk length 2 or 3; all chunks distinct.

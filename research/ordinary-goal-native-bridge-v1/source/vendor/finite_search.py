...
def compile_parent(parent,bank,work,limit=200000,syntax_checker=None):
    ...
    for row in parent:
        ...
        for qi,query in enumerate(formulas):
            for mapping in M.match(row["statement"],query,row["floating"],{},work,syntax_checker):
                states=[(mapping,[])]
                    ...
                    for prior,slots in states:
                        if {t for t in h["statement"] if t in variables}<=set(prior):
                            ...
                            for pi,premise in enumerate(formulas):
                                ...
                                for nxt in M.match(h["statement"],premise,row["floating"],prior,work,syntax_checker):
...
def emit(derivation,parent,bank,work,floating_labels=()):
    ...
    for label in proof:
        ...
        elif label in ("cA","cB","cC") or label in floating_labels:

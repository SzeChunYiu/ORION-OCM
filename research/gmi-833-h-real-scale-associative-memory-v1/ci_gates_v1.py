"""CI gates for gmi-833-h-real-scale-associative-memory-v1, kept out of the
...
def closure_consistency(result=None, recon=None):
    result = result or load("RESULT_V1.json")
    recon = recon or load("ISSUE_833_RECONCILIATION_H06_V1.json")
...
def two_route_namespace():
    ...
    import independent_oracle_am_v1  # noqa: F401
...
def selftest():
    ...
    recon = load("ISSUE_833_RECONCILIATION_H06_V1.json")
    checks = []
    ...
    dirty_recon["replacements"].append(
        {"anchor": recon["replacements"][0]["anchor"],
         "old": "- [ ] Finite-state/automata intelligence.",
         "new": "- [x] Finite-state/automata intelligence. — ✅ fabricated"})

...
gmi-833-h-real-scale-associative-memory-v1 (issue #833, section H).
...
loss or claim.
    ...
    python3 -I -B  real_scale_associative_memory_v1.py
    python3 -I -O -B real_scale_associative_memory_v1.py
"""
import json
import os
import random
...
PREDICTED_CLASS = "CUE_ASSOCIATION_FANOUT"
...
def decision(name, ql, fa, in_store, p1, p0, e1, e0):
    ...
    the fit majority (1); the executor asserted the fit multi-element fraction
...
def block_errors(rows, name):
    ...
    for ql, yv, fa, in_store, p1, p0, e1, e0 in rows:
        if decision(name, ql, fa, in_store, p1, p0, e1, e0) != yv:
            e += 1
    return e
...
def check_nulls(rec):
    ...
    ok = (le == label["errors"] and de == design["errors"]
          ...
          and label["seed"] == 20260923 and design["seed"] == 20260924)
...
def check_ladder(rec):
    lad = rec["ladder"]
    errs = lad["errors"]
    ok = (lad["readout"] == "ASSOC>=2" and len(errs) == 8
...
def check_control(rec):
    ...
    ok = (pc["winner"] == "LEN<=7" and pc["assoc2_errors"] == 51702
...
def check_regen(rec):
    ...
    ok = (rg["same_class"] and rg["class"] == PREDICTED_CLASS
          and rg["primary"]["winner"] == "ASSOC>=2"
          and rg["regen"]["winner"] == "ASSOC>=2")
...
def hostile_tampered_replay(rec):
    ...
    before = block_errors(rows, "ASSOC>=2")
    rows[0][1] = 1 - rows[0][1]
    after = block_errors(rows, "ASSOC>=2")
...
def main():
    rec = load("scope_SIGMA_H06R.json")
    ...
    rows["H06"] = {"row": ROW, "sigma": SIGMA, "predicted_class": PREDICTED_CLASS,
    ...
    result = {
        "schema": "GMI833HRealScaleAssociativeMemoryLedgerV1",
        "scope": SIGMA, "row": ROW,
        ...
        "scale": scale,
        "rows_closed": [ROW] if rows["H06"]["complete"] else [],
        "rows_open": [] if rows["H06"]["complete"] else [ROW],
        "rows": rows,
    ...
    print("RESULT_V1.json written: %s supported=%d/11 complete=%s"
          % (SIGMA, supported, rows["H06"]["complete"]))
...
def _evidence(rkey):
    return {
        ...
        "R02": "GRAMMAR_G_AM_REGISTERED_AND_DIGESTED",

"""Route B for gmi-833-h-real-scale-associative-memory-v1: a source-separated
oracle.

This file imports NOTHING from grammar_am_v1, run_real_scale_associative_memory_v1
or real_scale_associative_memory_v1. It re-derives, from FREEZE_V1.md and its
...
test_real_scale_associative_memory_v1.py via an `ast` scan and a `sys.modules`
assertion, not by this comment.

    python3 -I -B  independent_oracle_am_v1.py
"""
import json
import os
import random
...
ALPHABET_WIDTH = 27
SEED_LABEL = 20260923
SEED_DESIGN = 20260924
EXPECT_CLASS = "CUE_ASSOCIATION_FANOUT"
...
def decide(name, ql, fa, in_store, p1, p0, e1, e0):
...
def block_errors(rows, name):
    return sum(1 for ql, yv, fa, in_store, p1, p0, e1, e0 in rows
               if decide(name, ql, fa, in_store, p1, p0, e1, e0) != yv)


def main():
    rec = fetch("scope_SIGMA_H06R.json")
    ...
    checks["holdout_counts"] = bool(
        ...
        and ho["n"] == 97018 and ho["shared"] == 53243 and ho["unique"] == 43775
        and ho["winner"] == "ASSOC>=2" and ho["winner_class"] == EXPECT_CLASS)
    ...
    checks["rank_stage"] = bool(
        rec["rank_stage"]["winner"] == "ASSOC>=2"
        and rec["rank_stage"]["winner_class"] == EXPECT_CLASS
        and rec["rank_stage"]["winner_errors"] == 8600
        and rec["rank_stage"]["majority_errors"] == 91819)
    rg = rec["regen"]
    checks["regeneration_same_class"] = bool(
        rg["same_class"] and rg["class"] == EXPECT_CLASS
        and rg["primary"]["winner_errors"] == 25907
        and rg["regen"]["winner_errors"] == 31249)

    # store ladder: exact, monotone, ends at the held winner
    lad = rec["ladder"]
    errs_l = lad["errors"]
    checks["ladder"] = bool(
        lad["readout"] == "ASSOC>=2" and len(errs_l) == 8
        and all(a >= b for a, b in zip(errs_l, errs_l[1:]))
        and errs_l[-1] == ho["winner_errors"]
        and errs_l == [49200, 41310, 37421, 30716, 25239, 20226, 13755, 1249])
    ...
    checks["control"] = bool(
        pc["winner"] == "LEN<=7" and pc["winner_class"] == "DESCRIPTOR_LENGTH_THRESHOLD"
        and pc["assoc2_errors"] == 51702 and not f1 and pc["control_fires"])
    ...
    out = {
        "schema": "GMI833HRealScaleAssociativeMemoryOracleV1",
        "route": "B", "scope": SIGMA,

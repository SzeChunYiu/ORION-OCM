...
# --- OOP adapters for test suite ---

class ObligationSpace:
    """Binary obligation space O(B) = {0,1}^B."""

    def __init__(self, budget):
        self.budget = int(budget)
        self.size = 1 << self.budget

    def encode(self, features):
        if len(features) != self.budget:
            raise ValueError("feature length must equal budget")
        if any(f not in (0, 1) for f in features):
    ...
    def decode(self, oid):
        if oid < 0 or oid >= self.size:
            raise ValueError("oid out of range")
        out = []
        for i in range(self.budget - 1, -1, -1):
            out.append((oid >> i) & 1)
        return out
...
class _DomainBase:
    ...
    def expressible_set(self):
        _, pred = domain_fn(self.index)
        return frozenset(k for k in range(1 << self.budget) if pred(k))

    def coverage_fraction(self):
        n = len(self.expressible_set())
        return Fraction(n, 1 << self.budget)

    def impossible_sample(self):
        _, pred = domain_fn(self.index)
        for k in range(1 << self.budget):
            if not pred(k):
                return k
        return -1


class D1_CoefficientSystem(_DomainBase):
    index = 1
    name = "D1_Coefficient"


class D2_MemorySystem(_DomainBase):
    index = 2
    name = "D2_Memory"


class D3_AttentionSystem(_DomainBase):
    index = 3
    name = "D3_Attention"


class D4_SymbolicSystem(_DomainBase):
    index = 4
    name = "D4_Symbolic"


class D5_NeuralSystem(_DomainBase):
    index = 5
    name = "D5_Neural"


class D6_DynamicalSystem(_DomainBase):
    index = 6
    name = "D6_Dynamical"


class D7_DistributedSystem(_DomainBase):
    index = 7
    name = "D7_Distributed"


class D8_SelfRewritingSystem(_DomainBase):
    index = 8
    name = "D8_SelfRewriting"
...
class PairwiseSynergy:
    ...
    def synergy(self, di, dj):
        # Structured domains define Di∪Dj as set-union of expressible sets,
        # so true synergistic residues are empty by construction.
        return Fraction(0)
...
class BoundedCompleteness:
    ...
    def full_union_coverage(self):
        union = set()
        for d in self.domains:
            union |= set(d.expressible_set())
        return Fraction(len(union), 1 << self.budget)

    def uncovered_obligations(self):
        union = set()
        for d in self.domains:
            union |= set(d.expressible_set())
        return frozenset(k for k in range(1 << self.budget) if k not in union)


def run_completeness_attack(budget=BUDGET):
    domains = [
        D1_CoefficientSystem(budget),
        D2_MemorySystem(budget),
        D3_AttentionSystem(budget),
        D4_SymbolicSystem(budget),
        D5_NeuralSystem(budget),
        D6_DynamicalSystem(budget),
        D7_DistributedSystem(budget),
        D8_SelfRewritingSystem(budget),
    ]
    coverage_by_domain = {d.name: d.coverage_fraction() for d in domains}
    analyzer = PairwiseSynergy(domains)
    pairwise = {}
    for i, di in enumerate(domains):
        for dj in domains[i + 1:]:
            pairwise[(di.name, dj.name)] = analyzer.synergy(di, dj)
    completeness = BoundedCompleteness(domains, budget)
    full = completeness.full_union_coverage()
    uncovered = completeness.uncovered_obligations()
    impossible = {d.name: d.impossible_sample() for d in domains}
    return {
        "coverage_by_domain": coverage_by_domain,
        "pairwise_synergy": pairwise,
        "full_union_coverage": full,
        "uncovered_count": len(uncovered),
        "impossible_samples": impossible,
    }

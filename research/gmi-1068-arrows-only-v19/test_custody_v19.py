...
import custody_v19 as core
COVERAGE={}


@contextmanager
def overlay(verified):
    with tempfile.TemporaryDirectory(prefix='gmi-v19-custody-') as temporary:
        root=Path(temporary)
...
class CustodyTests(unittest.TestCase):
    def test_real_inherited_inputs_and_tampering(self):
        ...
        files=('research/gmi-1068-reference-envelope-v18/prefix_v18.py',
               'research/gmi-1068-partial-context-v15/context_v15.py',
               'research/gmi-1068-recursive-audit-v18/CURRENT_ACCOUNTING_V18.json',
               'research/gmi-1068-amendment-governance-v16/AMENDMENT_LEDGER_V16.json',
               core.RECEIPTS[0],core.RECEIPTS[1],core.RECEIPTS[2],core.PACKAGE+'/FREEZE_V19.md')
        rejected=0
        for name in files:
            with overlay(valid) as root:
                ...
                rejected+=1
        with overlay(valid) as root:
            target=root/'research/gmi-1068-reference-envelope-v18/prefix_v18.py'
            target.write_bytes(target.read_bytes()+b'\n')
            path=root/core.RECEIPTS[0]
            receipt=json.loads(path.read_text())
            receipt['inputs']['prefix_v18.py']=core.digest(target)
    ...
    def test_unavailable_inputs_and_path_escape(self):
        valid=core.verify(ROOT)
        missing=0
        for name in ('research/gmi-1068-reference-envelope-v18/prefix_v18.py',
                     'research/gmi-1068-partial-context-v15/context_v15.py',
                     core.PACKAGE+'/FREEZE_V19.md',core.RECEIPTS[2]):
            with overlay(valid) as root:
                (root/name).unlink()

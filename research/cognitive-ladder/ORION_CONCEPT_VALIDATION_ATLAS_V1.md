...
| SCOPED_FAILURE_KNOWLEDGE | ADAPT | PARENT_SUFFICIENT | PILOT_IMPLEMENTED_THIS_TRANCHE | P2 primary discriminator |
| SEARCH_MORE_VS_JUMP | ADAPT | PARENT_SUFFICIENT | PILOT_IMPLEMENTED_THIS_TRANCHE | P2 flagship discriminator |
| SPARSE_ACTIVE_SUBSPACE | ADAPT | PARENT_SUFFICIENT | PILOT_IMPLEMENTED_THIS_TRANCHE | P3 primary |
...
*ADAPT* · scientific status **PARENT_SUFFICIENT** · implementation `PILOT_IMPLEMENTED_THIS_TRANCHE` · paper role: P2 primary discriminator
...
**Disposition reason.** SETTLED against parents: the failure-epistemology closure already froze the strongest-parent gap at 0/32 and assigned the mechanism to TMS/nogood plus responsibility. Its OWN stated falsifier is that ORION beats the strong parent on a scientifically meaningful protected coordinate. The pilot is therefore run as an attempt at that falsifier, with expectation PARENT_SUFFICIENT, and is narrowed to the one coordinate the closure did not fix: reopening keyed on assumptions after a regime change.

**Preserved terminals (do not re-litigate).**

- `PARENT_SUFFICIENT (results/FAILURE_PILOT_V1.json): the full-strength nogood parent avoids all 640 avoidable work units and ties on four of seven worlds`
- `ACCUMULATION_RESIDUAL_CONFINED_TO_PROBE_COST (results/DIAGNOSIS_E2_V1.json): with the cause withheld, the governed greedy rule provably reproduces the hand-authored decision tree; the residual is 42 probe units of 573 and is memoisation`
...
- `E4 independent worlds: the saturation-as-authority policy makes 232 false escalations of 2000, confirming that saturation alone must not authorise a jump`
...
*ADAPT* · scientific status **PARENT_SUFFICIENT** · implementation `PILOT_IMPLEMENTED_THIS_TRANCHE` · paper role: P2 flagship discriminator
...
*ADAPT* · scientific status **PARENT_SUFFICIENT** · implementation `PILOT_IMPLEMENTED_THIS_TRANCHE` · paper role: P3 primary
...
**Disposition reason.** SETTLED against parents: reframed around the crossover query count, which is the only form of the claim that survives the index-construction hostile

**Preserved terminals (do not re-litigate).**

- `PARENT_SUFFICIENT (results/SCALING_PILOT_V1.json): an ordinary index matches k, k/N, query work and correctness exactly at every scale under a supplied family key`
- `INDEX_MAINTENANCE_DOMINATES (results/SUBSPACE_E1_V1.json): with the key withheld, a hand-specified prefix beats the discovering arm wherever it stays adequate, and the discovering arm's index bill is not repaid at 30x`
...
- `LEAVE_ONE_OUT_DISCOVERY_INCOMPLETE (results/DEPEND_E3_V1.json): with the graph withheld, 16 to 25 per cent of methods carry support no single-element ablation can see, and the cheap parents win on total work by two to four orders of magnitude`

# GMI #833 finite history, switching and hysteresis choice v1
...
This tranche extends the finite architecture-choice correspondence from #893 by making one piece of developmental history explicit: the previously selected candidate architecture. It proves exact finite switching/hysteresis and history-erasure laws. It does not claim a stochastic or real-world migration theory.

## 1. Registered history-conditioned choice

Fix a finite architecture-name-free candidate-architecture set `M`. At the registered current ecology, candidate architecture `m` has exact nonnegative base cost `c(m)`. The previous candidate architecture is `h in M`. Switching/migration burden is an exact nonnegative matrix

`K : M x M -> Q_{>=0}`.

The current choice correspondence is

`Sel(h) = argmin_{m in M} [ c(m) + K(h,m) ]`.

Tie sets are preserved exactly.

Define **history-dependent current candidate architecture** iff there exist `h1,h2` such that
...
The criterion above is definitional but operationally important: two histories matter scientifically only when they alter the exact current choice set under the same instantaneous ecology/resources.
...
Thus identical instantaneous costs can yield a different observed candidate architecture solely because transition burden retains the previous form.
...
Therefore current choice is history-independent.
...
Fix candidate `m*`. If for every previous candidate architecture `h` and rival `n != m*`,
...
The receipt enumerates 13,824 exact three-architecture contexts formed from base costs in `{0,1,2}^3` and binary switching matrices. It finds 3,735 candidate/context margin certificates and verifies every one produces the predicted history-independent singleton choice.
...
Suppose a registered migration/reset intervention maps every previous candidate architecture to one canonical preselection state `h0`. Choice after reset is then, by construction,

`Sel_reset(h)=Sel(h0)`

for every original `h`. Hence the previous candidate-architecture identity no longer affects the current choice correspondence.
...
The executor rejects malformed candidate-architecture sets, missing base/switching entries, negative base cost, negative switching cost and unknown previous/reset candidate architectures.

History-dependence and history-erasure are exact scoped predicates. No result here implies that history is globally irrelevant outside the registered state summary. A richer non-Markov history may contain information not represented by the single previous-architecture variable.
...
The GMI residual is the architecture-name-free integration into the architecture-choice programme, exact machine-readable claim boundaries, and connection to architecture-prior-free and reachable-architecture derivation.

## 9. Forbidden promotions

This result does not establish stochastic switching dynamics, endogenous learning of switching costs, general non-Markov history compression, real-world migration calibration, universal hysteresis, complete architecture dynamics, or complete GMI.

...
class CustodyTests(unittest.TestCase):
    def setUp(self):
        self.tmp=tempfile.TemporaryDirectory(prefix='hcr-custody-')
    ...
    def test_numeric_change_with_pass_status_refused(self):
        _,expected=c.verify(self.root);bad=json.loads(expected);bad['hierarchy']['first_gain']+=1
    ...
    def test_original_payload_field_projection_refused(self):
        ...
        del bad['original_full']['results']['hierarchy_witness']['original_payload']['candidates']
        with patch.object(c,'HERE',self.root),patch('check_v1.run',return_value=bad):
            with self.assertRaisesRegex(ValueError,'payload mismatch'):c.replay(self.root)
    ...
    def test_changed_raw_source_refused(self):
        binding,_=verified_sources(self.root);p=self.root/binding['files'][0]['copy']

"""Close exactly three reviewed optional-structure atoms; enforce complete adjudication records."""
import hashlib
...
ROOT = HERE.parents[1]
BASE = ROOT / "research/gmi-1068-recursive-audit-v12/SCOPE_SNAPSHOT_V12.json"
BASE_SHA = "1b6eed814493b0dfc8914bdb102e1f4c4494cff34db16ba7b991adcfa636846f"
...
CONTRACT = load("audit_contract_v13", HERE / "audit_contract_v13.py")
CLOSING = set(CONTRACT.ATOM_SPECS)


def evaluate(current=None, receipt=None):
    ...
    need(hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA, "V12 baseline drift")
    before = json.loads(BASE.read_text())
    if current is None:
        current = json.loads((HERE / "SCOPE_SNAPSHOT_V13.json").read_text())
    if receipt is None:
        receipt = json.loads((ROOT / CONTRACT.PACKAGE / "RESULT_V13.json").read_text())
    ...
    need(result["registered_atoms"] == 222 and result["unresolved_atoms"] == 207,
    ...
    need(len(scientific) == 7, "scientific closure count drift")

"""Actual two-closure successor plus rehashed scope, closure and custody attacks."""
...
spec=importlib.util.spec_from_file_location('checked_snapshot_v25',HERE/'check_snapshot_v25.py')
...
class SnapshotTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.snapshot=json.loads((HERE/'SCOPE_SNAPSHOT_V25.json').read_text())
        cls.receipt=json.loads((checker.ROOT/checker.CONTRACT.PACKAGE/'RESULT_V25.json').read_text())
    ...
    def test_actual_snapshot_no_alarm(self):
        ...
        self.assertEqual(len(result['scientific_atoms_closed']),24)
        self.assertEqual((result['original_fulfilled'],result['remaining_atoms'],
                          result['qualified_replacements'],result['active_unresolved']),(32,190,2,188))
    ...
    def test_rehashed_snapshot_mutations(self):
        ...
        wrong=checker.CONTRACT.witness(checker.ROOT,'oracle_v25.py','python_symbol','raw')
        changes=[(('source_issue',),1068.0),(('source_issue',),True),(('observed_main',),'0'*40),(('audited_base',),'0'*40),
            ...
            (repair('R1','id'),'unregistered-repair'),(repair('R1','evidence'),[wrong]),
            (atom('R1',0,'status'),'UNKNOWN'),(atom('R1',5,'status'),'CLOSED'),
        ...
        changes.append((atom('R1',8,'status'),'CLOSED'))
        invalid=[]
        ...
        bad=deepcopy(self.snapshot);bad['rounds']['R1']['artifacts']=[a for a in bad['rounds']['R1']['artifacts']
            if a['path']!=checker.CONTRACT.PACKAGE+'/FREEZE_V25.md'];invalid.append(bad)
        bad=deepcopy(self.snapshot);path=HERE/'audit_contract_v25.py'
        bad['rounds']['R1']['artifacts'].append({'path':str(path.relative_to(checker.ROOT)),
    ...
    def test_receipt_mutations(self):
        ...
        coverage_key=next(iter(self.receipt['coverage']['test_tables_v25']))
        ...
        changes=[(('atoms',),{'GMI2-R1-001':{'status':'VERIFIED_AT_REGISTERED_SCOPE'}}),
            (('registered_results','Y1','scope'),'all process ontologies have the same unique primitive fields'),
            (('registered_results','Y2','status'),'PENDING_REVIEW'),
            ...
            (('coverage','test_tables_v25',coverage_key),True),
            (('coverage','test_tables_v25',coverage_key),float(self.receipt['coverage']['test_tables_v25'][coverage_key])),
        ...
        for value in ({}, {'original_survivors':[],'reconciled_rows':6},
                      {'original_survivors':self.receipt['survivor_reconciliation']['original_survivors'],'reconciled_rows':True},
                      {'original_survivors':self.receipt['survivor_reconciliation']['original_survivors'],'reconciled_rows':6.0}):
            changes.append((('survivor_reconciliation',),value))
        ...
        for path in ((),('kernel',),('source_bindings',),('coverage','test_tables_v25'),('inputs',),('registered_results',)):
        ...
        self.assertEqual(len(invalid),73)
    ...
    def test_coupled_result_scope_and_original_promotion(self):
        ...
        for result in checker.CONTRACT.RESULTS:
            ...
            claim='All process ontologies are absolutely minimal.'
            ...
            count+=1
        snapshot,receipt=deepcopy(self.snapshot),deepcopy(self.receipt)
        atom=snapshot['rounds']['R1']['obligations'][5];atom['status']='CLOSED'
        ...
        for atom_id in checker.CONTRACT.ATOM_SPECS:
            snapshot,receipt=deepcopy(self.snapshot),deepcopy(self.receipt)
            atom=next(row for row in snapshot['rounds']['R1']['obligations'] if row['id']==atom_id)
            atom['disposition']='Six independently stored fields are uniquely necessary without observer premises.'
    ...
    def test_current_accounting_mutations(self):
        ...
        for path,value in [(('original_fulfilled',),33),(('original_unresolved',),189),
                (('active_unresolved',),187),(('qualified_replacements',),3),(('original_total',),222.0),
        ...
        invalid.append(json.loads((checker.BASE.parent/'CURRENT_ACCOUNTING_V24.json').read_text()))

"""Close only the registered R1-001/007 guarded-foundation atoms; preserve every other original record; enforce complete adjudication records."""
import hashlib
...
ROOT = HERE.parents[1]
BASE = ROOT / "research/gmi-1068-recursive-audit-v24/SCOPE_SNAPSHOT_V24.json"
BASE_SHA = "710cd59c62662e337e50a364bded34bc3936a87e356a30f7df8bc13090e544e8"
...
CONTRACT = load("audit_contract_v25", HERE / "audit_contract_v25.py")
...
def evaluate(current=None, receipt=None, accounting=None):
    ...
    need(hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA, "V24 baseline drift")
    before = json.loads(BASE.read_text())
    if current is None:
        current = json.loads((HERE / "SCOPE_SNAPSHOT_V25.json").read_text())
    if receipt is None:
        receipt = json.loads((ROOT / CONTRACT.PACKAGE / "RESULT_V25.json").read_text())
    need(type(current) is dict and set(current) == set(before), "snapshot top-level fields")
    for field in before:
        if field != "rounds":
            expected = "89252791e2050ca83640d44c576517fcea980d29" if field in ("observed_main", "audited_base") else before[field]
    ...
    need(result["registered_atoms"] == 222 and result["unresolved_atoms"] == 190,
    ...
    need(len(scientific) == 24, "scientific closure count drift")
    expected_accounting = CONTRACT.current_accounting(current, receipt["amendment_carry"])
    if accounting is None:
        accounting = json.loads((HERE / "CURRENT_ACCOUNTING_V25.json").read_text())
    exact(accounting, expected_accounting, "current accounting drift")
    for key, value in {"original_total": 222, "original_fulfilled": 32, "original_unresolved": 190,
                       "qualified_replacements": 2, "active_unresolved": 188}.items():

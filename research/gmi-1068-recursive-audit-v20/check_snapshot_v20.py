"""Close only original R3-004 while preserving every other original record; enforce complete adjudication records."""
import hashlib
...
ROOT = HERE.parents[1]
BASE = ROOT / "research/gmi-1068-recursive-audit-v19/SCOPE_SNAPSHOT_V19.json"
BASE_SHA = "b1d00cd9fc7e7de1be9a76f4c17633b3d907e9b66e00171653bbc5558e15fc92"
...
CONTRACT = load("audit_contract_v20", HERE / "audit_contract_v20.py")
...
def evaluate(current=None, receipt=None, accounting=None):
    ...
    need(hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA, "V19 baseline drift")
    before = json.loads(BASE.read_text())
    if current is None:
        current = json.loads((HERE / "SCOPE_SNAPSHOT_V20.json").read_text())
    if receipt is None:
        receipt = json.loads((ROOT / CONTRACT.PACKAGE / "RESULT_V20.json").read_text())
    ...
    need(result["registered_atoms"] == 222 and result["unresolved_atoms"] == 201,
    ...
    need(len(scientific) == 13, "scientific closure count drift")
    expected_accounting = CONTRACT.current_accounting(current, receipt["amendment_carry"])
    if accounting is None:
        accounting = json.loads((HERE / "CURRENT_ACCOUNTING_V20.json").read_text())
    exact(accounting, expected_accounting, "current accounting drift")
    for key, value in {"original_total": 222, "original_fulfilled": 21, "original_unresolved": 201,
                       "qualified_replacements": 2, "active_unresolved": 199}.items():

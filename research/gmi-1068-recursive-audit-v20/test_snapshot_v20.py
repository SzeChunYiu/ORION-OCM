...
spec=importlib.util.spec_from_file_location('checked_snapshot_v20',HERE/'check_snapshot_v20.py')
...
class SnapshotTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.snapshot=json.loads((HERE/'SCOPE_SNAPSHOT_V20.json').read_text())
        cls.receipt=json.loads((checker.ROOT/checker.CONTRACT.PACKAGE/'RESULT_V20.json').read_text())
    ...
    def test_actual_snapshot_no_alarm(self):
        ...
        self.assertEqual(result['new_scientific_atoms_closed'],['GMI2-R3-004'])
        self.assertEqual(len(result['scientific_atoms_closed']),13)
        self.assertEqual((result['original_fulfilled'],result['remaining_atoms'],
                          result['qualified_replacements'],result['active_unresolved']),(21,201,2,199))
    ...
    def test_rehashed_snapshot_mutations(self):
        ...
        wrong=checker.CONTRACT.witness(checker.ROOT,'FrontierOrderV20.lean','lean_declaration','frontier_empty')
        changes=[(repair('R14','scope'),'all theories are complete'),(repair('R15','status'),'PENDING_REVIEW'),
            ...
            (atom('R3',3,'evidence'),[wrong]),(atom('R3',0,'status'),'CLOSED'),
        ...
        bad=deepcopy(self.snapshot);bad['rounds']['R3']['artifacts']=[a for a in bad['rounds']['R3']['artifacts']
            if a['path']!=checker.CONTRACT.PACKAGE+'/FREEZE_V20.md'];invalid.append(bad)
        bad=deepcopy(self.snapshot);path=HERE/'audit_contract_v20.py'
        ...
        self.assertEqual(len(invalid),27)
    ...
    def test_receipt_mutations(self):
        ...
        changes=[(('atoms',),{'GMI2-R1-001':{'status':'VERIFIED_AT_REGISTERED_SCOPE'}}),
            (('registered_results','T1','scope'),'all infinite frontiers are finite'),
            (('registered_results','T2','status'),'PENDING_REVIEW'),
            (('atoms','GMI2-R3-004','title'),'all theories complete'),
            (('atoms','GMI2-R3-004','scope'),'unconditional infinite frontier'),
            (('atoms','GMI2-R3-004','status'),'PENDING_REVIEW'),
            ...
            (('coverage','test_simulation_v20','simulation_machines'),True),
            (('coverage','test_simulation_v20','simulation_machines'),119113.0),
            ...
            (('tests_run',),True),(('tests_run',),14.0),(('tests_run',),13),
        ...
        for path in ((),('kernel',),('source_bindings',),('coverage','test_simulation_v20'),('inputs',),('registered_results',)):
        ...
        self.assertEqual(len(invalid),63)
    ...
    def test_coupled_result_scope_and_original_promotion(self):
        ...
        for result in checker.CONTRACT.RESULTS:
            ...
            claim='Every current-value frontier safely preserves all nondeterministic continuations.'
    ...
    def test_current_accounting_mutations(self):
        ...
        for path,value in [(('original_fulfilled',),22),(('original_unresolved',),200),
                (('active_unresolved',),198),(('qualified_replacements',),3),(('original_total',),222.0),
        ...
        invalid.append(json.loads((checker.BASE.parent/'CURRENT_ACCOUNTING_V19.json').read_text()))

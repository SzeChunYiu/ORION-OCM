...
exec(compile((HERE / "check.py").read_bytes(), str(HERE / "check.py"), "exec"), C.__dict__)
...
class BindingChecks(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.original = json.loads((HERE / "MANIFEST_V2.json").read_text())
...
class ReplayControls(unittest.TestCase):
    ...
    def test_replay_compares_full_payload_and_scalar_types(self):
        keys = ("schema", "status", "anchors", "bindings", "checks",
                "package_inventory", "python", "authority", "loaded_runtime_sources")
        recorded = {k: {} for k in keys}
        recorded["checks"] = {"count": 1, "passed": True}
        C.compare_recorded(recorded, copy.deepcopy(recorded))
        for value in ({"count": True, "passed": True}, {"count": 1.0, "passed": True},
                      {"count": 1, "passed": 1}, {"count": 1}):
            bad = copy.deepcopy(recorded)
            bad["checks"] = value
            with self.assertRaises(C.CheckError):
                C.compare_recorded(recorded, bad)
        for key in keys:
            bad = copy.deepcopy(recorded)
            del bad[key]
            with self.assertRaises(C.CheckError):
    ...
    def test_only_python_execution_description_may_differ(self):
        keys = ("schema", "status", "anchors", "bindings", "checks",
                "package_inventory", "python", "authority", "loaded_runtime_sources")
        a = {k: {} for k in keys}
        b = copy.deepcopy(a)
        b["python"] = "3.12 patch metadata"
        C.compare_recorded(a, b)
        b["unexpected"] = True
        with self.assertRaises(C.CheckError):
            C.compare_recorded(a, b)

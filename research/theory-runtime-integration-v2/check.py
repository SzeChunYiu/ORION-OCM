...
ANCHORS = {
    ...
    "runtime": "7f8488edd77992ba0313fa9457294ed7bbc623cf",
}
...
def strict_json(text):
    def pairs(items):
        ...
        for key, value in items:
            if key in result:
                raise CheckError("REJECTED", "Duplicate JSON key: " + key)
            result[key] = value
        return result

    def constant(value):
        raise CheckError("REJECTED", "Non-finite JSON constant")

    try:
        value = json.loads(text, object_pairs_hook=pairs, parse_constant=constant)
        json.dumps(value, allow_nan=False)
    except (ValueError, TypeError) as exc:
        raise CheckError("REJECTED", "Invalid finite JSON") from exc
    return value


def compare_recorded(recorded, actual):
    # Every deterministic field is compared. Only the interpreter description is
    # execution metadata; supported CPython 3.12 is independently required below.
    expected_keys = {"schema", "status", "anchors", "bindings", "checks",
                     "package_inventory", "python", "authority", "loaded_runtime_sources"}
    require(type(recorded) is dict and type(actual) is dict
            and set(recorded) == set(actual) == expected_keys, "Replay shape changed")
    def stable(data):
        return json.dumps({k: v for k, v in data.items() if k != "python"},
                          sort_keys=True, separators=(",", ":"), allow_nan=False)
    require(stable(recorded) == stable(actual), "Complete deterministic replay differs")
...
def verify_bindings(manifest, roots, revoked=()):
    ...
    if (type(manifest) is not dict
            ...
            or manifest["schema"] != "ocm.theory-runtime-engineering.v2"
...
def finite_checks(roots):
    ...
    # V1 exercised only the first training support. Each essential support must
    # independently disable the persisted generator while leaving its history.
    from ocm.runtime.ocm_runtime import OCMRuntime
    lifecycle_controls = []
    for support_index in range(3):
        with tempfile.TemporaryDirectory(prefix="ocm-all-generator-supports-") as directory:
            runtime = OCMRuntime(Path(directory))
            train = (M.PolynomialTask("shift-square-increment", (2, 2, 1)),
                     M.PolynomialTask("shift-square-double", (2, 4, 2)))
            held = (M.PolynomialTask("shift-square-decrement", (0, 2, 1)),
                    M.PolynomialTask("fourth-power-of-shift", (1, 4, 6, 4, 1)))
            receipt = M.admit_generator(runtime, tuple((t, M.solve(t)) for t in train), held)
            generator = receipt["generator_id"]
            restored = OCMRuntime(Path(directory))
            before = M.load_generator(restored, generator).fingerprint
            evidence = [r["evidence_id"] for r in receipt["training"]] + [receipt["validation_evidence"]]
            prefix = tuple(e.event_hash for e in restored.events)
            restored.revoke([evidence[support_index]])
            reopened = OCMRuntime(Path(directory))
            require(generator in reopened.state.ks.atom_map(), "Revocation deleted generator history")
            require(tuple(e.event_hash for e in reopened.events[:len(prefix)]) == prefix,
                    "Revocation rewrote prior ledger history")
            try:
                M.load_generator(reopened, generator)
            except ValueError:
                refused = True
            else:
                refused = False
            require(refused, "An essential generator support failed to revoke reuse")
            lifecycle_controls.append({"support": "training_" + str(support_index)
                                       if support_index < 2 else "holdout_validation",
                                       "loaded_before_revocation": bool(before),
                                       "refused_after_restart": refused,
    ...
    return {"theory_calibration": source_result,
            ...
            "M3_all_support_revocations": lifecycle_controls,
            "limits": "Finite engineering checks; shared premises are not independently validated; no protected study."}


def loaded_runtime_sources(clean, manifest):
    root = Path(clean["runtime"]).resolve()
    registered = manifest["repositories"]["runtime"]["files"]
    result = {}
    for name, module in sorted(sys.modules.items()):
        if not (name == "ocm" or name.startswith("ocm.")):
            continue
        path = getattr(module, "__file__", None)
        require(path is not None, "Unbound runtime namespace module: " + name)
        path = Path(path).resolve()
        require(path.is_relative_to(root) and path.suffix == ".py",
                "Runtime module imported outside verified private source: " + name)
        rel = path.relative_to(root).as_posix()
        require(rel in registered and digest(path.read_bytes()) == registered[rel]["sha256"],
                "Loaded runtime source drift: " + name)
        result[name] = {"path": rel, "sha256": registered[rel]["sha256"]}
    require(bool(result), "No runtime sources observed")
    return result


def package_inventory():
    names = ("check.py", "test_bindings.py", "MANIFEST_V2.json", "README.md",
             "REVIEW_V2.md", "RUNTIME_DELTA_V2.json")
    result = {name: digest((HERE / name).read_bytes()) for name in names}
    workflow = ".github/workflows/theory-runtime-integration-v2.yml"
    result[workflow] = digest((HERE.parents[1] / workflow).read_bytes())
    return result
...
def main(argv=None):
    ...
    parser.add_argument("--compare", type=Path, help="Compare all deterministic fields with a reviewed replay")
    parser.add_argument("--revoke-row", action="append", default=[])
    args = parser.parse_args(argv)
    roots = {"theory": args.theory, "runtime": args.runtime}
    try:
        if sys.implementation.name != "cpython" or sys.version_info[:2] != (3, 12):
            raise CheckError("CANNOT_CHECK", "Registered interpreter is CPython 3.12")
        package_before = package_inventory()
        manifest = strict_json((HERE / "MANIFEST_V2.json").read_text())
        ...
        with tempfile.TemporaryDirectory(prefix="ocm-bound-source-") as directory:
            ...
            loaded = loaded_runtime_sources(clean, manifest)
        ...
        result = {"schema": "ocm.theory-runtime-replay.v2",
                  ...
                  "authority": dict(AUTHORITY), "loaded_runtime_sources": loaded}
        if args.compare:
            compare_recorded(strict_json(args.compare.read_text()), result)
        code = 0

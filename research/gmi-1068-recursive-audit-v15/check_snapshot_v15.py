"""Close exactly two reviewed partial-context atoms; enforce complete adjudication records."""
import hashlib
...
ROOT = HERE.parents[1]
BASE = ROOT / "research/gmi-1068-recursive-audit-v14/SCOPE_SNAPSHOT_V14.json"
BASE_SHA = "f692d247f459ccd0d4b51b542f856450d043203c078cd7922e1539335776c7af"
...
CONTRACT = load("audit_contract_v15", HERE / "audit_contract_v15.py")
...
def evaluate(current=None, receipt=None):
    ...
    need(hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA, "V14 baseline drift")
    before = json.loads(BASE.read_text())
    if current is None:
        current = json.loads((HERE / "SCOPE_SNAPSHOT_V15.json").read_text())
    if receipt is None:
        receipt = json.loads((ROOT / CONTRACT.PACKAGE / "RESULT_V15.json").read_text())
    ...
    need(result["registered_atoms"] == 222 and result["unresolved_atoms"] == 204,
    ...
    need(len(scientific) == 10, "scientific closure count drift")

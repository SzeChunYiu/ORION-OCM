...
def test_default_search_path_is_byte_identical(monkeypatch):
    # Historical default-path digest retains the pre-adjoint-repair runtime.
...
def test_trace_does_not_change_the_search(monkeypatch):
    # Historical default-path digest retains the pre-adjoint-repair runtime.
    from gmi_microscope.historical_vm_v1 import historical_vm
    historical = historical_vm()
    monkeypatch.setattr(b1, "VM", historical.VM)

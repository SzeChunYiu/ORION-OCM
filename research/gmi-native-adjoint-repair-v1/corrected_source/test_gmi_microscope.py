...
def test_vm_zoo_deterministic_and_remint_invariant_response():
    """Historical R1/R4 goldens retain the frozen pre-adjoint-repair VM source."""
    from gmi_microscope import smooth, morph, zoo
    from gmi_microscope.historical_vm_v1 import historical_vm
    VMRow = historical_vm().VMRow
...
def test_b0_equivalence_receipt_reproduces(tmp_path, monkeypatch):
    """B0: implementation-equivalent rewrites merge, collision pairs split, init audited; the receipt is deterministic."""
    from gmi_microscope import equiv, ecology
    from gmi_microscope.historical_vm_v1 import historical_vm
    historical = historical_vm()
    monkeypatch.setattr(ecology, "VM", historical.VM)
    monkeypatch.setattr(ecology, "lifecycle_vector", historical.lifecycle_vector)
...
def test_g8_size_census_replays_small_sizes_and_its_witnesses(monkeypatch):
    ...
    from gmi_microscope.historical_vm_v1 import historical_vm
    monkeypatch.setattr(size_census, "VM", historical_vm().VM)
...
def test_r10_invasion_cells_replay_and_competition_is_not_independent_scoring(monkeypatch):
    ...
    from gmi_microscope.historical_vm_v1 import historical_vm
    historical = historical_vm()
    monkeypatch.setattr(invasion, "VM", historical.VM)
    monkeypatch.setattr(invasion, "lifecycle_vector", historical.lifecycle_vector)
...
def test_r11_biosphere_episode_is_deterministic_resumable_and_cites_its_screen_audit(monkeypatch):
    ...
    from gmi_microscope import biosphere, b1
    from gmi_microscope.historical_vm_v1 import historical_vm
    historical = historical_vm()
    monkeypatch.setattr(b1, "VM", historical.VM)
    monkeypatch.setattr(b1, "VMRow", historical.VMRow)

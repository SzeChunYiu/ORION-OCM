#!/usr/bin/env python3
"""Host-portable replication instrument for exact parity-3, frozen V6.

The preregistration is NN_NONNN_POINT_PARITY3_PREREG_V6.json. Standard library
only. Emits a complete JSON measurement packet.

V6 repairs the interpreter scope of the V5 opcode instrument. On CPython 3.13
the first-ever trace of a code object loses its first frame's opcode events,
which V5's per-frame validation correctly refuses, so 3.13 could not be
measured at all. The cause is specific and reproducible: any later trace of the
same code object is complete. V6 therefore runs one discarded priming trace per
candidate before the recorded witness, and records the priming witness so the
effect stays visible. Nothing else changes: the same four frozen candidates,
the same schedule, the same resource coordinates, the same adjudication rule,
and the same recorded host and interpreter envelope. Per-frame validation is
unchanged, so if priming ever fails the packet is still refused.
...
PREREG = HERE / "NN_NONNN_POINT_PARITY3_PREREG_V6.json"
PARENT_HARNESSES = (HERE / "nn_nonnn_point_parity3_experiment_v5.py",
                    HERE / "nn_nonnn_point_parity3_experiment_v4.py")
INPUTS = tuple((a, b, c) for a in (0, 1) for b in (0, 1) for c in (0, 1))
EXPECTED = tuple(a ^ b ^ c for a, b, c in INPUTS)


# The four candidate bodies are byte-identical to the V5 and V4 harnesses. The
# instrument proves that below by hashing the source segments of all three.
...
def _trace_session(fn):
    """One trace session over the whole input domain."""
...
def trace_candidate_opcodes(fn):
    """Prime the code object, then collect the recorded witness.

    On CPython 3.13 the first-ever trace of a code object loses its first
    frame's opcode events, while any later trace of the same code object is
    complete. One discarded priming session is therefore enough, and it is
    harmless on interpreters that never had the defect. The priming witness is
    returned alongside the recorded one so the effect stays visible instead of
    being silently absorbed.
    """
    priming = _trace_session(fn)
    recorded = _trace_session(fn)
    return recorded, priming
...
def instrumentation_preflight():
    ...
    forward, diagnostics, priming_diagnostics = {}, {}, {}
    for cid, row in CANDIDATES.items():
        calls, priming = trace_candidate_opcodes(row["fn"])
        diagnostics[cid] = opcode_witness_diagnostics(row["fn"], calls)
        priming_diagnostics[cid] = opcode_witness_diagnostics(row["fn"], priming)
        forward[cid] = {"count": validate_opcode_witness(row["fn"], calls), "calls": calls}
    reverse = {}
    for cid in reversed(list(CANDIDATES)):
        calls, _ = trace_candidate_opcodes(CANDIDATES[cid]["fn"])
    ...
    # Whether priming was needed on this interpreter is itself a recorded fact.
    priming_was_required = any(
        row["complete_frames"] != len(INPUTS) for row in priming_diagnostics.values())
    return {
        ...
        "priming_witness_diagnostics": priming_diagnostics,
        "priming_was_required_on_this_interpreter": priming_was_required,
...
def candidate_identity_record() -> dict:
    """Bind this harness's candidates to every earlier portable harness."""
    local = candidate_source_hashes(Path(__file__))
    parents, available, identical = {}, {}, True
    for harness in PARENT_HARNESSES:
        present = harness.is_file()
        available[harness.name] = present
        if present:
            parents[harness.name] = candidate_source_hashes(harness)
            identical = identical and parents[harness.name] == local
        else:
            identical = False
    return {
        "local_source_sha256": local,
        "parent_harnesses": [h.name for h in PARENT_HARNESSES],
        "parent_harness_available": available,
        "parent_source_sha256": parents,
        "byte_identical_to_parent": identical,
        "identity_basis": "exact source segment bytes; interpreter-version independent",
    }


def validate_preregistration(prereg):
    require(prereg.get("schema") == "NN_NONNN_POINT_PARITY3_PREREG_V6", "wrong prereg schema")
    ...
    require(prereg.get("independent_prospective_prediction") is False,
            "V6 must disclose that V1, V2, V4 and V5 outcomes were read")
...
def run_experiment(host_label: str) -> dict:
    ...
    packet = {
        "schema": "NN_NONNN_POINT_PARITY3_RESULT_V6",
    ...
    try:
        ...
        for cid, row in CANDIDATES.items():
            try:
                calls, priming = trace_candidate_opcodes(row["fn"])
                diagnostics[cid] = opcode_witness_diagnostics(row["fn"], calls)
                diagnostics[cid]["priming"] = opcode_witness_diagnostics(row["fn"], priming)
...
def deterministic_self_test() -> dict:
    ...
    identity = candidate_identity_record()
    require(identity["byte_identical_to_parent"],
            "candidates drifted from an earlier portable harness")
    diagnostics, priming_diagnostics = {}, {}
    for cid, row in CANDIDATES.items():
        calls, priming = trace_candidate_opcodes(row["fn"])
        diagnostics[cid] = opcode_witness_diagnostics(row["fn"], calls)
        priming_diagnostics[cid] = opcode_witness_diagnostics(row["fn"], priming)
    ...
    priming_was_required = any(
        row["complete_frames"] != len(INPUTS) for row in priming_diagnostics.values())
    return {
        "terminal": "PARITY3_V6_INSTRUMENT_SELF_TEST_GREEN",
        "candidate_identity": identity,
        "witness_diagnostics": diagnostics,
        "priming_witness_diagnostics": priming_diagnostics,
        "priming_was_required_on_this_interpreter": priming_was_required,

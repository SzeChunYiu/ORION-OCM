"""Close exactly three reviewed atoms; enforce complete adjudication records."""
import hashlib
...
ROOT = HERE.parents[1]
BASE = ROOT / "research/gmi-1068-recursive-audit-v10/SCOPE_SNAPSHOT_V10.json"
BASE_SHA = "158dbf47c2247cbc888b185318ac4aa2fc0c5166053fc00a585885fa92c6e939"
...
CONTRACT = load("audit_contract_v11", HERE / "audit_contract_v11.py")
...
def evaluate(current=None, receipt=None):
    ...
    need(hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA, "V10 baseline drift")
    before = json.loads(BASE.read_text())
    if current is None:
        current = json.loads((HERE / "SCOPE_SNAPSHOT_V11.json").read_text())
    if receipt is None:
        receipt = json.loads((ROOT / CONTRACT.PACKAGE / "RESULT_V11.json").read_text())
    ...
    for name in gate.NODES:
        ...
        for original, actual in zip(old["obligations"], new["obligations"]):
            ...
            if atom in CLOSING:
                need(name in ("R1", "R2"), "wrong atom owner")
    ...
    need(result["registered_atoms"] == 222 and result["unresolved_atoms"] == 211,
    ...
    return {"status": "PASS", "registered_atoms": 222, "governance_atoms_closed": 8,
            "scientific_atoms_closed": sorted(closed), "remaining_atoms": 211,

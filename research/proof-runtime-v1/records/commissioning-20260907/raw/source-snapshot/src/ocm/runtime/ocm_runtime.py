...
class OCMRuntime:
    ...
    def _emit(self, event_type: EventType, status: EventStatus, *, inputs: Iterable[str] = (), outputs: Iterable[str] = (), evidence: Iterable[str] = (), operator: str = "runtime", payload: Mapping[str, Any] | None = None, delta: ResourceVector | None = None, seed: str | None = None) -> OCMEvent:
        if self._replay_required:
            raise RuntimeRefusal("REPLAY_REQUIRED", "complete verified replay before further event writes")
    ...
    def _apply(self, ev: OCMEvent) -> None:
        ...
        p = ev.payload
        t = ev.event_type
        if t is EventType.OBJECT_BATCH_ADMITTED:
            if ev.status is not EventStatus.PASS:
                raise RuntimeRefusal("INVALID_ADMISSION_BATCH_STATUS")
            ks, certificates, _, outputs, evidence, resources = self._stage_admission_batch(p)
            if (ev.output_object_ids != outputs or ev.evidence_ids != evidence
                    or ev.input_object_ids or ev.resources != resources
                    or ev.operator_fingerprint != "kso.admit_batch.v1"):
                raise RuntimeRefusal("INVALID_ADMISSION_BATCH_ENVELOPE")
            # One publication: no prefix of the staged field/certificates is visible.
            self.state = replace(self.state, ks=ks, certificates=certificates)
            f, e, r = self._solve_epochs
            self._solve_epochs = (f + 1, e, r)
            return
    ...
    # ------------------------------------------------------------------ objects
    def _stage_admission_batch(self, payload: Mapping[str, Any]):
        """Validate the versioned event entirely in locals, without runtime writes."""
        if (type(payload) is not dict or set(payload) != {"schema", "items"}
                or payload["schema"] != "ocm.object-admission-batch.v1"
                or type(payload["items"]) is not list or not payload["items"]):
            raise RuntimeRefusal("INVALID_ADMISSION_BATCH_PAYLOAD")
        ks, certificates = self.state.ks, dict(self.state.certificates)
        receipts, outputs, evidence = [], [], set()
        resources = ResourceVector()
        for item in payload["items"]:
            try:
                if type(item) is not dict or set(item) != {"atom", "edges", "certificate"}:
                    raise ValueError("invalid item fields")
                if type(item["atom"]) is not dict or type(item["edges"]) is not list:
                    raise ValueError("invalid atom or edges")
                atom = _atom_from(item["atom"])
                # Historical _edge_from omits meta; preserve it only in this new contract.
                edges = tuple(replace(_edge_from(d), meta=tuple(d["meta"].items())) for d in item["edges"])
                certificate = AD.CertificateKind(item["certificate"])
                canonical = {"atom": _atom_to(atom), "edges": [_edge_to(e) for e in edges],
                             "certificate": certificate.value}
                if canonical_bytes(canonical) != canonical_bytes(item):
                    raise ValueError("batch item is not canonically round-trippable")
            except (KeyError, TypeError, ValueError, AttributeError) as exc:
                raise RuntimeRefusal("INVALID_ADMISSION_BATCH_ITEM", str(exc)) from exc
            ks, receipt = AD.admit(ks, atom, edges, certificate, revoked=self.state.revoked)
            certificates[atom.atom_id] = certificate.value
            receipts.append(receipt)
            outputs.extend((atom.atom_id, *(e.edge_id for e in edges)))
            evidence.update(atom.warrant.evidence)
            resources = resources + receipt.resources
        return ks, certificates, tuple(receipts), tuple(outputs), tuple(sorted(map(str, evidence))), resources

    def admit_batch(self, items: Sequence[tuple[S.Atom, Sequence[S.Hyperedge], AD.CertificateKind | str]]) -> tuple[AD.AdmissionReceipt, ...]:
        """Admit an ordered nonempty batch in one crash-atomic, CAS-bound event.

        All items are detached and validated before persistence; the reducer stages
        them again before one state publication. No invalid prefix is admitted.
        Event schema v1 remains unchanged; the additive OBJECT_BATCH_ADMITTED_V1
        event carries payload schema ocm.object-admission-batch.v1. Older readers
        refuse this new event type. Existing single-admit events are unchanged.
        An interruption during emission/reduction is uncertain completion: further
        event writes refuse until complete verified replay, even if the in-memory
        ledger head already advanced. Failed replay never clears that guard.
        """
        if self._replay_required:
            raise RuntimeRefusal("REPLAY_REQUIRED", "complete verified replay before admission")
        if not isinstance(items, Sequence) or isinstance(items, (str, bytes)) or not items:
            raise RuntimeRefusal("EMPTY_OR_INVALID_ADMISSION_BATCH")
        try:
            payload = json.loads(canonical_bytes({"schema": "ocm.object-admission-batch.v1", "items": [
                {"atom": _atom_to(a), "edges": [_edge_to(e) for e in edges], "certificate": AD.CertificateKind(c).value}
                for a, edges, c in items]}))
        except (TypeError, ValueError, AttributeError) as exc:
            raise RuntimeRefusal("INVALID_ADMISSION_BATCH_ITEM", str(exc)) from exc
        _, _, receipts, outputs, evidence, resources = self._stage_admission_batch(payload)
        try:
            ev = self._emit(EventType.OBJECT_BATCH_ADMITTED, EventStatus.PASS, outputs=outputs,
                            evidence=evidence, operator="kso.admit_batch.v1", payload=payload, delta=resources)
            self._apply(ev)
        except BaseException:
            # Append may be durable even if it raised; emission may have advanced
            # the local head before reduction failed. Never append from that state.
            self._replay_required = True
            raise
        return receipts

...
import check_lean_v24 as kernel
    ...
    def test_registered_statement_types_reject_source_valid_mutants(self):
        ...
        changes = [
            [("ProofTargetsV24.lean",
              r"theorem selected_projection_contract[\s\S]*?(?=\ntheorem preference_contract)",
              "theorem selected_projection_contract : True := True.intro\n")],
            [("ProofTargetsV24.lean",
              r"theorem preference_contract[\s\S]*?(?=\ntheorem plan_intersection_contract)",
              "theorem preference_contract : True := True.intro\n")],
            [("ProofTargetsV24.lean",
              r"theorem plan_intersection_contract[\s\S]*?(?=\nend ProofTargetsV24)",
              "theorem plan_intersection_contract : True := True.intro\n")],
        ]
        ...
        for index, mutation in enumerate(mutations):
            with self.subTest(mutation=index), tempfile.TemporaryDirectory(prefix="v24-kernel-hostile-") as directory:
                root = Path(directory)

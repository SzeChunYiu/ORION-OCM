...
from proof_contract_v24 import ENTRIES, SOURCE_NAMES, SOURCE_PATHS, audit_source
...
def evaluate(source_overrides=None, require_source_compile=False):
    ...
    audit = audit_source()
    try:
        with tempfile.TemporaryDirectory(prefix="gmi-v24-lean-") as output:
            ...
            audit_path = Path(output) / "RegisteredProofsV24.lean"
    ...
    return {"status": "PASS", "lean_version": "4.19.0", "sources": bindings,
            ...
            "source_assumptions": "actual V15 partial Context and P intersection E; explicit profile projections; active selected witnesses; V12 primitive ordered ring and Int instance; all candidate identities; strictly positive prices for Pareto inclusion; partial loss in declared preorder; common plan universe and injective relabeling with mapped ambient guard"}

...
spec = importlib.util.spec_from_file_location("checked_snapshot_v17", HERE / "check_snapshot_v17.py")
...
class SnapshotTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.snapshot = json.loads((HERE / "SCOPE_SNAPSHOT_V17.json").read_text())
        cls.receipt = json.loads((checker.ROOT / checker.CONTRACT.PACKAGE / "RESULT_V17.json").read_text())
    ...
    def test_actual_snapshot_no_alarm(self):
        ...
        self.assertEqual(result["new_scientific_atoms_closed"], ["GMI2-R2-005"])
        self.assertEqual(len(result["scientific_atoms_closed"]), 11)
        self.assertEqual((result["remaining_atoms"], result["original_fulfilled"],
                          result["qualified_replacements"], result["active_unresolved"]), (203,19,2,201))
    ...
    def test_snapshot_mutations(self):
        ...
        wrong = checker.CONTRACT.witness(checker.ROOT, "ContextMapsV17.lean", "lean_declaration", "dual_twice")
        mutations = [
            ...
            (atom("R2", 4, "evidence"), [wrong]),
            (atom("R2", 4, "title"), "a different original requirement"),
            (atom("R2", 4, "status"), "OPEN"),
            (atom("R2", 4, "disposition"), "undefined histories carry a total context preorder"),
            (atom("R2", 4, "historical_status"), "CLOSED"),
            (atom("R2", 4, "id"), "GMI2-R2-099"),
            ...
            (atom("R2", 4, "evidence"), []),
            (atom("R2", 4, "evidence"), [checker.CONTRACT.witness(
                checker.ROOT, "RESULT_V17.json", "json_pointer", "/coverage")]),
        ...
        missing["rounds"]["R2"]["artifacts"] = [a for a in missing["rounds"]["R2"]["artifacts"]
                                                  if a["path"] != checker.CONTRACT.PACKAGE + "/FREEZE_V17.md"]
        added = deepcopy(self.snapshot)
        path = HERE / "audit_contract_v17.py"
        ...
        source, target = swapped["rounds"]["R2"]["obligations"][4], swapped["rounds"]["R2"]["obligations"][2]
        source["status"] = "OPEN"
        target.update(status="CLOSED", disposition=source["disposition"], evidence=source["evidence"])
        extended = deepcopy(self.snapshot)
        extended["rounds"]["R2"]["obligations"][4]["unsupported_authority"] = "universal"
    ...
    def test_receipt_mutations(self):
        ...
        mutations = [
            ...
            (("coverage", "test_viability_v17", "viability_cases"), True),
            (("coverage", "test_viability_v17", "viability_cases"), 4165.0),
        ...
        for key in self.receipt:
            ...
            del bad[key]
            invalid.append(bad)
        for path in ((), ("kernel",), ("source_bindings",), ("coverage", "test_viability_v17"), ("inputs",)):
            bad = deepcopy(self.receipt)
            cursor = bad
            for key in path:
                cursor = cursor[key]
    ...
    def test_current_accounting_mutations(self):
        ...
        for path, value in [(("original_fulfilled",), 18), (("original_unresolved",), 204),
                (("active_unresolved",), 202), (("qualified_replacements",), 0),

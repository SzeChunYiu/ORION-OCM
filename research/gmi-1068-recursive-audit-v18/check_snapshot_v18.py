"""Close exactly one reviewed reference-measure atom; enforce complete adjudication records."""
import hashlib
...
ROOT = HERE.parents[1]
BASE = ROOT / "research/gmi-1068-recursive-audit-v17/SCOPE_SNAPSHOT_V17.json"
BASE_SHA = "491cc11bb987a637f3413f53f74fcf0c1c00c0564adc7da24b5f982d583f32d3"
...
CONTRACT = load("audit_contract_v18", HERE / "audit_contract_v18.py")
...
def evaluate(current=None, receipt=None, accounting=None):
    ...
    need(hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA, "V17 baseline drift")
    before = json.loads(BASE.read_text())
    if current is None:
        current = json.loads((HERE / "SCOPE_SNAPSHOT_V18.json").read_text())
    if receipt is None:
        receipt = json.loads((ROOT / CONTRACT.PACKAGE / "RESULT_V18.json").read_text())
    ...
    if accounting is None:
        accounting = json.loads((HERE / "CURRENT_ACCOUNTING_V18.json").read_text())

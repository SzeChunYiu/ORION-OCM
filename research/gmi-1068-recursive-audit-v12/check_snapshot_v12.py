"""Close exactly one reviewed scalarization atom; enforce complete adjudication records."""
import hashlib
...
ROOT = HERE.parents[1]
BASE = ROOT / "research/gmi-1068-recursive-audit-v11/SCOPE_SNAPSHOT_V11.json"
BASE_SHA = "579e9ae78bf992b13ea5449b756f7da17ae6a694afe95dfc28989a79a6e56c07"
...
CONTRACT = load("audit_contract_v12", HERE / "audit_contract_v12.py")
...
def evaluate(current=None, receipt=None):
    ...
    need(hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA, "V11 baseline drift")
    before = json.loads(BASE.read_text())
    if current is None:
        current = json.loads((HERE / "SCOPE_SNAPSHOT_V12.json").read_text())
    if receipt is None:
        receipt = json.loads((ROOT / CONTRACT.PACKAGE / "RESULT_V12.json").read_text())
    ...
    need(result["registered_atoms"] == 222 and result["unresolved_atoms"] == 210,
    ...
    need(len(scientific) == 4, "scientific closure count drift")

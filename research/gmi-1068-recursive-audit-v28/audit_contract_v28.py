"""Fixed original-witness closure contract and inherited source custody."""
import hashlib
...
def load(path):
    spec = importlib.util.spec_from_file_location("v28_contract_" + path.stem, path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


SPECS = load(Path(__file__).with_name("audit_specs_v28.py"))
PINS = load(Path(__file__).with_name("audit_pins_v28.py"))
...
def atom_evidence(root, atom):
    ...
    if not PINS.FINAL:
        raise OSError("final reviewed V28 proof pins unavailable")
    return [witness(root, "RESULT_V28.json", "json_pointer", "/atoms/" + atom),
            witness(root, "oracle_v28.py", "python_symbol", "certify"),
            witness(root, "WITNESS_TRANSLATIONS_V28.json", "json_pointer", "/rows")] + [
        witness(root, name, "lean_declaration", symbol.rsplit(".", 1)[-1]) for name, symbol in PINS.ATOM_DECLARATIONS[atom]]
...
def repair(root, name):
    if not PINS.FINAL:
        raise OSError("final reviewed V28 proof pins unavailable")
    evidence = [witness(root, "check_witnesses_v28.py", "python_symbol", "evaluate"),
                witness(root, "RESULT_V28.json", "json_pointer", "/kernel" if name == "R14" else "/coverage")]
    extras = {"R2": PINS.REPAIR_DECLARATION,
              "R14": ("proof_contract_v28.py", "python_symbol", "audit_source"),
              "R15": ("oracle_v28.py", "python_symbol", "certify")}
    ...
    return {"id": name + "-witnesses-v28", "scope": REPAIR_SCOPES[name],
...
def validate_receipt(root, receipt):
    ...
    need(type(receipt) is dict and set(receipt) == {
        ...
        "claim_boundaries", "witness_reconciliation"}, "receipt fields")
    fixed = {"schema": "GMI_1068_IRREDUCIBILITY_WITNESSES_V28", "tests_run": 11,
             "overall_closure": "OPEN", "scientific_truth_certified": False, "claim_boundaries": BOUNDARIES,
             "witness_reconciliation": SPECS.WITNESSES}
    ...
    inputs = {entry["path"].rsplit("/", 1)[-1]: entry["sha256"] for entry in artifacts
              if not entry["path"].endswith("/RESULT_V28.json")}
    ...
    exact(load(package / "audit_witnesses_v28.py").verify(root), SPECS.WITNESSES, "original witness reconciliation drift")
    custody = load(package / "custody_v28.py")
    try:
    ...
    exact(receipt["coverage"], load(package / "coverage_v28.py").REQUIRED, "coverage receipt drift")
    need(digest(package / "proof_contract_v28.py") == PROOF_CONTRACT_SHA, "reviewed proof-type drift")
    proof = load(package / "proof_contract_v28.py")
...
def current_accounting(snapshot, amendment_carry):
    ...
    return {"schema": "GMI_1068_CURRENT_ACCOUNTING_V28", "snapshot_content_sha256": snapshot_sha,

"""Close only the registered R2-009 original-witness atom; preserve every other original record; enforce complete adjudication records."""
import hashlib
...
ROOT = HERE.parents[1]
BASE = ROOT / "research/gmi-1068-recursive-audit-v27/SCOPE_SNAPSHOT_V27.json"
BASE_SHA = "937e3990cb9c65e20972a655434bb5619d9b00c3f6a47c29bef9ba5ad6a810f0"
...
CONTRACT = load("audit_contract_v28", HERE / "audit_contract_v28.py")
...
def evaluate(current=None, receipt=None, accounting=None):
    ...
    need(hashlib.sha256(BASE.read_bytes()).hexdigest() == BASE_SHA, "V27 baseline drift")
    before = json.loads(BASE.read_text())
    if current is None:
        current = json.loads((HERE / "SCOPE_SNAPSHOT_V28.json").read_text())
    if receipt is None:
        receipt = json.loads((ROOT / CONTRACT.PACKAGE / "RESULT_V28.json").read_text())
    need(type(current) is dict and set(current) == set(before), "snapshot top-level fields")
    for field in before:
        if field != "rounds":
            expected = "8a10a2a6720ccecab9ad1adaa36e3cc18613b56a" if field in ("observed_main", "audited_base") else before[field]
    ...
    if accounting is None:
        accounting = json.loads((HERE / "CURRENT_ACCOUNTING_V28.json").read_text())

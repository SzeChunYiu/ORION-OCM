...
spec=importlib.util.spec_from_file_location('checked_snapshot_v22',HERE/'check_snapshot_v22.py')
...
class SnapshotTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.snapshot=json.loads((HERE/'SCOPE_SNAPSHOT_V22.json').read_text())
        cls.receipt=json.loads((checker.ROOT/checker.CONTRACT.PACKAGE/'RESULT_V22.json').read_text())
    ...
    def test_actual_snapshot_no_alarm(self):
        ...
        self.assertEqual(len(result['scientific_atoms_closed']),20)
        self.assertEqual((result['original_fulfilled'],result['remaining_atoms'],
                          result['qualified_replacements'],result['active_unresolved']),(28,194,2,192))
    ...
    def test_rehashed_snapshot_mutations(self):
        ...
        wrong=checker.CONTRACT.witness(checker.ROOT,'ProofTargetsV22.lean','lean_declaration','gated_success_contract')
        changes=[(('source_issue',),1068.0),(('source_issue',),True),(('observed_main',),'0'*40),(('audited_base',),'0'*40),
            ...
            (atom('R3',3,'evidence'),[wrong]),(atom('R3',6,'status'),'UNKNOWN'),
        ...
        for field,value in (('title','retargeted barrier'),('disposition','all physical causes identified'),('evidence',[wrong])):
            changes.append((atom('R3',6,field),value))
        ...
        bad=deepcopy(self.snapshot);bad['rounds']['R3']['artifacts']=[a for a in bad['rounds']['R3']['artifacts']
            if a['path']!=checker.CONTRACT.PACKAGE+'/FREEZE_V22.md'];invalid.append(bad)
        bad=deepcopy(self.snapshot);path=HERE/'audit_contract_v22.py'
    ...
    def test_receipt_mutations(self):
        ...
        coverage_key=next(iter(self.receipt['coverage']['test_execution_v22']))
        ...
        changes=[(('atoms',),{'GMI2-R1-001':{'status':'VERIFIED_AT_REGISTERED_SCOPE'}}),
            (('registered_results','V1','scope'),'all resources admit subtraction'),
            (('registered_results','V2','status'),'PENDING_REVIEW'),
            ...
            (('coverage','test_execution_v22',coverage_key),True),
            (('coverage','test_execution_v22',coverage_key),float(self.receipt['coverage']['test_execution_v22'][coverage_key])),
        ...
        for path in ((),('kernel',),('source_bindings',),('coverage','test_execution_v22'),('inputs',),('registered_results',)):
    ...
    def test_coupled_result_scope_and_original_promotion(self):
        ...
        for result in checker.CONTRACT.RESULTS:
            ...
            claim='Every physical barrier has a unique least causal explanation.'
        ...
        for atom_id in checker.CONTRACT.ATOM_SPECS:
            ...
            atom['disposition']='All possible interventions have been completely classified.'
    ...
    def test_current_accounting_mutations(self):
        ...
        for path,value in [(('original_fulfilled',),29),(('original_unresolved',),193),
                (('active_unresolved',),191),(('qualified_replacements',),3),(('original_total',),222.0),
        ...
        invalid.append(json.loads((checker.BASE.parent/'CURRENT_ACCOUNTING_V21.json').read_text()))

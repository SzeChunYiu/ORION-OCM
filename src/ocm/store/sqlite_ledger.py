...
class SQLiteLedgerStore:
    ...
    The public methods intentionally mirror the subset of ``LedgerStore`` used by
    ``OCMRuntime``: ``entries``, ``head``, ``append``, ``append_identified`` and
    ``verify``.  Payloads are stored as canonical JSON text; row hashes are
    computed by the exact incumbent ``compute_entry_hash`` function.
    """
    ...
    def _row_entry(row: tuple[Any, ...]) -> LedgerEntry:
        ...
        return LedgerEntry(
            int(sequence), str(kind), payload, str(prev_hash), str(entry_hash)
        )
    ...
    def entries(self, kind: str | None = None) -> tuple[LedgerEntry, ...]:
        ...
        Full-chain verification remains the authority for replay/export.  The
        performance difference is that ordinary appends do not first replay every
        historic row and do not rewrite historic bytes.
        """
        query = (
            "SELECT sequence, kind, payload_json, prev_hash, entry_hash "
    ...
    def head(self) -> LedgerEntry | None:
        """Return the indexed tail row without replaying older history."""
        with self._connect() as connection:
        ...
        *,
        ...
        try:
            ...
            self._check_expected_head(
                None if tail is None else tail.entry_hash, expected_head
            )
            entry = self._insert(
                connection,
                kind=kind,
    ...
    def append_identified(
        ...
        *,
        ...
        if declared != transaction_id:
            raise ValueError(
                "payload transaction_id disagrees with the declared transaction_id"
            )
    ...
    def export_jsonl_bytes(self) -> bytes:
        """Canonical JSONL projection identical to incumbent row encoding."""
        chunks = []

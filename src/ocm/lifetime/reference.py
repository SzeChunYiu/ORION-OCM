...
class OllamaReferenceArm:
    ...
    def _digest(self) -> str | None:
        """The pinned model identity: Ollama's content digest for the tag (from /api/tags)."""
        try:
            with urllib.request.urlopen(f"{self.host}/api/tags", timeout=60) as r:
                d = json.loads(r.read())
    ...
    def say(self, utt: str) -> str:
        if utt == "__restart__":
            # the transcript is the reference arm's persistent memory (its analogue of the OCM ledger):
            # a restart re-sends the same context and keeps the transcript, so earlier statements survive
            self.restarts += 1
            return "restarted"
        ...
        msgs = [{"role": "system", "content": SYSTEM + "\n\n" + self._context()}] + self.history[-40:] + [{"role": "user", "content": utt}]
    ...
    def info(self) -> dict:
        return {"model": self.model, "digest": self.digest, "facts_in_prompt": len(self.facts), "lessons": len(self.lessons), "restarts": self.restarts, "transcript_turns": len(self.history) // 2, "information_binding": INFORMATION_BINDING}
...
def phase_A_reference(arm: OllamaReferenceArm) -> dict[str, Any]:
    ...
    for lesson, use, ask, passive in M7.LESSONS:
        ...
        arm.say(lesson)
        steps["relearned"].append(classify(arm.say(ask)) == "yes" or classify(arm.say(use)) in {"yes", "other"})   # M7: "said so" on ask OR "Noted" on use

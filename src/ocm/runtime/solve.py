...
def navigate_stage(ks: KnowledgeSpace, seed: Sequence[Fraction], task: Task, cfg: SolveConfig, revoked: Iterable[Hashable]) -> tuple[StageResult, dict[str, Any]]:
    ...
    n = len(ks.ids)
    res = ResourceVector(navigation_work=3 * n * n)
    ...
    return StageResult(Stage.NAVIGATION, worst, reason, object_ids=tuple(task.targets), payload=payload, resources=res), {"act_w": act_w, "act_x": act_x, "background": background, "witness": witness, "outcomes": outcomes}
...
def extract_stage(ks: KnowledgeSpace, seed: Sequence[Fraction], nav: Mapping[str, Any], cfg: SolveConfig, revoked: Iterable[Hashable]) -> tuple[StageResult, dict[str, Any]]:
    ...
    rho_x = SP.surprise(ks, nav["act_x"], nav["background"], seed, cfg.alpha, cfg.surprise_model, revoked=rv, mode=N.NavigationMode.EXPLORATORY)

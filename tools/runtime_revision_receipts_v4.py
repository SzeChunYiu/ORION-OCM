"""Immutable fourth-revision custody; no historical recipe executes on current code.
...
PARENT_COMMIT = "ddc2be3104f4f9ac5e476eee96245007c89b0369"
CONFIG_PATH = f"docs/provenance/{REVISION}/REVISION_V4.json"
CONFIG_SHA256 = "6a39b71f0e5cefef2045a3b12aa4adf0e8cfb98e9ff94cde1ae4aabc1ddba120"
...
class ParentSnapshot:
    def __init__(self, root: Path, config: dict, *, upstream=False):
        spec = config["upstream_manifest" if upstream else "parent_manifest"]
        expected_parent = config["upstream_commit"] if upstream else PARENT_COMMIT
        ...
        m = self.manifest
        if m["schema"] != "ocm.parent-source-evidence.v4" or m["parent_commit"] != expected_parent:
            raise ReceiptError("parent snapshot identity changed")
        commit = base64.b64decode(m["git_commit_object_base64"], validate=True)
        if git_hash("commit", commit) != expected_parent:
        ...
        if set(files) != set(self.contents):
            raise ReceiptError("incomplete exact parent archive")
        if upstream:
            return
        other = ParentSnapshot(root, config, upstream=True)
        self.upstream_contents = other.contents
        replacements = {p: {"from_sha256": digest(data), "to_sha256": digest(other.contents[p])}
            for p, data in self.contents.items() if p.startswith(("docs/", "research/"))
            and p in other.contents and data != other.contents[p]}
        if replacements != config["historical_path_replacements"]:
            raise ReceiptError("undeclared historical merge replacement")
        # Both full Git trees remain archived. At shared historical paths the
        # exact newer upstream bytes are retained; local-only evidence stays exact.
        historical = {p: data for p, data in self.contents.items() if p.startswith(("docs/", "research/"))}
        historical.update({p: data for p, data in other.contents.items() if p.startswith(("docs/", "research/"))})
        for rel, data in historical.items():
            if raw(root, rel) != data:
...
class CurrentReceipts:
    def __init__(self, root: Path):
        ...
        if sha(root, CONFIG_PATH) != CONFIG_SHA256:
            raise ReceiptError("immutable fourth-revision config changed")
        self.config = read_json(root, CONFIG_PATH)
        if (self.config.get("schema") != "ocm.runtime-revision.v4"
            ...
            raise ReceiptError("fourth-revision identity or milestone inventory changed")
    ...
    def _spec(self, milestone: int) -> dict:
        if type(milestone) is not int or str(milestone) not in self.config["milestones"]:
            raise ReceiptError("milestone has no declared fourth successor")
    ...
    def _replay(self, inventory: dict) -> dict:
        ...
        required = {"schema": "ocm.engineering-replay.v4", "revision": REVISION,
    ...
    def build(self, milestone: int) -> dict:
        ...
        return {"schema": "ocm.runtime-successor-receipt.v4", "revision": REVISION,
                "receipt": f"M{milestone}_RECEIPT_RUNTIME_20260905_V4", "terminal": spec["terminal"],
                ...
                "upstream_snapshot": self.config["upstream_manifest"],
    ...
    def verify(self, milestone: int) -> None:
        ...
        if milestone in self._visiting:
            raise ReceiptError("cyclic fourth-successor dependency")
        recorded = read_json(self.root, spec["successor_path"])
        self._visiting.add(milestone)
        try:
            if recorded != self.build(milestone):
                raise ReceiptError(f"M{milestone} fourth successor DRIFT")
...
def revision_main(root: Path, argv: list[str], milestone: int) -> int:
    if argv not in (["--verify"], ["--write-current"]):
        print("Historical receipts are immutable. Use --verify or --write-current for the explicit V4 successor.")
        return 2
    try:
        current = CurrentReceipts(root)
        if argv == ["--verify"]:
            current.verify(milestone)
            print(f"M{milestone} V4 successor verified: archived parent custody and current engineering only")
        else:
...
def verify_upstream_receipt(root: Path, relative: str):
    current = CurrentReceipts(root)
    snapshot = ParentSnapshot(root, current.config)
    previous = parse_json(snapshot.upstream_contents[relative], relative)
    for path, expected in previous["bound_files"].items():
        if path not in snapshot.upstream_contents or digest(snapshot.upstream_contents[path]) != expected:
            raise ReceiptError(f"upstream historical binding drift: {path}")
    return {"receipt": relative, "upstream_commit": current.config["upstream_commit"],
            "status": "ARCHIVED_UPSTREAM_BINDINGS_VERIFIED", "legacy_recipe_execution": "NOT_EXECUTED",
            "terminal_at_upstream": previous.get("terminal"), "current_scientific_promotion": "NOT_ESTABLISHED"}


def upstream_main(root: Path, argv: list[str], relative: str) -> int:
    if argv != ["--verify"]:
        print("Historical upstream receipts are immutable. Use --verify for archived custody only.")
        return 2
    try:
        print(json.dumps(verify_upstream_receipt(root, relative), sort_keys=True))
        return 0
    except (ReceiptError, OSError, KeyError, TypeError, ValueError, zipfile.BadZipFile) as exc:
        print(f"UPSTREAM CUSTODY REFUSED: {exc}")
        return 1


if __name__ == "__main__":
    if len(sys.argv) < 2 or not sys.argv[1].isdigit():
        print("Usage: runtime_revision_receipts_v4.py MILESTONE --verify|--write-current")
        raise SystemExit(2)

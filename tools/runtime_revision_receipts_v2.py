"""Immutable second-revision custody; no historical recipe executes on current code.
...
REVISION = "runtime_revision_20260905_v2"
PARENT_COMMIT = "479e78165fb455481cadcf03fd8e3a99ba79c5af"
CONFIG_PATH = f"docs/provenance/{REVISION}/REVISION_V2.json"
CONFIG_SHA256 = "b3fab774ba2e4c350fa2413aa6bab50c5bc6d79c962fd1a31c6cfcfe819ffd1f"
...
def source_inventory(root: Path) -> dict[str, str]:
    """Exact runtime resources, tests, tools and package metadata; no Git dependency."""
...
class ParentSnapshot:
    def __init__(self, root: Path, config: dict):
        ...
        m = self.manifest
        if m["schema"] != "ocm.parent-source-evidence.v2" or m["parent_commit"] != PARENT_COMMIT:
        ...
        for spec in config["milestones"].values():
            ...
            for bound, expected in previous["bound_files"].items():
...
class CurrentReceipts:
    def __init__(self, root: Path):
        ...
        if sha(root, CONFIG_PATH) != CONFIG_SHA256:
            raise ReceiptError("immutable second-revision config changed")
        self.config = read_json(root, CONFIG_PATH)
        if (self.config.get("schema") != "ocm.runtime-revision.v2"
            ...
            raise ReceiptError("second-revision identity or milestone inventory changed")
    ...
    def _spec(self, milestone: int) -> dict:
        if type(milestone) is not int or str(milestone) not in self.config["milestones"]:
            raise ReceiptError("milestone has no declared second successor")
    ...
    def _replay(self, inventory: dict) -> dict:
        ...
        required = {"schema": "ocm.engineering-replay.v2", "revision": REVISION,
        ...
        for label, spec in requirements.items():
            ...
            if (not suites or counts["tests"] != len(cases)
                    ...
                    or counts["skipped"] != skipped or counts["failures"] != 0 or counts["errors"] != 0
    ...
    def build(self, milestone: int) -> dict:
        ...
        for rel in previous["bound_files"]:
            if rel in by_path:
        ...
        return {"schema": "ocm.runtime-successor-receipt.v2", "revision": REVISION,
                "receipt": f"M{milestone}_RECEIPT_RUNTIME_20260905_V2", "terminal": spec["terminal"],
    ...
    def verify(self, milestone: int) -> None:
        ...
        if milestone in self._visiting:
            raise ReceiptError("cyclic second-successor dependency")
        recorded = read_json(self.root, spec["successor_path"])
        self._visiting.add(milestone)
        try:
            if recorded != self.build(milestone):
                raise ReceiptError(f"M{milestone} second successor DRIFT")
...
def revision_main(root: Path, argv: list[str], milestone: int) -> int:
    if argv not in (["--verify"], ["--write-current"]):
        print("Historical receipts are immutable. Use --verify or --write-current for the explicit V2 successor.")
        return 2
    try:
        current = CurrentReceipts(root)
        if argv == ["--verify"]:
            current.verify(milestone)
            print(f"M{milestone} V2 successor verified: archived parent custody and current engineering only")
        else:
...
if __name__ == "__main__":
    if len(sys.argv) < 2 or not sys.argv[1].isdigit():
        print("Usage: runtime_revision_receipts_v2.py MILESTONE --verify|--write-current")
        raise SystemExit(2)

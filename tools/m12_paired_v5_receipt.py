#!/usr/bin/env python3
"""Generate or verify docs/provenance/M12_PAIRED_RECEIPT_V1.json (the V5 paired-lifetimes study (fresh streams on the corrected runtime; issue #38 M12 gates)).
...
def fresh() -> dict:
    ev = json.loads((ROOT / "research/ocm-m12/M12_PAIRED_LIFETIMES_EVAL_V5.json").read_text(encoding="utf-8"))
    rep = json.loads((ROOT / "docs/provenance/M12_PAIRED_REPLICATION_RECEIPT_V5.json").read_text(encoding="utf-8"))
    try:
        head = subprocess.run(["git", "-C", str(ROOT), "rev-parse", "HEAD"], capture_output=True, text=True, check=True).stdout.strip()
    except Exception:  # noqa: BLE001
        head = "UNKNOWN"
    return {"receipt": "M12_PAIRED_RECEIPT_V5", "terminal": (ev["deterministic"]["decision"] if ev["study_status"].startswith("PROTECTED_PREREGISTERED_V5") else "CANNOT_CHECK (not a protected V5 run)") if rep["verdict"] == "MATCH" else "CANNOT_CHECK (replication mismatch)", "git_head_at_generation": head,
            "bound_files": {rel: sha(ROOT / rel) for rel in BOUND}, "deterministic_results": {"v5": ev["deterministic"], "study_status": ev["study_status"], "replication": rep["verdict"], "preregistration_sha256": ev["preregistration_sha256"], "stream_manifest_sha256": ev["stream_manifest_sha256"]},
...
def main(argv: list[str]) -> int:
    ...
    if "--verify" in argv:
        ...
        if drift:
            print("DRIFT:", drift)
            return 1
        print("M12 paired V5 receipt verified; bound files:", len(new["bound_files"]))
        return 0
